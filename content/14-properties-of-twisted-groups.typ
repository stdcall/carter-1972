#[
  ...
  #source(263, "250")
  = 14. Further Properties of the Twisted Groups <ch:14>
  ...
  == 14.1 The Finite Twisted Groups <sec:14-1>
  ...
  has type $A_l$, where $l>=2$, then $K$ admits an automorphism of order $2$, so
  must be the field $gf(q^2)$ for some prime-power $q$. The same applies if
  $frak(L)$ has type $D_l$ or $E_6$. If $frak(L)$ has type $D_4$ and the graph
  automorphism of $G$ used in the construction has order $3$, then $K$ must be a
  field admitting an automorphism of order $3$. Thus $K=gf(q^3)$ for some
  prime-power $q$. If $frak(L)$ has type $B_2$ or $F_4$, then $K$ is a field of
  characteristic $2$ admitting an automorphism $theta$ such that $2theta^2=1$.
  If $frak(L)$ has type $G_2$, $K$ is a field of characteristic $3$ admitting an
  automorphism $theta$ such that $3theta^2=1$. We determine which finite fields
  ...
  $lambda^(p theta^2)=lambda^(p^(2r+1))$, and so $lambda^(p^(2r+1))=lambda$ for
  all $lambda in K$. Thus $K$ is contained in $gf(p^(2r+1))$, hence $n$ divides
  ...
  diagram used in the construction. The finite twisted groups will be denoted by
  $
  ...
  $tw(2, F_4)(K)$ were discovered by Ree #bib-ref("ree1961f4", "ree1961twisted")
  ...
  #source(265, "252")
  #smallcaps[Proof.] Let $S$ be one of the equivalence classes in $Phi$ defined
  in #book-ref("l", "13-2-1", [13.2.1]). Then #book-ref("p", "13-6-3", [13.6.3])
  ...
  as in #book-ref("th", "7-1-1", [7.1.1]) where $lambda_i=chi(q_i)$. It follows
  that
  $
    sigma dot h(chi)
  ...
  $abs(Hhat^1)$ is the number of self-conjugate $K$-characters of $Q$. In order
  to determine the number of self-conjugate $K$-characters of $Q$ we consider
  the $rho$-orbits of $Pi$ and the corresponding $rho$-orbits of the set
  $q_1,dots,q_l$ of fundamental weights. Let $J$ be such a $rho$-orbit. If
  ...
  element of $gf(q^2)$ and the value at $overline(q)_i$ is then determined. Thus
  the values of $chi$ on this orbit can be chosen in $q^2-1$ ways. Similarly if
  $abs(J)=3$ the values of $chi$ on the corresponding orbit can be chosen in
  $q^3-1$ ways. Thus the number of self-conjugate $K$-characters of $Q$ is
  $ product_J (q^abs(J)-1) $ <eq:14-1-2-orbit-character-count>
  over all $rho$-orbits $J$ of $Pi$. However, the roots of $x^abs(J)-1$ are the
  eigenvalues of the isometry $tau$ of $frak(V)$ on the subspace spanned by the
  fundamental roots in $J$. Thus
  ...
  of $Q$ (#book-ref("th", "13-7-2", [13.7.2])). Thus there is a homomorphism of
  $Hhat^1$ onto $H^1$ obtained by restricting $chi$ from $Q$ to $P$. The kernel
  of this homomorphism is the set of self-conjugate $K$-characters $chi$ of $Q$
  which are the identity on $P$.
  #block(sticky: true)[
    Thus $abs(Hhat^1) slash abs(H^1)=d$, where $d$ is the order of the group of
    #source(266, "253")
  ...
  Suzuki or Ree group. Then $H^1$ consists of all $sigma$-invariant elements of
  $H$ by #book-ref("th", "13-7-4", [13.7.4]), since we have a finite field. Also
  $h(chi) in H^1$ if and only if
  ...
  $Q slash P$ given in section #book-ref(
    "sec",
    "8-6",
    [8.6],
  ).
  ...
  #block(sticky: true)[
    #smallcaps[Proof.] This follows from the double coset decomposition of $G^1$
    with respect to $B^1$. The number of elements in the double coset
...
#[
  ...
  #source(267, "254")
  == 14.2 Factorization of the Polynomial $P_(W^1)(t)$ <sec:14-2>

  As with the Chevalley groups, it is possible to simplify the expression
  obtained for the order of the twisted groups by finding a factorization of the
  polynomial
  ...
  in section #book-ref("sec", "9-4", [9.4]).
  ...
  Let $P_(W_J^1)(t)$ and $Pbar_(W_J^1)(t)$ denote the corresponding polynomials
  for the groups $W_J^1$, where $J$ is any $rho$-invariant subset of $Pi$. For
  each $rho$-invariant subset $J$ let $O_J$ be the number of $rho$-orbits in
  $J$. We shall show that $P_(W^1)(t)$ and $Pbar_(W^1)(t)$ satisfy the following
  identities:
  ...
  these identities with the ones in section #book-ref("sec", "9-4", [9.4]).)
  ...
  (c) We now consider the polynomial $Pbar_(W^1)(t)$. In order to show that this
  polynomial satisfies the above identity we must consider the operation of the
  Weyl group on the Coxeter complex. Note that the isometry $tau$ of $frak(V)$
  also operates on the Coxeter complex.
  ...
  of elements in the orbit containing $C_J$ which are fixed by $w tau$. We show
  that
  ...
  Let $frak(U)$ be the subspace of $frak(V)$ of elements fixed by $w tau$. Then
  the elements of the Coxeter complex which intersect $frak(U)$ are precisely
  those which are fixed by $w tau$. For if an element $frak(K)$ of the complex
  intersects $frak(U)$ it contains a point in common with $w tau(frak(K))$, and
  so $w tau(frak(K))=frak(K)$. Conversely, if $w tau(frak(K))=frak(K)$, $w tau$
  must fix some point of $frak(K)$ since $w tau$ is an isometry. Thus $frak(K)$
  intersects $frak(U)$.

  We now consider the complex in $frak(U)$ whose elements have form
  $frak(K) inter frak(U)$, where $frak(K)$ is an element of the Coxeter complex
  with $w tau(frak(K))=frak(K)$. We
  #source(269, "256")
  wish to find the dimension of $frak(K) inter frak(U)$, and consider first the
  case in which $frak(K)=C_J$ for some $J$. Since $w tau(C_J)=C_J$ we have
  ...
  the same way as $tau$. It follows that the elements of $C_J$ fixed by $w tau$
  ...
  Now $w$, $tau$ and $w tau$, being orthogonal transformations, have eigenvalues
  $1$, $-1$ or pairs of complex conjugates. Thus
  $
  ...
  #block(sticky: true)[
    (d) We prove next an analogue of #book-ref("p", "9-4-7", [9.4.7]) by showing
    that
  ]
  $ sum_J (-1)^(O_J) tr_((frak(I)_J)_n) tau=tr_(Ihat_n) tau, $
  <eq:14-2-1-trace-identity>
  where $(frak(I)_J)_n$ is the space of homogeneous polynomials on $frak(V)$ of
  degree $n$ invariant under $W_J$, $Ihat_n$ is the space of homogeneous
  ...
  function of $Wtau$ acting on $frak(S)_n$, the space of homogeneous polynomials
  ...
  of elements fixed by $T$. Since $T$ commutes with $tau$, $T tau$ leaves $M_0$
  and $M_1$ invariant. In fact $T tau=0$ on $M_0$ and $T tau=tau$ on $M_1$. Thus
  the trace of $T tau$ on $M$ is equal to the trace of $tau$ on $M_1$.
  ...
  as required. We observe that $tr_(Ihat_n) tau=tr_(frak(I)_(n-N)) tau$ since by
  #book-ref("l", "9-4-6", [9.4.6]) we have
  ...
  is the trace of $tau$ on $frak(I)_n$, as in #book-ref("p", "9-3-3", [9.3.3]).
  ...
  Since $frak(V)=frak(V)_J ⊕ frak(V)_J^perp$, where $W_J$ operates trivially on
  $frak(V)_J^perp$, the basic invariants of $W_J$ on $frak(V)$ may be taken as
  the basic invariants of $W_J$ on $frak(V)_J$ together with $l-k$ additional
  elements satisfying $tau(x_i)=eta_i x_i$, where $eta_(k+1),dots,eta_l$ are the
  eigenvalues of $tau$ on $frak(V)_J^perp$. Thus the coefficient of $t^n$ in
  $
    1/((1-epsilon_(J 1) t^(d_(J 1))) dots
    (1-epsilon_(J k) t^(d_(J k)))(1-eta_(k+1) t) dots (1-eta_l t))
...
#[
  ...
  == 14.3 The Orders of the Finite Twisted Groups <sec:14-3>
  ...
  #block(width: 100%, breakable: false)[
    ...
    $
    <eq:14-3-1-order>
    _where $d$ is as in section #book-ref("sec", "14-1", [14.1]) and
    $d_i,epsilon_i$ as in section #book-ref("sec", "14-2", [14.2])._
  ]

  #block(sticky: true)[
    #smallcaps[Proof.] We have shown in #book-ref("p", "14-2-1", [14.2.1]) that
  ]
  $
    sum_(w in W^1) t^(l(w))
    &=product_(i=1)^l lr(((1-epsilon_i t^(d_i))/(1-eta_i t))) \
  ...
  as in section #book-ref("sec", "3-6", [3.6]). Then $tau$ operates on the
  orthonormal basis $(e_i)$ of $frak(V)$ by
...
#[
  ...
  == 14.4 The Simplicity of the Twisted Groups <sec:14-4>
  ...
  contained in $H^1$. As in the proof of #book-ref("th", "11-1-2", [11.1.2]) we
  see that every element of $G_1^1$ commutes with every element of $U^1$ and
  every element of $V^1$. Let $h(chi) in G_1^1$. Since
  ...
  indecomposable Weyl group, or $abs(I)=2$ and the two elements of $I$ generate
  a dihedral group of order $16$. In either case it is impossible to decompose
  $I$ into non-empty complementary subsets which commute with one another. It is
  therefore sufficient by #book-ref("th", "11-1-1", [11.1.1]) to prove that
  $G^1=(G^1)'$. We shall show in fact that $(G^1)'$ contains $X_S^1$ for each
  equivalence class $S$ of $Phi$.

  (b) Suppose all the roots have the same length. Then the equivalence classes
  $S$ of $Phi$ are of type $A_1$, $A_1 times A_1$, $A_1 times A_1 times A_1$, or
  $A_2$. Let $h(chi)$ be an element of $H^1$. We use the following relations:
  ...
  assigning the fundamental-weight character values on an orbit adjacent to $r$
  gives $chi(r)$ as the inverse of a field norm, which takes both non-zero
  values of $K_0$.
  ...
  parameters, gives $chi(r)=c^2$. Since $K$ has more than three elements, we can
  again choose $chi(r) != 1$.
  ...
  If $S$ has type $A_1 times A_1$ or $A_1 times A_1 times A_1$, we see similarly
  that $X_S^1 subset.eq (G^1)'$, even if $K_0=upright("GF")(2)$.
  ...
  lies in $(G^1)'$. If $K_0 != upright("GF")(2)$ we can choose a self-conjugate
  $K$-character of $Q$ such that $chi(r+rbar) != 1$. For $lambda in K^*$ take
  $chi=chi_(r,lambda)chi_(rbar,lbar)$. Then $chi(r)=lambda^2/lbar$,
  $chi(rbar)=lbar^2/lambda$ and $chi(r+rbar)=lambda lbar$. Suppose
  $chi(r+rbar)=1$ for all $lambda$. Then $lambda lbar=1$ for all $lambda != 0$
  in $K$ and so $lambda^2=1$ for all $lambda != 0$ in $K_0$. Thus $K_0$ is a
  finite field. Let $K_0=upright("GF")(q)$. Then $lbar=lambda^q$ and
  $lambda^(q+1)=1$ for all $lambda != 0$ in $K$. Thus $mu=1$ for all $mu != 0$
  in $K_0$, so that $K_0=upright("GF")(2)$, the excluded case.

  Let $t_1,u_1$ be elements of $K$ satisfying $u_1+u1bar=-N_(r,rbar)t_1 t1bar$.
  Choose a self-conjugate character $chi$ of $Q$ such that $chi(r+rbar) != 1$
  and define $t,u in K$ by
  $
    t & =t_1/(chi(r)-1), \
    u & =u_1/(chi(r+rbar)-1)
  ...
  (c) Suppose all the roots have the same length and $K_0=upright("GF")(2)$. For
  each equivalence class $S$ of $Phi$ we define elements $x_S (t)$ or
  ...
  #book-ref(
    ...
    "13-3",
    [13.3],
  ) that this root system has type:
  $
  ...
  In each case, except $G^1=attach(A, tl: 2)_2$, the root system with Weyl group
  $W^1$ has roots of two different lengths, and two equivalence classes
  correspond to roots
  #source(278, "265")
  of the same length if and only if they have the same type. Now $W^1$ operates
  transitively on roots of a given length, and since
  $ n_w X_S^1 n_w^(-1)=X_(w(S))^1, quad w in W^1, $ <eq:14-4-class-conjugation>
  ...
  where $S_3={r+s,rbar+s}$ and $S_4={r+rbar+s}$. (We have used the fact that $K$
  has characteristic $2$.) Thus $x_(S_3)(t u)x_(S_4)(t tbar u) in (G^1)'$ for
  all $t in K$, $u in K_0$. However, $x_(S_3)(t u) in (G^1)'$ since $S_3$ has
  type $A_1 times A_1$. Thus $x_(S_4)(1) in (G^1)'$ and so
  ...
  If $G^1$ is of type $attach(A, tl: 2)_(2k)$, the equivalence classes $S_1,S_2$
  are of type $A_1 times A_1$ and $A_2$. Let $S_1={r,rbar}$ and
  $S_2={s,sbar,s+sbar}$. These fundamental roots $r,rbar,s,sbar$ of $frak(L)$
  may be chosen so they are related in the manner shown in the Dynkin diagram of
  $frak(L)$.
  ...
  If $G^1$ has type $attach(D, tl: 3)_4$ the equivalence classes correspond to a
  root system of type $G_2$. Let $S_1,S_2$ be classes corresponding to short
  ...
  equivalence classes $S$ of $Phi$ are of type $A_1 times A_1$, $B_2$ or $G_2$.
  Suppose that $G^1=attach(B, tl: 2)_2 (K)$ or $attach(F, tl: 2)_4 (K)$. Then
  the equivalence classes have type $A_1 times A_1$ or $B_2$. Let $h(chi)$ be an
  element of $H^1$. Then $chi(rbar)=chi(r)^(lambda(rbar)theta)$ by #book-ref(
    "th",
    "13-7-4",
    [13.7.4],
  ...
  Now let $S={a,b,a+b,2a+b}$ be an equivalence class of type $B_2$. Suppose that
  $alpha(t),beta(u)$ are defined as in #book-ref("p", "13-6-4", [13.6.4 (vi)]).
  Then
  ...
  by #book-ref("p", "13-6-4", [13.6.4 (vi)]). Since $beta(u) in (G^1)'$ for all
  $u$ we have $alpha((chi(b)-1)t) in (G^1)'$ for all $t$. As before we may
  ...
  #book-ref("th", "13-7-4", [13.7.4]) we showed that $H^1$ contains the element
  ...
  $lambda != 0$ in $K$, hence $lambda^4=1$ for all $lambda != 0$ in $K$. Now $K$
  is a field of characteristic $3$, so if $K != upright("GF")(3)$ we can choose
  $chi$ so that $chi(3a+2b) != 1$. It follows that $gamma(v) in (G^1)'$ for all
  $v in K$.
  ...
  follows that $lambda^(-6+6theta)=1$ and so $lambda^(-4)=1$. Thus $lambda^4=1$
  for all $lambda != 0$ in $K$. If $K != upright("GF")(3)$ we can therefore
  choose $chi$ so that $chi(3a+b) != 1$. Then $beta(u) in (G^1)'$ for all
  $u in K$.
  ...
  $attach(B, tl: 2)_2 (2)$, $attach(G, tl: 2)_2 (3)$, $attach(F, tl: 2)_4 (2)$.
  Since the subgroups $X_S^1$ generate $G^1$ we have $G^1=(G^1)'$ except in
  these cases. Thus $G^1$ is simple by #book-ref("th", "11-1-1", [11.1.1]). #qed
...
#[
  ...
  == 14.5 Identification with some Classical Groups <sec:14-5>
  ...
  _Here $epsilon$ is defined to be $1$ if $l$ is even and a non-zero element of
  $K$ satisfying $epsilon+overline(epsilon)=0$ if $l$ is odd._
  ...
  corresponding automorphism of the Lie algebra $frak(L)_K$. If we then combine
  this automorphism with the field automorphism $t -> tbar$ of $K$ we obtain a
  ‘semi-automorphism’ $psi$ of $frak(L)_K$ which satisfies the conditions
  $
  ...
  elements $x_r (t)$ for $r in plus.minus Pi$, $t in K$, generate $G$ it follows
  that the map $theta -> psi theta psi^(-1)$ transforms $G$ into itself. It is
  in fact an automorphism of $G$, for it is invertible and satisfies
  $
  ...
  $(l+1) times (l+1)$ matrices of trace $0$. The fundamental root vectors may be
  taken as $e_(i,i+1)$ for $i=0,1,...,l-1$ and the fundamental co-roots are then
  ...
  transpose of $M$) and $M -> A^(-1) M A$ are both automorphisms of $frak(L)_K$.
  Combining them we obtain an automorphism $M -> -A^(-1) M' A$. Under this
  automorphism we have
  $ e_(i j) -> (-1)^(i+j+1)e_(l-j,l-i). $ <eq:14-5-1-matrix-unit-image>
  Thus this automorphism behaves in the same way as the automorphism induced by
  the symmetry of the Dynkin diagram when operating on $e_(i,i+1)$ and
  ...
  #block(sticky: true)[
    Now we have seen that $G$ consists of the automorphisms of $frak(L)_K$ given
    by
  ]
  $ M -> T M T^(-1), quad T in upright("SL")_(l+1)(K). $
  <eq:14-5-1-inner-matrix-map>
  We consider which of these automorphisms commute with $psi$. In order for this
  to be so, $T$ must satisfy the condition
  $
  ...
  by its upper and lower unitriangular matrices. Thus $G^1$, the group generated
  by $U^1$ and $V^1$, consists of all transformations $M -> T M T^(-1)$, where
  $T in upright("SU")_(l+1)(K,f)$. Therefore $G^1$ is isomorphic to
  $upright("PSU")_(l+1)(K,f)$. #qed
  ...
  #block(sticky: true)[
    ...
    $K$ under the field automorphism used to define $attach(D, tl: 2)_l (K)$ and
    $f$ is the quadratic form_
  ]
  $
  ...
  generated by matrices $exp(t e_r)$, where $r in Pi$ or $-Pi$ and $t in K$, and
  all its matrices $T$ satisfy $T' A T=A$. If we use the matrix representation
  given in #book-ref("sec", "11-2-3", [11.2.3]), the matrices $exp(t e_r)$ for
  $r in Pi$ are
  $
  ...
  These matrices correspond to the nodes $1,2,...,l$ respectively in the Dynkin
  diagram
  ...
  and interchanges the last two. It is convenient to write the rows and columns
  in the order $1,2,...,l-1,-1,-2,...,-(l-1),l,-l$. Then
  ...
  we shall need to consider the matrices fixed under this transformation. These
  are the matrices $T in Omega_(2l)(K,f_D)$ which satisfy $B T=Tbar B$.
  ...
  We consider the conjugate subgroup $S^(-1) Omega_(2l)(K,f_D)S$ and investigate
  which matrices $M$ in this subgroup correspond to matrices
  ...
  $B_0 S_0=overline(S)_0$, we see by matrix multiplication that $B T=Tbar B$ if
  and only if
  $
  ...
  $V^1$ become the elementary orthogonal subgroups for the form $f$ over $K_0$.
  These generate $Omega_(2l)(K_0,f)$. The matrix group generated by these fixed
  root subgroups is therefore $S Omega_(2l)(K_0,f)S^(-1)$. Since $G^1$ is
  generated by $U^1$ and $V^1$, passing to the quotient by scalar matrices gives
