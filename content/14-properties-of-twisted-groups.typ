...
#let chapter = [
  #[
    #import "main-defs.typ": bib-ref, book-ref, mathclap, source
    #let gf = math.op("GF")
    ...
    #lemma(<l:exceptional-twisting-field>)[
      #block(width: 100%, breakable: false)[
        Let $K=gf(p^n)$ be a finite field of characteristic $p$ admitting an
        automorphism $theta$ satisfying $p theta^2=1$. Then $n$ is odd and
        $theta$ has the form
        $
    ...
    #proof[
      ...
      $lambda^(p^r)=lambda^(p^m)$. Hence $lambda^theta=lambda^(p^m)$ as
      required.
    ]

    Lemma #book-ref("l", "exceptional-twisting-field") shows that if $frak(L)$
    has type $B_2$ or $F_4$ then $K=gf(2^(2m+1))$ for some $m$, and if $frak(L)$
    has type $G_2$ then $K=gf(3^(2m+1))$ for some $m$.
    ...
    #lemma(<l:twisted-standard-subgroup-orders>)[
      #block(width: 100%, breakable: false)[
        #enum(
          numbering: "(i)",
          indent: 0pt,
        ...
        where $eta_1,dots,eta_l$ are the eigenvalues of the isometry $tau$ of
    ...
    #proof[
      ...
      ). Then #book-ref("p", "twisted-root-subgroup-elements") shows that
      $abs(X_S^1)=q^abs(S)$. By #book-ref(
        "p",
        "twisted-unipotent-factorization",
      ) it follows that $abs(U^1)=q^N$ and $abs((U_w^-)^1)=q^(l(w))$.

      We now consider $abs(H^1)$. Suppose all the roots have the same length.
      Then $sigma dot h_r (t)=h_rbar (tbar)$ by #book-ref(
        "l",
        "twisting-diagonal-elements",
      ). Let $h(chi)$ be an element of $H$, where $chi$ is a $K$-character of
      $Q$. Then
      $
      ...
      $lambda_i=chi(q_i)$. It follows that
      $
        sigma dot h(chi)
      ...
      consider the $rho$-orbits of $Pi$ and the corresponding $rho$-orbits of
      the set $q_1,dots,q_l$ of fundamental weights. Let $J$ be such a
      $rho$-orbit. If $abs(J)=1$ the value of a self-conjugate $K$-character
      $chi$ at the corresponding $q_i$ can be any non-zero element of $gf(q)$,
      so can be chosen in $q-1$ ways. If $abs(J)=2$ the value of $chi$ at $q_i$
      can be any non-zero element of $gf(q^2)$ and the value at $overline(q)_i$
      is then determined. Thus the values of $chi$ on this orbit can be chosen
      in $q^2-1$ ways. Similarly if $abs(J)=3$ the values of $chi$ on the
      corresponding orbit can be chosen in $q^3-1$ ways. Thus the number of
      self-conjugate $K$-characters of $Q$ is
      $
        product_J (q^abs(J)-1)
      ...
      $K$-character of $P$ which can be extended to a self-conjugate
      $K$-character of $Q$ (#book-ref("th", "simply-laced-twisted-torus")). Thus
      there is a homomorphism of $Hhat^1$ onto $H^1$ obtained by restricting
      $chi$ from $Q$ to $P$. The kernel of this homomorphism is the set of
      self-conjugate $K$-characters $chi$ of $Q$ which are the identity on $P$.
      #block(sticky: true)[
      ...
      Now suppose that there are roots of different lengths, i.e. that $G^1$ is
      a Suzuki or Ree group. Then $H^1$ consists of all $sigma$-invariant
      elements of $H$ by #book-ref("th", "unequal-length-twisted-torus"), since
      we have a finite field. Also $h(chi) in H^1$ if and only if
      $ chi(rbar)=chi(r)^(lambda(rbar)theta), quad r in Pi. $
      <eq:twisted-standard-subgroup-orders-exceptional-fixed-character>
      As before the values of $chi$ on a $rho$-orbit $J$ of $Pi$ can be chosen
      in $q^abs(J)-1$ ways. (Note that all $rho$-orbits now have two elements.)
      Thus
    ...
    _Note._ The number $d$ in #book-ref("l", "twisted-standard-subgroup-orders")
    takes the value $(l+1,q+1)$ for $tw(2, A_l)(q^2)$; $(4,q^l+1)$ for
    $tw(2, D_l)(q^2)$; $(3,q+1)$ for $tw(2, E_6)(q^2)$; and $1$ for
    $tw(3, D_4)(q^3)$.
    ...
    #proposition(<p:twisted-order-length-sum>)[
      #block(width: 100%, breakable: false)[
        $
    ...
    #proof[
      #block(sticky: true)[
        This follows from the double coset decomposition of $G^1$ with respect
        to $B^1$. The number of elements in the double coset $B^1 n_w B^1$,
        $w in W^1$, is $abs(B^1) dot abs((U_w^-)^1)$ by #book-ref(
          "p",
          "twisted-bruhat-form",
      ...
      by #book-ref("l", "twisted-standard-subgroup-orders"). The result follows.
    ]
  ...
  #[
    ...
    #proposition(<p:twisted-poincare-factorization>)[
      #block(width: 100%, breakable: false)[
        $
          sum_(w in W^1) t^(l(w))
    ...
    #proof[
      ...
      polynomials for the groups $W_J^1$, where $J$ is any $rho$-invariant
      subset of $Pi$. For each $rho$-invariant subset $J$ let $O_J$ be the
      number of $rho$-orbits in $J$. We shall show that $P_(W^1)(t)$ and
      $Pbar_(W^1)(t)$ satisfy the following identities:
      ...
      #source(268, "255")
      where the sums are taken over all $rho$-invariant subsets of $Pi$.
      (Compare these identities with the ones in section #book-ref(
        "sec",
        "solomon-theorem",
      ).)
      ...
      Let $J_w$ be the set of roots $r in Pi$ such that $w(r) in Phi^+$. Then
      the coefficient of $t^(l(w))$ in the above sum is
      $
      ...
      this polynomial satisfies the above identity we must consider the
      operation of the Weyl group on the Coxeter complex. Note that the isometry
      $tau$ of $frak(V)$ also operates on the Coxeter complex.

      #block(sticky: true)[
        Now each orbit of the Coxeter complex under the operation of $W$
        contains just one element $C_J$ by #book-ref(
          "p",
          "coxeter-face-orbit-representative",
        ), where
      ]
      $
        C_J={v;(v,r)=0 "for" r in J, quad (v,r)>0 "for" r in Pi-J}.
      $ <eq:twisted-poincare-factorization-fundamental-face>
      For each $rho$-invariant subset $J$ of $Pi$, let $n_J (w tau)$ be the
      number of elements in the orbit containing $C_J$ which are fixed by
      $w tau$. We show that
      ...
      the case in which $frak(K)=C_J$ for some $J$. Since $w tau(C_J)=C_J$ we
      have $w(C_(rho(J)))=C_J$ and so $rho(J)=J$ by #book-ref(
        "p",
        "coxeter-face-orbit-representative",
      ). Thus $tau(C_J)=C_J$ and $w(C_J)=C_J$. Therefore $w$ fixes every element
      of $C_J$ by #book-ref("cor", "face-stabilizer-fixes-pointwise") and so
      $w tau$ acts on $C_J$ in the same way as $tau$. It follows that the
      elements of $C_J$ fixed by $w tau$ are the elements of $C_J$ fixed by
      $tau$. However, $dim C_J=abs(Pi-J)$ and each $rho$-orbit of $Pi-J$
      contributes $1$ to the dimension of the $tau$-invariant elements of $C_J$.
      Thus we have
      $
      ...
      some $J$, by #book-ref("p", "coxeter-face-orbit-representative"). Then
      $
        w tau w'(C_J)=w'(C_J)
      ...
      We now apply #book-ref("l", "hyperplane-complex-euler-sum") to the complex
      induced in $frak(U)$. We have seen that the number of elements of this
      complex of dimension $i$ is given by
      ...
      #block(sticky: true)[
        (d) We prove next an analogue of #book-ref(
          "p",
          "alternating-invariant-dimension",
        ) by showing that
      ]
      ...
      $rho$-invariant subsets $J$ of $Pi$. Note that the terms
      $dim (frak(I)_J)_n$ and $dim Ihat_n$ in #book-ref(
        "p",
        "alternating-invariant-dimension",
      ) have been replaced here by the trace of $tau$ acting on these two
      subspaces.

      #block(sticky: true)[
        Consider the group $Wtau$ generated by $W$ and $tau$. The remarks in (c)
        show that the stabilizer of $C_J$ in $Wtau$ is $WJtau$. Therefore the
        unit character of $WJtau$ induced up to $Wtau$ satisfies
      ]
      $
        1_(WJtau)^(Wtau)(w tau)=n_J (w tau),
      $
      <eq:twisted-poincare-factorization-induced-unit>
      just as in #book-ref("l", "induced-character-face-count"). Now let $chi$
      be the trace function of $Wtau$ acting on $frak(S)_n$, the space of
      homogeneous polynomials of degree $n$. Then we have
      $
        chi_(WJtau)^(Wtau)(w tau) & =1_(WJtau)^(Wtau)(w tau) dot chi(w tau) \
                                  & =n_J (w tau) chi(w tau),
      $ <eq:twisted-poincare-factorization-induced-character>
      as in the proof of #book-ref("p", "alternating-invariant-dimension"). By
      (c) it follows that
      $
        sum_J (-1)^(O_J)
      ...
      as in the proof of #book-ref("p", "alternating-invariant-dimension"). Thus
      $
      ...
      set of elements fixed by $T$. Since $T$ commutes with $tau$, $T tau$
      leaves $M_0$ and $M_1$ invariant. In fact $T tau=0$ on $M_0$ and
      $T tau=tau$ on $M_1$. Thus the trace of $T tau$ on $M$ is equal to the
      trace of $tau$ on $M_1$.
      ...
      as required. We observe that $tr_(Ihat_n) tau=tr_(frak(I)_(n-N)) tau$
      since by #book-ref("l", "alternating-polynomial-factorization") we have
      $
      ...
      on $frak(V)_J^perp$, the basic invariants of $W_J$ on $frak(V)$ may be
      taken as the basic invariants of $W_J$ on $frak(V)_J$ together with $l-k$
      ...
      and so may deduce that $P_(W^1)(t)=Pbar_(W^1)(t)$ by induction on
      $abs(J)$. This completes the proof.
    ]
  ...
  #[
    ...
    #theorem(<th:finite-twisted-order>)[
      #block(width: 100%, breakable: false)[
        ...
        where $d$ is as in section #book-ref("sec", "finite-twisted-groups") and
    ...
    #proof[
      #block(sticky: true)[
        We have shown in #book-ref("p", "twisted-poincare-factorization") that
      ]
      $
        sum_(w in W^1) t^(l(w))
        &=product_(i=1)^l lr(((1-epsilon_i t^(d_i))/(1-eta_i t))) \
      ...
      #source(273, "260")
      since $d_1+dots+d_l=N+l$ by #book-ref("th", "degree-product-and-sum").
      Replacing $t$ by $t^(-1)$ we have
      ...
      The result now follows from #book-ref("p", "twisted-order-length-sum") on
      replacing $t$ by $q$.
    ]
    ...
    #theorem(<th:finite-twisted-order-table>)[
      #block(sticky: true)[
      ]
  ...
  #[
    ...
    #theorem(<th:twisted-group-simplicity>)[
      #block(sticky: true)[
        The twisted groups are all simple, except for
        $attach(A, tl: 2)_2 (2^2)$, $attach(B, tl: 2)_2 (2)$,
        $attach(G, tl: 2)_2 (3)$, $attach(F, tl: 2)_4 (2)$.
      ]
    ]

    #proof[
      (a) By #book-ref("th", "twisted-bn-pair") each twisted group $G^1$
      contains $B^1,N^1$ as a $(B,N)$-pair. We shall therefore use the criterion
      for simplicity given in #book-ref("th", "bn-pair-simplicity-criterion").
      Three out of the four conditions given in #book-ref(
        "th",
        "bn-pair-simplicity-criterion",
      ) are easily checked. $B^1$ is soluble since it is a subgroup of the
      soluble group $B=U H$. Let $G_1^1$ be a normal subgroup of $G^1$ contained
      in $B^1$. Then $G_1^1$ is contained in $U^1 H^1$ and its conjugate
      $V^1 H^1$, so $G_1^1$ is contained in $H^1$. As in the proof of #book-ref(
        "th",
        "chevalley-group-simplicity",
      ) we see that every element of $G_1^1$ commutes with every element of
      $U^1$ and every element of $V^1$. Let $h(chi) in G_1^1$. Since
      $
        h(chi)x_r (t)h(chi)^(-1)=x_r (chi(r)t),
      $ <eq:twisted-group-simplicity-torus-action>
      #source(276, "263")
      it follows from #book-ref("p", "twisted-root-subgroup-elements") that
      $chi(r)=1$ for each root $r in Phi$. This is because each root $r$ occurs
      in some equivalence class $S$, and so there is an element in $X_S^1$
      involving $r$. Hence $G_1^1=1$ and
      ...
      distinguished generators of $W^1$ is the set of elements $w_0^J$ as $J$
      runs over the $rho$-orbits of $Pi$. We have seen in section #book-ref(
        "sec",
        "twisted-weyl-group-structure",
      ) that these elements either form the set of fundamental reflections in
      some indecomposable Weyl group, or $abs(I)=2$ and the two elements of $I$
      generate a dihedral group of order $16$. In either case it is impossible
      to decompose $I$ into non-empty complementary subsets which commute with
      one another. It is therefore sufficient by #book-ref(
        "th",
        "bn-pair-simplicity-criterion",
      ...
      classes $S$ of $Phi$ are of type $A_1$, $A_1 times A_1$,
      $A_1 times A_1 times A_1$, or $A_2$. Let $h(chi)$ be an element of $H^1$.
      ...
      We use the self-conjugate characters of #book-ref(
        "th",
        "simply-laced-twisted-torus",
      ). By conjugating under $W^1$ we may suppose that $S=Phi_J^+$ for a
      $rho$-orbit $J$ of $Pi$. If $S={r}$ and $K_0 != upright("GF")(2)$, we can
      choose such a character with $chi(r) != 1$. Indeed, $h_r (c)$ gives
      $chi(r)=c^2$ for $c in K_0^*$. This suffices unless
      $K_0=upright("GF")(3)$. In that case, assigning the fundamental-weight
      character values on an orbit adjacent to $r$ gives $chi(r)$ as the inverse
      of a field norm, which takes both non-zero values of $K_0$.

      If $S$ has type $A_1 times A_1$ or $A_1 times A_1 times A_1$, the product
      of the elements $h_r (c)$ over the orbit, with $c in K^*$ and conjugate
      ...
      $u_1+u1bar=-N_(r,rbar)t_1 t1bar$. Choose a self-conjugate character $chi$
      of $Q$ such that $chi(r+rbar) != 1$ and define $t,u in K$ by
      $
        t & =t_1/(chi(r)-1), \
        u & =u_1/(chi(r+rbar)-1)
            -frac(N_(r,rbar)(chi(rbar)-1)t tbar, chi(r+rbar)-1).
      $ <eq:twisted-group-simplicity-a2-solve>
      Then $u+ubar=-N_(r,rbar)t tbar$ and
      $x_r (t_1)x_rbar (t1bar)x_(r+rbar)(u_1)$ is in $(G^1)'$. Thus
      $X_S^1 subset.eq (G^1)'$, as required.
      ...
      $x_S (t,u)$ of $X_S^1$ as in #book-ref(
        "p",
        "twisted-root-subgroup-products",
      ). We recall that it has been shown in (b) that $x_S (t) in (G^1)'$ if $S$
      has type $A_1 times A_1$ or $A_1 times A_1 times A_1$.
      ...
      group $W^1$ has roots of two different lengths, and two equivalence
      classes correspond to roots
      #source(278, "265")
      ...
      Suppose the Dynkin diagram of $W^1$ has a double bond. Let $S_1,S_2$ be
      two equivalence classes corresponding to fundamental roots joined by this
      double bond. If $G^1$ is not of type $attach(A, tl: 2)_(2k)$ these classes
      are of type $A_1 times A_1$ and $A_1$. Let $S_1={r,rbar}$ and $S_2={s}$.
      Then the commutator relations show that
      $
      ...
      $K$ has characteristic $2$.) Thus
      $x_(S_3)(t u)x_(S_4)(t tbar u) in (G^1)'$ for all $t in K$, $u in K_0$.
      However, $x_(S_3)(t u) in (G^1)'$ since $S_3$ has type $A_1 times A_1$.
      Thus $x_(S_4)(1) in (G^1)'$ and so $X_(S_4)^1 subset.eq (G^1)'$. Hence
      $(G^1)'$ contains a subgroup $X_S^1$ of type $A_1$ as well as one of type
      $A_1 times A_1$, thus $(G^1)'$ contains $X_S^1$ for all $S$.

      If $G^1$ is of type $attach(A, tl: 2)_(2k)$, the equivalence classes
      $S_1,S_2$ are of type $A_1 times A_1$ and $A_2$. Let $S_1={r,rbar}$ and
      $S_2={s,sbar,s+sbar}$. These fundamental roots $r,rbar,s,sbar$ of
      $frak(L)$ may be chosen so they are related in the manner shown in the
      Dynkin diagram of $frak(L)$.
      ...
      where $S_3={r+s+sbar,rbar+s+sbar}$ and
      $S_4={r+s,rbar+sbar,r+rbar+s+sbar}$. Thus
      $x_(S_3)(t vbar)x_(S_4)(t u,t tbar v) in (G^1)'$. However,
      $x_(S_3)(t vbar) in (G^1)'$ since $S_3$ has type $A_1 times A_1$, thus
      $x_(S_4)(t u,t tbar v) in (G^1)'$ also. Putting $t=1$ we have
      $x_(S_4)(u,v) in (G^1)'$ and so $X_(S_4)^1 subset.eq (G^1)'$. It now
      follows as before that $X_S^1 subset.eq (G^1)'$ for all $S$.

      If $G^1$ has type $attach(D, tl: 3)_4$ the equivalence classes correspond
      to a root system of type $G_2$. Let $S_1,S_2$ be classes corresponding to
      short roots of $G_2$ whose sum is short. $S_1,S_2$ both have type $A_1$.
      Let $S_1={r}$, $S_2={s}$. Then the commutator relations show that
      $
        [x_(S_2)(u),x_(S_1)(t)]=x_(S_3)(t u),
      $ <eq:twisted-group-simplicity-triality-commutator>
      where $S_3={r+s}$. Thus $x_(S_3)(1) in (G^1)'$ and so
      $X_(S_3)^1 subset.eq (G^1)'$. Hence $X_S^1 subset.eq (G^1)'$ for all $S$
      as before.
      ...
      by #book-ref("p", "twisted-root-subgroup-products", suffix: [ (v)]). Now
      by #book-ref("th", "unequal-length-twisted-torus") we can find an element
      $h(chi) in H^1$ for which $chi(rbar)$ takes any prescribed non-zero value
      in $K$. Thus if $K != upright("GF")(2)$ we may choose $chi$ so that
      $chi(rbar) != 1$. It follows that $X_S^1 subset.eq (G^1)'$.

      Now let $S={a,b,a+b,2a+b}$ be an equivalence class of type $B_2$. Suppose
      that $alpha(t),beta(u)$ are defined as in #book-ref(
        "p",
        "twisted-root-subgroup-products",
        suffix: [ (vi)],
      ). Then
      ...
      By #book-ref("th", "unequal-length-twisted-torus") we can choose
      $h(chi) in H^1$ so that $chi(a+b)$ takes any non-zero value in $K$. Thus
      $beta(u) in (G^1)'$ for all $u$, provided $K != upright("GF")(2)$. We also
      have
      $
      ...
      by #book-ref("p", "twisted-root-subgroup-products", suffix: [ (vi)]).
      Since $beta(u) in (G^1)'$ for all $u$ we have
      $alpha((chi(b)-1)t) in (G^1)'$ for all $t$. As before we may choose $chi$
      with $chi(b) != 1$ provided $K != upright("GF")(2)$. Thus
      ...
      #book-ref("th", "unequal-length-twisted-torus") we showed that $H^1$
      ...
      by #book-ref("p", "twisted-root-subgroup-products", suffix: [ (vii)]). Now
      ...
      by #book-ref("p", "twisted-root-subgroup-products", suffix: [ (vii)]). Now
      $chi(3a+b)=lambda^(-6theta+2)$. Suppose $chi(3a+b)=1$ for all $lambda$.
      Then $lambda^(-6theta+2)=1$ and so $lambda^(-2+2theta)=1$ since
      $6theta^2=2$. It follows that $lambda^(-6+6theta)=1$ and so
      $lambda^(-4)=1$. Thus $lambda^4=1$ for all $lambda != 0$ in $K$. If
      $K != upright("GF")(3)$ we can therefore choose $chi$ so that
      $chi(3a+b) != 1$. Then $beta(u) in (G^1)'$ for all $u in K$.

      Now consider the elements $alpha(t)$. We have
      $
        h(chi)alpha(t)h(chi)^(-1)=alpha(chi(b)t)
      $ <eq:twisted-group-simplicity-ree-alpha-action>
      and so, by #book-ref("p", "twisted-root-subgroup-products", suffix: [
        (vii)]), we have
      $
        h(chi)alpha(t)h(chi)^(-1)alpha(t)^(-1)
        =alpha((chi(b)-1)t)beta(u)gamma(v)
      $ <eq:twisted-group-simplicity-ree-alpha-commutator>
      for certain elements $u,v in K$. However, $beta(u)$ and $gamma(v)$ belong
      to $(G^1)'$ and therefore $alpha((chi(b)-1)t)$ is in $(G^1)'$ also. Now
      $chi(b)=-lambda^(6theta-4)$. Suppose $chi(b)=1$ for all $lambda$. Then
      $-lambda^(6theta-4)=1$ and so $-lambda^(2-4theta)=1$ since $6theta^2=2$.
      It follows that $lambda^(12theta-8)=1$ and $-lambda^(6-12theta)=1$, whence
      ...
      $attach(F, tl: 2)_4 (2)$. Since the subgroups $X_S^1$ generate $G^1$ we
      have $G^1=(G^1)'$ except in these cases. Thus $G^1$ is simple by
      #book-ref("th", "bn-pair-simplicity-criterion").
    ]
  ...
  #[
    ...
    #theorem(<th:twisted-unitary-identification>)[
      #block(sticky: true)[
      ...
      Here $epsilon$ is defined to be $1$ if $l$ is even and a non-zero element
      of $K$ satisfying $epsilon+overline(epsilon)=0$ if $l$ is odd.

      #source(282, "269")
    ]

    #proof[
      By #book-ref("th", "simple-lie-algebra-isomorphism") there is an
      ...
      combine this automorphism with the field automorphism $t -> tbar$ of $K$
      we obtain a ‘semi-automorphism’ $psi$ of $frak(L)_K$ which satisfies the
      conditions
      $
      ...
      transformation $psi theta psi^(-1)$ is easily seen to be an automorphism
      of $frak(L)_K$ and we have
      $
      ...
      generators $x_r (t)$ for $r in plus.minus Pi$, so is equal to $sigma$.
      Thus the elements of $G$ invariant under $sigma$ are those which commute
      with the semi-automorphism $psi$.

      In the present case $frak(L)_K$ may be taken as the Lie algebra of
      $(l+1) times (l+1)$ matrices of trace $0$. The fundamental root vectors
      may be taken as $e_(i,i+1)$ for $i=0,1,...,l-1$ and the fundamental
      co-roots are then
      $
        e_(i i)-e_(i+1,i+1), quad i=0,1,...,l-1.
      $ <eq:twisted-unitary-identification-simple-coroots>
      The automorphism of $frak(L)_K$ given by the symmetry of the Dynkin
      diagram acts as follows:
      $
      ...
      $V^1$ are precisely the upper and lower unitriangular matrices of the
      group $upright("SU")_(l+1)(K,f)$. However, $upright("SU")_(l+1)(K,f)$ is
      generated by its upper and lower unitriangular matrices. Thus $G^1$, the
      group generated by $U^1$ and $V^1$, consists of all transformations
      $M -> T M T^(-1)$, where $T in upright("SU")_(l+1)(K,f)$. Therefore $G^1$
      is isomorphic to $upright("PSU")_(l+1)(K,f)$.
    ]
    ...
    #source(284, "271")
    #theorem(<th:twisted-orthogonal-identification>)[
      #block(sticky: true)[
        $attach(D, tl: 2)_l (K)$ is isomorphic to the orthogonal group
        $P Omega_(2l)(K_0,f)$, where $K_0$ is the fixed field of $K$ under the
        field automorphism used to define $attach(D, tl: 2)_l (K)$ and $f$ is
        the quadratic form
      ]
      $
    ...
    #proof[
      It was shown in #book-ref("th", "classical-group-identifications") that
      ...
      Transformation by $B$ leaves the first $l-2$ matrices $exp(t e_r)$
      invariant and interchanges the last two. It is convenient to write the
      rows and columns in the order $1,2,...,l-1,-1,-2,...,-(l-1),l,-l$. Then
      $
      ...
      These are the matrices $T in Omega_(2l)(K,f_D)$ which satisfy
      $B T=Tbar B$.

      We now change the basis of the underlying $2l$-dimensional space over $K$
      so that the point which originally had coordinates
      $
        y_1,...,y_l,y_(-1),...,y_(-l)
