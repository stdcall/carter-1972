#let chapter = [
  #[
    ...
    shall show that these groups are all simple, apart from a few exceptions
    ...
    $S_3$. $A_1(3)$ has order $12$ and is isomorphic to the alternating group
    #source(189, "176")
  ...
  #[
    ...
    groups. In order to do this we first describe a matrix representation of
    each of the simple Lie algebras $A_l,B_l,C_l,D_l$. We refer to Jacobson’s
    book #bib-ref(
      "jacobson1962",
    ) for proofs of the statements about the simple Lie algebras which we
    require in this connection.

    === _The algebra $A_l$_ <sec:matrix-algebra-a>

    The Lie algebra $frak(L)$ of all $(l+1)times(l+1)$ matrices of trace $0$
    over $CC$ is isomorphic to $A_l$. Let $frak(H)$ be the subalgebra of all
    diagonal matrices in $frak(L)$ and $e_(i j)$ be the elementary matrix with
    ...
    in $frak(L)$. The elements of $frak(H)$ have form
    $
      h=mat(
        lambda_1, , , , , , , ;
        , lambda_2, , , , , , ;
    ...
    $theta(e_r)=-e_(-r)$ for each root $r$. Thus the fundamental co-roots
    together with the root vectors $e_r$ form a Chevalley basis, by the argument
    of #book-ref("sec", "matrix-algebra-a").
  ...
  #[
    ...
    simple Lie algebras $A_l,B_l,C_l,D_l$ which have just been described. In the
    first place it is evident that for all the matrices $e_r$ in these
    ...
    $e_r^2$ (when this is not zero) are all divisible by $2$. This fact enables
    us to transfer to an arbitrary field. For each matrix $e_r$ in one of the
    above representations and each element $t$ in an arbitrary field $K$,
    $exp(t e_r)$ is a well-defined non-singular matrix over $K$.
