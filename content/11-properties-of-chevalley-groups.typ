...
#[
  ...
  ] <ch:properties-of-chevalley-groups>
  ...
  groups of type $A_l,B_l,C_l,D_l$ are #ix(
    "simplicity, of Chevalley groups",
    sub: "of groups with $(B,N)$-pair",
  )isomorphic to certain classical groups.
  ...
  $(B,N)$-pair. We recall from section @sec:bn-pairs that in such a group $G$
  the quotient group $W=N\/(B ∩ N)$ is generated by a set of elements $w_i$,
  $i in I$, such that $w_i^2=1$. Let $G'$ denote the #nx(
    "G-prime",
    [$G'$],
    group: "latin-g",
  )[The commutator subgroup (derived group) of $G$]#nx(
    "G-prime",
    [$G'$],
    group: "latin-g",
  )[The commutator subgroup (derived group) of $G$]commutator subgroup of $G$.

  #ix(
    "Tits’ theorems, on finite groups with $(B,N)$-pair",
    sub: "on simplicity of groups with $(B,N)$-pair",
  )#theorem()[
    #block(sticky: true)[
      ...
      [the set $I$ cannot be decomposed into two non-empty complementary subsets
        $J,K$ such that $w_j$ commutes with $w_k$ for all $j in J$, $k in K$.],
    )
    Then $G$ is simple.
  ...
  #proof[
    ...
    containing $B$, thus $G_1 B=P_J$ for some subset $J$ of $I$, by
    @th:subgroups-containing-borel. Let $K$ be the subset of $I$ complementary
    to $J$. Let $j in J$ and $k in K$, and let $n_j,n_k$ be elements of $N$
    corresponding to $w_j,w_k in W$ under the natural homomorphism. If we define
    the length function $l(w)$ as in the proof of
    @p:bruhat-double-coset-uniqueness we have $l(w_j w_k)>l(w_k)$. By
    @p:increasing-length-bruhat-product we see that
    ...
    by axiom @def:bn-4[_BN_ 4]. Thus we have
    $
    ...
    $w_k in W_J$. This contradicts the fact, shown in the proof of
    @th:abstract-parabolic-lattice, that the elements $w_i$ form a minimal set
    of generators for $W$. Hence we have
    $
      B n_k n_j n_k B ∩ G_1 != emptyset.
    ...
    by @th:abstract-parabolic-lattice. It follows that $w_k w_j w_k$ is either
    $1$ or $w_j$. $w_k w_j w_k=1$ implies that $w_j=1$, which is impossible;
    thus we have $w_k w_j w_k=w_j$. Hence for each $j in J$, $k in K$ we have
  ...
  Theorem @th:bn-pair-simplicity-criterion shows that groups with a $(B,N)$-pair
  have a definite tendency to be simple. Now the Chevalley groups $G=frak(L)(K)$
  all have
  #source(185, "172")
  $(B,N)$-pairs by @p:chevalley-bn-pair. Thus we may use
  @th:bn-pair-simplicity-criterion to try #ix(
    "simplicity, of Chevalley groups",
  )to prove the simplicity of the Chevalley groups. The proof goes through in
  all but a few cases.

  #theorem()[
    #block(sticky: true)[
      (i) Let $frak(L)$ be a simple Lie algebra over $CC$ and $K$ be an
      arbitrary field. Then the Chevalley group $G=frak(L)(K)$ is simple, except
      for $A_1(2)$, $A_1(3)$, $B_2(2)$, $G_2(2)$.
    ]
    (ii) Each Chevalley group (even a non-simple one) has trivial centre.
  ] <th:chevalley-group-simplicity>

  #proof[
    #block(sticky: true)[
      By @th:bn-pair-simplicity-criterion it will be sufficient to prove that
    ]
    ...
    Let $G_1$ be such a normal subgroup. Now by @p:longest-weyl-element the Weyl
    group $W$ contains an element $w_0$ which transforms every positive root
    into a negative root. Let $n_0$ be a corresponding element of $N$. Then we
    have
    $ n_0 U n_0^(-1)=V $ <eq:chevalley-group-simplicity-opposite-unipotent>
    by @l:monomial-conjugation-root-subgroups, and also
    ...
    by @cor:opposite-borel-intersection. However, $H$ normalizes $U$ and so we
    have
    ...
    argument used in the proof of @th:bn-pair-simplicity-criterion shows that
    either
    $ G=Z B quad "or" quad Z subset.eq inter.big_(g in G) g B g^(-1). $
    <eq:chevalley-group-simplicity-centre-dichotomy>
    If $G=Z B$ then $B$ is normal in $G$. Since by
    @th:parabolic-normalizers-and-conjugacy $B$ is its own normalizer in $G$ we
    have $Z B=B$. Therefore $Z subset.eq B$, and since $Z$ is normal we have
    $
    ...
    #lemma()[
      #block(sticky: true)[
        (i) Let $r in Phi$ and $t$ be a non-zero element of $K$. Let $Q$ be the
        additive group of weights. Then there is a $K$-character $chi$ of $Q$
        such that $chi(r)=t^2$.
      ]
      (ii) There is a $K$-character $chi$ of $Q$ such that $chi(r)=t$, unless
      $frak(L)=A_1$ or $frak(L)=C_l$ and $r$ is a long root.
    ] <l:weight-character-values>

    #proof[
      #block(sticky: true)[
        (i) Let $chi_(r,t)$ be the $K$-character of $Q$ defined, as in
        @th:diagonal-subgroup-characters, by
      ]
      $
      ...
      section @sec:diagonal-subgroup that $P,Q$ are free abelian groups of rank
      $l$ and that $P$ is a subgroup of $Q$ of finite index. Let $m$ be the
      greatest integer such that $(1\/m)r in Q$. ($m$ is the highest common
      factor of the coefficients of $r$ when expressed as an integral
      ...
      for any $n>1$, and therefore $r_1$ forms part of some basis for $Q$. Hence
      a $K$-character $chi$ of $Q$ can be chosen so that $chi(r_1)$ is an
      arbitrary non-zero element of $K$. Let
      $ h_r=frac(2r, lr((r,r))) $ <eq:weight-character-values-coroot>
      #source(187, "174")
      be the co-root corresponding to $r$. Then $(h_r,r_1) in ZZ$ since
      $r_1 in Q$, by @sec:diagonal-subgroup. Thus
      $
      ...
      by @sec:diagonal-subgroup, and $frac(1, 2)r in Q$ only if each $A_(j i)$
      for $j=1,dots,l$ is divisible by $2$. A glance at the Cartan matrices of
      the simple Lie algebras (listed in section
      @sec:description-of-simple-lie-algebras) shows that this can happen only
      if $frak(L)=A_1$ or if $frak(L)=C_l$, $(l>=2)$, and $r$ is the long
      fundamental root. If $r$ is not a fundamental root it can be transformed
      into one by an element of $W$ and the property $frac(1, 2)r in Q$ is
      preserved. Thus $frac(1, 2)r in Q$ holds only for the roots of $A_1$ and
      the long roots of $C_l$.
    ]

    We may now complete the proof of @th:chevalley-group-simplicity and to do so
    we must show that $G=G'$.

    Suppose first that $K$ has at least four elements. Then there is a non-zero
    element $t in K$ such that $t^2!=1$. By @l:weight-character-values there
    exists for each $r in Phi$ a $K$-character $chi$ of $Q$ such that
    $chi(r)!=1$. Then $h(chi) in H$ by @th:diagonal-subgroup-characters and we
    have
    $
    ...
    $C_l$, @l:weight-character-values shows that there exists for each
    ...
    conjugate subgroups of $G$ by @l:monomial-conjugation-root-subgroups, and so
    ...
    $K$-character $chi$ of $Q$ such that $chi(r)!=1$, by
    @l:weight-character-values. Thus $X_r$ is contained in $G'$ in this case.
    Let $s$ be the long fundamental root of $C_l$ and $r$ the fundamental root
    joined to it in the Dynkin diagram. Then we have
...
#[
  ...
  === _The algebra $A_l$_ <sec:matrix-algebra-a>
  ...
  Thus we evidently have a root system of type $A_l$ (cf. section
  @sec:description-of-simple-lie-algebras) and the root space negative to
  $CC e_(i j)$ is $CC e_(j i)$. Let
  ...
  Hence $N_(-r,-s)=-N_(r,s)$. Since by @th:structure-constant-relations we have
  ...
  #lemma()[
    #block(sticky: true)[
      Let $A$ be an $n times n$ matrix over $CC$. Then the $n times n$ matrices
      $T$ satisfying
    ]
  ...
  ] <l:bilinear-form-exponential>

  #proof[
    The set of matrices $T$ satisfying $T’A+A T=0$ is closed under addition and
    scalar multiplication. Let $T_1,T_2$ satisfy this condition and consider the
    Lie product $[T_1 T_2]=T_1 T_2-T_2 T_1$. We have
    $
  ...
  #block(sticky: true)[
    in @l:bilinear-form-exponential and consider $2l times 2l$ matrices
  ]
  ...
  Thus $frak(L)$ is a simple algebra of type $D_l$ (compare section
  @sec:description-of-simple-lie-algebras) and the above decomposition is a
  Cartan decomposition. The co-roots of $frak(L)$ are the elements
  $
  ...
  For in each case we have $[h_r e_r]=2e_r$. As in @sec:matrix-algebra-a the map
  $theta(x)=-x'$ is an automorphism of $frak(L)$ satisfying $theta(e_r)=-e_(-r)$
  for each root $r$. Thus the fundamental co-roots together with the root
  vectors $e_r$ form a Chevalley basis, by the argument of
  @sec:matrix-algebra-a.
  ...
  form a Lie algebra as in @l:bilinear-form-exponential.
  ...
  decomposition giving a root system of type $B$ (cf. section
  @sec:description-of-simple-lie-algebras). The co-roots of $frak(L)$ are the
  elements
  $
    [e_(i j)-e_(-j,-i),-e_(-i,-j)+e_(j i)]=e_(i i)-e_(j
    j)-e_(-i,-i)+e_(-j,-j), \
  ...
  defined above form a Chevalley basis of $frak(L)$, as in
  @sec:matrix-algebra-d.
...
#[
  ...
  ] <sec:classical-group-identifications>
  ...
  for all $r in Phi$ and all $t in K$. By @l:exponential-conjugation we have
  $
  ...
  #lemma()[
    #block(sticky: true)[
      The kernel of the homomorphism $sigma:overline(G)->G$ is the centre $Z$ of
      $overline(G)$.
    ]
  ] <l:adjoint-kernel-centre>

  #proof[
    Let $y in overline(G)$ be in the kernel of $sigma$. Then $y x y^(-1)=x$ for
    all $x in frak(L)_K$. In particular $y$ commutes with $t e_r$ for all
    $r in Phi$. Thus $y$ commutes with $exp(t e_r)$ and so $y$ is in the centre
    $Z$ of $overline(G)$.
  ...
  We are now #ix("Ree’s theorem")able to prove a theorem of Ree identifying
  Chevalley groups of type $A_l,B_l,C_l,D_l$ with classical groups.

  #theorem()[
    #block(sticky: true)[
      (i) $A_l (K)$ #ix("linear groups")is isomorphic to the linear group
      $op("PSL")_(l+1)(K)$.
    ]

    #block(sticky: true)[
      (ii) $B_l (K)$ #ix("orthogonal groups")is isomorphic to the orthogonal
      group $P Omega_(2l+1)(K,f_B)$, except for $B_2 (2)$, where $f_B$ is the
      quadratic form
    ]
    ...
    (iii) $C_l (K)$ #ix("symplectic groups")is isomorphic to the symplectic
    group $op("PSp")_(2l)(K)$.
  ...
  ] <th:classical-group-identifications>

  #source(198, "185")
  Note that $f_B$ and $f_D$ are both quadratic forms of maximal index in the
  spaces concerned (cf. section @sec:orthogonal-groups).
  ...
  #proof[
    ...
    Thus, by @l:adjoint-kernel-centre,
    $ G tilde.eq overline(G)/Z tilde.eq op("PSL")_(l+1)(K). $
    <eq:classical-group-identifications-identification-a>

    (iv) Let $G=D_l (K)$. Then, by @l:bilinear-form-exponential and
    @sec:matrix-algebra-d, $overline(G)$ is a group of $2l times 2l$ matrices
    generated by elements $T$ satisfying $T’A T=A$, where
    ...
    and it follows by @l:adjoint-kernel-centre that
    ...
    Such #ix("symplectic groups")matrices are elements of the symplectic group
    $op("Sp")_(2l)(K)$. $overline(G)$ is generated by the matrices
    $
      cases(
        I+t(e_(i j)-e_(-j,-i)) ",",
        I-t(e_(-i,-j)-e_(j i)) ",",
    ...
    Now in the set of generators for $overline(G)$ described in
    @th:classical-group-identifications (ii), the terms involving $e_(i 0)$ and
    $e_(-i,0)$ vanish when $K$ has characteristic $2$. Hence the subspace
    $frak(W)$ of $frak(V)$ with basis $v_1,dots,v_l,v_(-1),dots,v_(-l)$ is
    invariant under $overline(G)$. We show that $overline(G)$ operates
    faithfully on $frak(W)$.

    We remarked in @l:adjoint-kernel-centre that the centre $Z$ of $overline(G)$
    consists only of scalar multiples of the identity. However, the elements of
    $overline(G)$ satisfy $T’A T=A$, where
    $
      A=mat(2, 0, 0; 0, 0, I_l; 0, I_l, 0)
    $ <eq:classical-group-identifications-matrix-b-two>
    and $lambda I$ can only satisfy this if $lambda^2=1$. In characteristic $2$
    this implies that $lambda=1$. Hence $Z=1$ and $overline(G)$ is isomorphic to
    $G$. It follows that $overline(G)$ is simple. (We exclude the exceptional
    case $B_2 (2)$.) Thus $overline(G)$ acts faithfully on $frak(W)$.

    Now the quadratic form $f_B$ in characteristic $2$ is non-degenerate but has
    defect $1$ (see section @sec:orthogonal-groups-characteristic-two). The
    vector space $frak(V)$ on which $f_B$ is defined therefore has non-singular
    symplectic subspaces of codimension $1$, and $frak(W)$ is such a subspace.
    We therefore compare the action of $overline(G)$ on $frak(W)$ with the
    action of the symplectic group $op("Sp")_(2l)(K)$.
    ...
    #source(201, "188")
    If these generators are compared with the ones in
    @th:classical-group-identifications (iii) it is clear that $overline(G)$ is
    a subgroup of $op("Sp")_(2l)(K)$. Moreover if $K$ is a perfect field each
    element of $K$ is a square and so $overline(G)=op("Sp")_(2l)(K)$. Thus
    $
      overline(G)=op("Sp")_(2l)(K)=O_(2l+1)(K,f_B)
    $ <eq:classical-group-identifications-perfect>
    as in section @sec:orthogonal-groups-characteristic-two and so
    $
    ...
    $op("Sp")_(2l)(K)$ (cf. section @sec:orthogonal-groups-characteristic-two).
