#[
  ...
  #source(117, "104")
  = 8. The Bruhat Decomposition <ch:8>

  == 8.1 Bruhat’s Lemma <sec:8-1>

  Let $B$ be the subgroup $U H$ of the Chevalley group $G$. In the case when the
  field $K$ is algebraically closed the subgroup $B$ and its conjugates in $G$
  are usually called Borel subgroups of $G$. In this case $G$ may be interpreted
  as an algebraic group and the Borel subgroups are precisely the maximal
  soluble connected subgroups of $G$. It was proved by Borel that any two such
  subgroups are conjugate (cf. Borel #bib-ref("borel1956")). We shall
  ...
  consider the subgroup $[X_(-r),U_r]$. Suppose $s$ is a positive root distinct
  from $r$ and suppose $-i r+j s in Phi$, where $i>0$, $j>0$. Then
  ...
  $U_r X_r$ is a subgroup of $U$. However, it contains all the root subgroups of
  $U$, so is the whole of $U$. #qed
  ...
  #block(sticky: true)[
    ...
    $X_(-r) subset.eq B union B n_r B$, i.e. that $x_(-r)(t) in B union B n_r B$
    for all $t in K$. If $t=0$, $x_(-r)(t) in B$. If $t != 0$ we have
  ]
  $
  ...
  #block(breakable: false)[
    ...
    #smallcaps[Proposition 8.1.5.] _Let $r in Pi$, $n in N$ and $w$ be the image
    of $n$ under the natural homomorphism from $N$ to $W$. Then_
  ...
  #block(sticky: true)[
    Now suppose that $w(r) in Phi^-$. Let $w'=w w_r$. Then $w'(r) in Phi^+$. Let
    $n'$ be an element of $N$ mapping to $w'$ under the natural homomorphism.
    Then we have
  ]
  ...
  <eq:8-1-5-inclusion>
  #block(sticky: true)[
    If $w(r) in Phi^-$, the set $B n B dot B n_r B$ contains elements from both
    $B n n_r B$ and $B n B$. For $n n_r$ is an element in this set from
  ...
  $g^(-1) in B n^(-1)B dot B n_r B$. Hence, by #book-ref("p", "8-1-5", [8.1.5]),
...
#[
  ...
  == 8.2 Groups with a $(B,N)$-Pair <sec:8-2>
  ...
  useful not merely for deriving further properties of the Chevalley groups, but
  also in connection with the ‘twisted groups’ discussed in chapters #book-ref(
    "ch",
    "13",
    [13],
  ) and #book-ref("ch", "14", [14]), and with the associated geometrical
  structures described in #chapter-ref(15).
  ...
  #block(sticky: true)[
    ...
    have $B ∩ N=H$, and by #book-ref("th", "7-2-2", [7.2.2]) we know that $H$ is
    normal in $N$ and that $N\/H$ is isomorphic to the Weyl group $W$. The Weyl
    group is generated by the fundamental reflections $w_r$, $r in Pi$, and we
    take these as the generating involutions in #book-ref("def", "bn-3", [_BN_
      3]). By #book-ref("cor", "8-1-6", [8.1.6]) we have, for each $r in Pi$,
  ]
  $ B n_r B dot B n B subset.eq B n_r n B union B n B. $
  <eq:8-2-1-bn-4>
  // E032: conjugating X_r requires n_r^(-1) on the right.
  #block(sticky: true)[
    If $r in Pi$ we have $X_r subset.eq B$, but $n_r X_r n_r^(-1)=X_(-r)$ is not
    in $B$. Thus $n_r B n_r != B$. Finally, $G$ is generated by $B$ and $N$. For
    $G$ is generated by the root subgroups $X_r$, and each root $r$ has the form
    $w(r_i)$ for some $w in W$ and $r_i in Pi$. Let $n$ be an element of $N$
    mapping to $w$ under the natural homomorphism. Then
  ]
  ...
  #block(breakable: false)[
    ...
    Then_
      ...
      [_$G=B N B$._],
      [
        _For each subset $J$ of $I$, let $W_J$ be the subgroup of $W$ generated
        by the elements $w_i$ for $i in J$, and $N_J$ be the subgroup of $N$
        mapping to $W_J$ under the natural homomorphism. Then $P_J=B N_J B$ is a
        subgroup of $G$._
      ],
    )
  ]
  ...
  #block(sticky: true)[
    #smallcaps[Proof.] We prove (ii) first. $B N_J B$ is certainly closed under
    inversion, and so we must prove it closed under multiplication. Let
    $n in N_J$. Then $n$ may be written in the form $n=n_1 n_2 dots n_k$, where
    each $n_i$ maps under the natural homomorphism to a generator $w_i$ of $W$
    with $i in J$. Thus
  ]
  $
  ...
  We now take $J=I$. Then $B N B$ is a subgroup of $G$. This subgroup certainly
  contains $B$ and $N$, and must be $G$ since $B$ and $N$ generate $G$. #qed
  ...
  #block(breakable: false)[
    ...
    #smallcaps[Proposition 8.2.3.] _Let $G$ be a group with a $(B,N)$-pair. Let
    $n,n'$ be elements of $N$. Then $B n B=B n' B$ if and only if $n,n'$ map to
    the same element of $W$ under the natural homomorphism from $N$ into $W$.
    Thus there is a natural 1–1 correspondence between double cosets of $B$ in
    $G$ and elements of $W$._
  ]
  ...
  $i in I$. We denote by $l(w)$ the shortest length of any expression for $w$ as
  a product of such generators. Suppose $B n B=B n' B$, where $n,n'$ correspond
  to $w,w'$ respectively, and suppose that $l(w)<=l(w')$. We show that $w=w'$ by
  induction on $l(w)$. If $l(w)=0$ we have $w=1$ and $B n B=B$. Thus $B n' B=B$
  and so $n' in B ∩ N$. Hence $w'=1$.

  #block(sticky: true)[
    Now suppose $l(w)>0$. Then $w=w_i w''$, where $i in I$ and $l(w'')=l(w)-1$.
    Let $n_i,n''$ be elements of $N$ corresponding to $w_i,w'' in W$. Then
  ]
  ...
  $w''=w_i w'$ or $w''=w'$. The latter is impossible since $l(w'')<l(w')$. Thus
  $w''=w_i w'$ and so $w'=w_i w''=w$.
  ...
  #block(breakable: false)[
    ...
    #smallcaps[Proposition 8.2.4.] _Let $G$ be a group with a $(B,N)$-pair. Let
    $w_i$, $i in I$, be one of the distinguished generators of $W$ and $w$ be an
    element of $W$ such that $l(w_i w)>=l(w)$. Let $n_i,n$ be elements of $N$
    mapped by the natural homomorphism into $w_i,w in W$. Then
  ...
  #block(sticky: true)[
    ...
    $B n_i B dot B n B$ intersects $B n B$, by #book-ref("def", "bn-4", [_BN_
      4]). Let $n',n_j$ be elements of $N$ mapped by the natural homomorphism to
    $w',w_j in W$. Then
  ]
  ...
  #book-ref(
    ...
    "8-2-3",
    [8.2.3],
  ), $w_i w'=w w_j$ or $w_i w'=w$. The former equation gives $w_i w'=w'$, whence
  $w_i=1$, which contradicts #book-ref("def", "bn-5", [_BN_ 5]). The latter
  gives $w_i w=w'$, which implies $l(w_i w)<l(w)$, a contradiction. #qed
  ...
  #block(breakable: false)[
    ...
    #smallcaps[Proposition 8.2.5.] _Suppose the notation is as in #book-ref(
      ...
      "8-2-4",
      [8.2.4],
    ). Then, if $l(w_i w)<=l(w)$, $B n_i B dot B n B$ has non-empty intersection
    with $B n B$._
  ]
...
#[
  ...
  == 8.3 Parabolic Subgroups <sec:8-3>
  ...
  show that the subgroups $P_J$ for the various subsets $J$ of $I$ are the only
  subgroups of $G$ containing $B$, and therefore that every parabolic subgroup
  is conjugate to some $P_J$. Furthermore, the subgroups $P_J$ are all distinct
  and non-conjugate in $G$, and each of them is equal to its normalizer.
  ...
  #block()[
    ...
    _where $l(w)=k$. Let $J$ be the subset $lr({i_1,i_2,dots,i_k})$ of $I$. Then
    the following three subgroups of $G$ are equal:_
    #enum(
      numbering: "(i)",
      indent: 1.1em,
      body-indent: 0.5em,
  ...
  #block(sticky: true)[
    ...
    $n_(i_alpha) in N$ corresponds to $w_(i_alpha) in W$. Each $n_(i_alpha)$ is
    in $N_J$, so $n in N_J$. Thus we have the inclusions
  ]
  ...
  and arguing similarly we see that each of $n_(i_1),n_(i_2),dots,n_(i_k)$ lies
  in $lr(⟨B,n B n^(-1)⟩)$. Hence $P_J$ is contained in $lr(⟨B,n B n^(-1)⟩)$ and
  the result follows. #qed

  #block(breakable: false)[
    #metadata((kind: "anchor")) <th:8-3-2>
    #smallcaps[Theorem 8.3.2.] _Let $G$ be a group with a $(B,N)$-pair. Then the
    subgroups $P_J$ are the only subgroups of $G$ containing $B$._
  ]

  #block(sticky: true)[
    #smallcaps[Proof.] Let $M$ be a subgroup of $G$ containing $B$. $M$ is a
    union of double cosets of $B$ in $G$, and each such double coset contains an
    element of $N$. Thus $M$ is generated by $B$ and a certain set of elements
    of $N$. Let $n_alpha$ be any element of $N$. Then, by #book-ref(
      "p",
      "8-3-1",
      [8.3.1],
  ...
  #block(breakable: false)[
    ...
    each subgroup $P_J$ of $G$ is equal to its normalizer. Furthermore distinct
    subgroups $P_J,P_K$ cannot be conjugate in $G$._
  ]
  ...
  #block(breakable: false)[
    ...
    #smallcaps[Theorem 8.3.4.] _Let $G$ be a group with a $(B,N)$-pair. Then the
    subgroups $P_J$ for distinct subsets $J$ of $I$ are all distinct.
    Furthermore we have $P_J ∩ P_K=P_(J ∩ K)$. Thus the subgroups $P_J$ form a
    lattice isomorphic to the lattice of subsets of $I$._
  ]

  #block(sticky: true)[
    #smallcaps[Proof.] It is necessary to show first that the elements $w_i$ of
    $W$ for $i in I$ form a minimal set of generators of $W$. Suppose we remove
    one element $j$ from $I$, and assume by way of contradiction that $W$ is
    generated by the elements $w_i$ for $i in I-{j}$. Then $G$ has a
  ...
  let $J={i_1,i_2,dots,i_k}$. Then, by #book-ref("p", "8-3-1", [8.3.1]), we have
  $lr(⟨B,n_j⟩)=B N_J B$. However, $lr(⟨B,n_j⟩)=B union B n_j B$ by #book-ref(
    "p",
    "8-3-1",
    [8.3.1],
  ...
  containing $B$, thus $P_J ∩ P_K=P_L$ for some subset $L$ of $I$ by #book-ref(
    "th",
    "8-3-2",
    [8.3.2],
  ...
  remaining generators of $W$, which is impossible. Thus $P_J ∩ P_K=P_(J ∩ K)$.
  ...
  we have $W_J=W_(J ∩ K)$, as before. This again means that some generator $w_i$
  is expressible in terms of the remainder, which gives a contradiction. Thus
  $P_J=P_K$ implies that $J=K$. #qed

  We emphasize here that all the results of sections #book-ref(
    "sec",
    "8-2",
    [8.2],
  ) and #book-ref("sec", "8-3", [8.3]) have been obtained using only the axioms
  for a $(B,N)$-pair.
]
...
#[
  ...
  == 8.4 A Canonical Form <sec:8-4>

  We now return to the situation in which $G$ is a Chevalley group $frak(L)(K)$.
  The results just proved on the double coset decomposition are valid for $G$
  since $G$ has a $(B,N)$-pair. Thus every element of $G$ can be written in the
  form $b_1 n b_2$, where $b_1,b_2 in B$ and $n in N$. However, it may be
  possible to express an element of $G$ in such a form in a number of different
  ways. We seek a canonical form for elements of $G$, i.e. a way of decomposing
  an element of $G$ so that each element has a unique expression in the given
  form.
  ...
  #block(sticky: true)[
    ...
    root $i r+j s$ is in $Psi$ also. We consider the situation in which $Phi^+$
    is expressed as the disjoint union of two closed subsets. Suppose
    $Phi^+=Psi_1 union Psi_2$, where $Psi_1 ∩ Psi_2$ is empty and $Psi_1,Psi_2$
    are closed. Let
  ]
  $
  ...
  Since $u in U^1$ we have $t_i=0$ whenever $r_i in Psi_2$. Since $u in U^2$ we
  have $t_i=0$ whenever $r_i in Psi_1$. Thus $t_i=0$ for all $i$ and $u=1$.
  ...
  #block(breakable: false)[
    ...
    #smallcaps[Corollary 8.4.2.] _Each element $u in U$ is uniquely expressible
    in the form $u=u_1 u_2$ with $u_1 in U^1$, $u_2 in U^2$._
  ]
  ...
  #block(breakable: false)[
    ...
    #smallcaps[Theorem 8.4.3.] _For each $w in W$ choose a coset representative
    $n_w in N$ which maps to $w$ under the natural homomorphism. Then each
    element_
    #source(129, "116")
  ...
  Since $n_w U_w^- n_w^(-1) subset.eq V$, this element is in $B ∩ V$, so is $1$
  by #book-ref("l", "7-1-2", [7.1.2]). Thus $b_1=b_2$ and $u_1=u_2$. #qed

  #source(130, "117")
  #block(breakable: false)[
    #metadata((kind: "anchor")) <cor:8-4-4>
    #smallcaps[Corollary 8.4.4.] _Each element of $G$ has a unique expression in
    the form $g=u_1 h n_w u$, where $u_1 in U$, $h in H$, $w in W$,
    $u in U_w^-$._
  ]
  ...
  #block(sticky: true)[
    By the uniqueness of expression of elements of $G$ in canonical form we have
  ]
  $
  ...
  #block(breakable: false)[
    ...
    #smallcaps[Corollary 8.4.7.] _Let $hat(G)$ be the group of automorphisms of
    $frak(L)_K$ generated by $G$ and $hat(H)$. Then $G$ is a normal subgroup of
    $hat(G)$ and $hat(G)\/G$ is isomorphic to $hat(H)\/H$._
  ]
...
#[
  ...
  == 8.5 The Levi Decomposition <sec:8-5>

  We shall now derive further information about the parabolic subgroups of the
  Chevalley group $G$. We have seen in sections #book-ref("sec", "8-2", [8.2])
  and #book-ref("sec", "8-3", [8.3]) that every parabolic subgroup of $G$ is
  conjugate to one of the subgroups $P_J=B N_J B$ for some subset $J$ of the set
  $Pi$ of fundamental roots. Furthermore distinct subsets of $Pi$ give rise to
  non-conjugate subgroups $P_J$. Thus $G$ has $2^l$ conjugacy classes of
  parabolic subgroups.
  ...
  #block(sticky: true)[
    #smallcaps[Proof.] $P_J=B N_J B$ is the subgroup of $G$ generated by $B$ and
    the elements $n_r$ for $r in J$. Since $n_r in lr(⟨X_r,X_(-r)⟩)$, it is
    clear that
  ]
  ...
  called the Levi decomposition, and $L_J$ and its conjugates in $U_J$ are
  ...
  positive integers, are also in $Phi^+ ∩ overline(Phi)_J$. Thus the commutator
  formula shows that $X_r$ normalizes $U_J$. Now suppose $r in Phi^- ∩ Phi_J$.
  Then $-r$ is not in $Phi^+ ∩ overline(Phi)_J$, and if $s$ is any root in
  $Phi^+ ∩ overline(Phi)_J$, all roots of form $i r+j s$, where $i,j$ are
  positive integers, are in $Phi^+ ∩ overline(Phi)_J$. For $i r+j s$ involves
  some fundamental root not in $J$ with a positive coefficient. Hence $X_r$
  normalizes $U_J$ in this case also, and so $U_J$ is normal in $P_J$ by
  #book-ref(
    "p",
    "8-5-1",
    [8.5.1],
  ...
  #block(sticky: true)[
    ...
    spanned by $J$, as in #book-ref("sec", "2-5", [section 2.5]). Then
  ]
  ...
  Finally consider the subgroup $frak(N)_G (U_J)$. This subgroup contains $P_J$,
  so must be one of the subgroups $P_(J_1)$ for some subset $J_1 supset.eq J$.
  If $J_1 supset J$, choose a root $r in J_1$ with $r in.not J$. Then
  $r in Phi^+ ∩ overline(Phi)_J$ and so $X_r subset.eq U_J$. Also
  $n_r in frak(N)_G (U_J)$. However $n_r X_r n_r^(-1)=X_(-r)$ is not contained
  in $U_J$ and we have a contradiction. Therefore $J_1=J$ and
  $frak(N)_G (U_J)=P_J$.
  #qed
]
...
#[
  ...
  == 8.6 The Finite Chevalley Groups <sec:8-6>
  ...
  We now consider the order of $H$. By #book-ref("th", "7-1-1", [7.1.1]) $H$ is
  the set of automorphisms $h(chi)$ of $frak(L)_K$ where $chi$ is a
  ...
  $chi$ runs through all #box[$K$-characters] of $P$. Since $P$ is isomorphic to
  $Q$ (both are free abelian groups of rank $l$), $hat(H)$ is isomorphic to the
  group of automorphisms $h(chi)$ for all #box[$K$-characters] $chi$ of $Q$.
  Thus there is an epimorphism $hat(H) arrow H$ obtained by restricting $chi$
  from $Q$ to $P$. The kernel consists of the automorphisms $h(chi)$, where
  $chi$ is a #box[$K$-character] of $Q$ which is the identity on $P$. The kernel
  is therefore isomorphic to the group of #box[$K$-characters] of the factor
  group $Q\/P$.
  ...
  #block()[
    ...
    it has been shown in #book-ref("sec", "7-1", [section 7.1]) that the bases
  ...
  #block(breakable: false)[
    ...
    #table(
      ...
      [$Q\/P$], [$ZZ_(l+1)$], [$ZZ_2$], [$ZZ_2$], [$ZZ_4$], [$ZZ_2 times ZZ_2$],
      table.hline(stroke: 0.3pt),
