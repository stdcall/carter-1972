...
#let chapter = [
  #[
    ...
    #lemma(<l:simple-root-normalizers>)[
    ...
    #proof[
      The commutator formula #book-ref("th", "chevalley-commutator-formula")
      ...
      combination of fundamental roots, at least one coefficient is positive.
      Thus each coefficient must be non-negative and we have a positive root.
      The root $r$ itself cannot occur in this form. Applying the commutator
      formula once more we have $[X_(-r),U_r] subset.eq U_r$. Thus $X_(-r)$
      normalizes $U_r$ also.
    ]

    #corollary(<cor:simple-root-unipotent-products>)[
      #block(breakable: false)[
    ...
    #block(breakable: false)[
      #corollary(<cor:monomial-simple-root-normalizer>)[
        $n_r$ normalizes $U_r$.
      ]
    ...
    #proposition(<p:rank-one-parabolic>)[
      #block(breakable: false)[
    ...
    #proof[
      ...
      #block(sticky: true)[
        Now $B=U H=X_r U_r H$, as in #book-ref(
          "cor",
          "simple-root-unipotent-products",
        ). Then we have
      ]
      $
      ...
      $
        x_(-r)(t) & =x_r (t^(-1))n_r (-t^(-1))x_r (t^(-1)), \
                  & quad "by" #book-ref("eq", "rank-one-weyl-element", suffix: [
                      (ii)]), \
                  & =x_r (t^(-1))h_r (-t^(-1))n_r x_r (t^(-1)), \
                  & quad "by" #book-ref("eq", "sl-two-diagonal", suffix: [
                      (iii)]).
    ...
    #proposition(<p:right-bruhat-multiplication>)[
      #block(breakable: false)[
    ...
    #proof[
      ...
      by #book-ref("cor", "monomial-simple-root-normalizer"). Hence
      ...
      $
        B n B dot B n_r B & =B n' n_r B dot B n_r B \
                          ...
                          & subset.eq B n' B dot (B union B n_r B),
                            quad "by" #book-ref("p", "rank-one-parabolic"), \
    ...
    #corollary(<cor:left-bruhat-multiplication>)[
      #block(breakable: false)[
    ...
    #proof[
      Let $g in B n_r B dot B n B$. Then $g^(-1) in B n^(-1)B dot B n_r B$.
      Hence, by #book-ref("p", "right-bruhat-multiplication"),
  ...
  #[
    #import "main-defs.typ": bib-ref, book-ref, chapter-ref, source
    ...
    #proposition(<p:chevalley-bn-pair>)[
      #block(breakable: false)[
    ...
    #proof[
      #block(sticky: true)[
        We show that the subgroups of $G$ previously denoted by $B,N$ satisfy
        the above axioms. By #book-ref("cor", "monomial-borel-intersections") we
        have $B ∩ N=H$, and by #book-ref("th", "monomial-quotient-weyl-group")
        we know that $H$ is normal in $N$ and that $N\/H$ is isomorphic to the
        Weyl group $W$. The Weyl group is generated by the fundamental
        reflections $w_r$, $r in Pi$, and we take these as the generating
        involutions in #book-ref(
          "def",
          "bn-3",
          [_BN_
            3],
        ). By #book-ref("cor", "left-bruhat-multiplication") we have, for each
        $r in Pi$,
      ]
      $ B n_r B dot B n B subset.eq B n_r n B union B n B. $
      ...
      by #book-ref("l", "monomial-conjugation-root-subgroups"). Thus $X_r$ is in
      the subgroup generated by $B$ and $N$, whence $lr(⟨B,N⟩)=G$. Therefore $G$
      has a $(B,N)$-pair.
    ]

    We now derive some consequences of the axioms for a $(B,N)$-pair.

    #proposition(<p:abstract-bruhat-and-parabolics>)[
      #block(breakable: false)[
        Let $G$ be a group with a $(B,N)$-pair. Then
          ...
          [$G=B N B$.],
          [
            For each subset $J$ of $I$, let $W_J$ be the subgroup of $W$
            generated by the elements $w_i$ for $i in J$, and $N_J$ be the
            subgroup of $N$ mapping to $W_J$ under the natural homomorphism.
    ...
    #proof[
      #block(sticky: true)[
        We prove (ii) first. $B N_J B$ is certainly closed under inversion, and
        so we must prove it closed under multiplication. Let $n in N_J$. Then
        $n$ may be written in the form $n=n_1 n_2 dots n_k$, where each $n_i$
        maps under the natural homomorphism to a generator $w_i$ of $W$ with
        $i in J$. Thus
      ]
      $
    ...
    It is evident from #book-ref("p", "abstract-bruhat-and-parabolics") that
    every double coset $B g B$ contains an element of $N$. We now consider the
    question of when two elements of $N$ lie in the same double coset.

    #proposition(<p:bruhat-double-coset-uniqueness>)[
      #block(breakable: false)[
        ...
        under the natural homomorphism from $N$ into $W$. Thus there is a
        natural 1–1 correspondence between double cosets of $B$ in $G$ and
        elements of $W$.
      ]
    ]
    ...
    #proof[
      Each element of $W$ is a product of generators $w_i$, $i in I$. We denote
      by $l(w)$ the shortest length of any expression for $w$ as a product of
      such generators. Suppose $B n B=B n' B$, where $n,n'$ correspond to $w,w'$
    ...
    #book-ref("p", "right-bruhat-multiplication").

    #proposition(<p:increasing-length-bruhat-product>)[
      #block(breakable: false)[
        Let $G$ be a group with a $(B,N)$-pair. Let $w_i$, $i in I$, be one of
        the distinguished generators of $W$ and $w$ be an element of $W$ such
        that $l(w_i w)>=l(w)$. Let $n_i,n$ be elements of $N$ mapped by the
        natural homomorphism into $w_i,w in W$. Then
    ...
    #proof[
      ...
      #block(sticky: true)[
        ...
        $B n_i B dot B n B$ intersects $B n B$, by #book-ref(
          "def",
          "bn-4",
          [_BN_
            4],
        ). Let $n',n_j$ be elements of $N$ mapped by the natural homomorphism to
        $w',w_j in W$. Then
      ]
      $
      ...
      #book-ref("p", "bruhat-double-coset-uniqueness"), $w_i w'=w w_j$ or
      $w_i w'=w$. The former equation gives $w_i w'=w'$, whence $w_i=1$, which
      contradicts #book-ref("def", "bn-5", [_BN_ 5]). The latter gives
      $w_i w=w'$, which implies $l(w_i w)<l(w)$, a contradiction.
    ]

    #proposition(<p:decreasing-length-bruhat-product>)[
      #block(breakable: false)[
        Suppose the notation is as in #book-ref(
          "p",
          "increasing-length-bruhat-product",
        ). Then, if $l(w_i w)<=l(w)$, $B n_i B dot B n B$ has non-empty
        intersection with $B n B$.
      ]
    ...
    #proof[
      #block(sticky: true)[
        By #book-ref("def", "bn-4", [_BN_ 4]) we have
      ]
      $
      ...
      #block(sticky: true)[
        ...
        #book-ref("p", "increasing-length-bruhat-product")
      ]
      $
    ...
    _Note._ It follows from #book-ref("p", "increasing-length-bruhat-product")
    and #book-ref("p", "decreasing-length-bruhat-product") that
    $l(w_i w)!=l(w)$.

    #corollary(<cor:decreasing-length-conjugate-product>)[
      #block(breakable: false)[
        Suppose the notation is as in #book-ref(
          "p",
          "increasing-length-bruhat-product",
        ). If $l(w_i w)<l(w)$ then $n_i in B n B n^(-1)B$.
      ]
    ]

    #proof[
      #block(sticky: true)[
        By #book-ref("p", "decreasing-length-bruhat-product") we have
      ]
      $
        n_i B n ∩ B n B != emptyset.
  ...
  #[
    ...
    $P_J$ of #book-ref("p", "abstract-bruhat-and-parabolics") is a parabolic
    subgroup. We shall show that the subgroups $P_J$ for the various subsets $J$
    of $I$ are the only subgroups of $G$ containing $B$, and therefore that
    every parabolic subgroup is conjugate to some $P_J$. Furthermore, the
    subgroups $P_J$ are all distinct and non-conjugate in $G$, and each of them
    is equal to its normalizer.

    We first prove a proposition describing the subgroup generated by $B$ and a
    single element of $N$.

    #proposition(<p:parabolic-generated-by-double-coset>)[
      #block()[
    ...
    #proof[
      ...
      #block(sticky: true)[
        ...
        #book-ref("cor", "decreasing-length-conjugate-product"),
        $n_(i_1) in lr(⟨B,n B n^(-1)⟩)$. Since
      ]
      $
    ...
    #theorem(<th:subgroups-containing-borel>)[
      #block(breakable: false)[
    ...
    #proof[
      #block(sticky: true)[
        ...
        Let $n_alpha$ be any element of $N$. Then, by #book-ref(
          "p",
    ...
    #theorem(<th:parabolic-normalizers-and-conjugacy>)[
      #block(breakable: false)[
        Let $G$ be a group with a $(B,N)$-pair. Then each subgroup $P_J$ of $G$
        is equal to its normalizer. Furthermore distinct subgroups $P_J,P_K$
        cannot be conjugate in $G$.
      ]
    ]
    ...
    #proof[
      ...
      by #book-ref("p", "parabolic-generated-by-double-coset"). Therefore
      $n in P_J$. It follows that $P_J=frak(N)(P_J)$.
      ...
      by #book-ref("p", "parabolic-generated-by-double-coset"). Therefore
      $n in P_K$ and so $P_K=P_J$. Thus distinct subgroups $P_J,P_K$ are
      non-conjugate.
    ]

    #theorem(<th:abstract-parabolic-lattice>)[
      #block(breakable: false)[
        Let $G$ be a group with a $(B,N)$-pair. Then the subgroups $P_J$ for
        distinct subsets $J$ of $I$ are all distinct. Furthermore we have
        $P_J ∩ P_K=P_(J ∩ K)$. Thus the subgroups $P_J$ form a lattice
        isomorphic to the lattice of subsets of $I$.
      ]
    ]

    #proof[
      #block(sticky: true)[
        It is necessary to show first that the elements $w_i$ of $W$ for
        $i in I$ form a minimal set of generators of $W$. Suppose we remove one
        element $j$ from $I$, and assume by way of contradiction that $W$ is
      ...
      generators. Let $n_j$ be an element of $N$ corresponding to $w_j in W$,
      and let $J={i_1,i_2,dots,i_k}$. Then, by #book-ref(
        "p",
        "parabolic-generated-by-double-coset",
      ), we have $lr(⟨B,n_j⟩)=B N_J B$. However, $lr(⟨B,n_j⟩)=B union B n_j B$
      by #book-ref("p", "parabolic-generated-by-double-coset") applied to the
      full set of generators of $W$. Thus $B N_J B=B union B n_j B$. By
      #book-ref("p", "bruhat-double-coset-uniqueness") this implies that
      $W_J={1,w_j}$, which gives a contradiction. Thus the elements $w_i$,
      ...
      #book-ref("th", "subgroups-containing-borel"). Now
      $P_(J ∩ K) subset.eq P_L$. We suppose by way of contradiction that
      $P_(J ∩ K)!=P_L$. Then certainly $L$ is not contained in $J ∩ K$ and we
      may assume without loss of generality that $L$ is not contained in $J$.
      However, $P_L subset.eq P_J$ and so $N_L subset.eq N_J$ by #book-ref(
        "p",
      ...
      $w_i$ is expressible in terms of the remainder, which gives a
      contradiction. Thus $P_J=P_K$ implies that $J=K$.
    ]
  ...
  #[
    #import "main-defs.typ": book-ref, chapter-ref, mathclap, source
    ...
    #lemma(<l:closed-root-partition-product>)[
      #block(breakable: false)[
        $U=U^1 U^2$, $U^1 ∩ U^2=1$.
      ]
    ]
    ...
    #proof[
      ...
      we have $t_i=0$ whenever $r_i in Psi_1$. Thus $t_i=0$ for all $i$ and
      $u=1$.
      ...
      #block()[
        for $m=1,2,dots$ and also define $U_m^1=U_m ∩ U^1$, $U_m^2=U_m ∩ U^2$.
        We show $U_m=U_m^1 U_m^2$ for all $m$. This is clear if $m$ is
        sufficiently large, and we prove it by descending induction.
      ]
      #block(sticky: true)[
    ...
    #corollary(<cor:closed-root-partition-uniqueness>)[
      #block(breakable: false)[
        Each element $u in U$ is uniquely expressible in the form $u=u_1 u_2$
        with $u_1 in U^1$, $u_2 in U^2$.
      ]
    ]
    ...
    #theorem(<th:canonical-bruhat-form>)[
      #block(breakable: false)[
    ...
    #proof[
      #block(sticky: true)[
        We shall show that $n_w U_w^+ n_w^(-1) subset.eq U$. By #book-ref(
          "l",
      ...
      #block(sticky: true)[
        for all $r,s in Phi$. Let $w=w_(r_1)w_(r_2) dots w_(r_k)$ be an
        expression of $w$ as a product of reflections. Then $n_w$ and
      ...
      #block(sticky: true)[
        by #book-ref("l", "monomial-conjugation-root-subgroups"). It follows
        that
      ]
      $
        n_w U_w^+ n_w^(-1)
      ...
      But clearly $B n_w U_w^- subset.eq B n_w B$, so we have equality. Since
      each element $g in G$ is in some double coset $B n_w B$, $g$ may be
      expressed in the form $g=b n_w u$, where $b in B$, $u in U_w^-$.
      ...
      #block(sticky: true)[
        ...
        #book-ref("p", "bruhat-double-coset-uniqueness") we have $w=w'$, thus
        $n_w=n_(w')$. It follows that
      ]
      $
      ...
      $1$ by #book-ref("l", "opposite-unipotent-intersection"). Thus $b_1=b_2$
      and $u_1=u_2$.
    ]

    #source(130, "117")
    #corollary(<cor:refined-bruhat-form>)[
      #block(breakable: false)[
    ...
    #proposition(<p:diagonal-conjugacy>)[
      #block(breakable: false)[
    ...
    #proof[
      #block(sticky: true)[
        ...
        #book-ref("cor", "refined-bruhat-form"). Then
      ]
    ...
    #proposition(<p:extended-diagonal-intersection>)[
      #block(breakable: false)[
        $G ∩ hat(H)=H$.
      ]
    ]
    ...
    #proof[
      ...
      #block(breakable: false)[
        ...
        since $U ∩ hat(H)=1$ by #book-ref(
          "l",
          "opposite-unipotent-intersection",
        ).
      ]
    ]

    #source(131, "118")
    #corollary(<cor:extended-chevalley-quotient>)[
      #block(breakable: false)[
        Let $hat(G)$ be the group of automorphisms of $frak(L)_K$ generated by
        $G$ and $hat(H)$. Then $G$ is a normal subgroup of $hat(G)$ and
        $hat(G)\/G$ is isomorphic to $hat(H)\/H$.
      ]
    ]

    #proof[
      #block(sticky: true)[
        $hat(H)$ normalizes $G$ by the remark following #book-ref(
          "th",
      ...
      by #book-ref("p", "extended-diagonal-intersection").
    ]
  ...
  #[
    ...
    #proposition(<p:parabolic-root-generators>)[
      #block(breakable: false)[
        $P_J$ is the subgroup of $G$ generated by $H$ and the root subgroups
        $X_r$ for $r in Phi^+ union Phi_J$.
      ]
    ]
    ...
    #proof[
      ...
      #block(sticky: true)[
        Conversely, it is clear that $H subset.eq P_J$ and $X_r subset.eq P_J$
        if $r in Phi^+$. Thus we consider the subgroups $X_r$ with
        $r in Phi^- ∩ Phi_J$. By #book-ref("p", "parabolic-root-system") each
        root in $Phi_J$ is the image of a root in $J$ under an element of $W_J$.
        Thus
      ]
      $
      ...
      by #book-ref("l", "monomial-conjugation-root-subgroups"). Therefore
      $X_r subset.eq P_J$ as required.
    ]
    ...
    #proof[
      We show that the subgroups generating $P_J$ all normalize $U_J$. It is
      clear that $H$ normalizes $U_J$. Let $r$ be a positive root. If
      $s in Phi^+ ∩ overline(Phi)_J$ all roots of form $i r+j s$, where $i,j$
      are positive integers, are also in $Phi^+ ∩ overline(Phi)_J$. Thus the
      commutator formula shows that $X_r$ normalizes $U_J$. Now suppose
      $r in Phi^- ∩ Phi_J$. Then $-r$ is not in $Phi^+ ∩ overline(Phi)_J$, and
      if $s$ is any root in $Phi^+ ∩ overline(Phi)_J$, all roots of form
      $i r+j s$, where $i,j$ are positive integers, are in
      $Phi^+ ∩ overline(Phi)_J$. For $i r+j s$ involves some fundamental root
      not in $J$ with a positive coefficient. Hence $X_r$ normalizes $U_J$ in
      this case also, and so $U_J$ is normal in $P_J$ by #book-ref(
        "p",
        "parabolic-root-generators",
      ).

      Now #book-ref("p", "parabolic-root-generators") shows that $P_J$ is
      generated by $U_J$ and $L_J$. Since $U_J$ is normal in $P_J$ we have
      $P_J=U_J L_J$.
  ...
  #[
    ...
    #block(sticky: true)[
      ...
      #book-ref("cor", "refined-bruhat-form"). Each element of $G$ has a unique
      expression in the form $u_1 h n_w u$, where $u_1 in U$, $h in H$,
      $w in W$, $u in U_w^-$. Thus we have
    ]
    $
    ...
    We now consider the order of $H$. By #book-ref(
      "th",
      "diagonal-subgroup-characters",
    ) $H$ is the set of automorphisms $h(chi)$ of $frak(L)_K$ where $chi$ is a
    ...
    #proposition(<p:finite-order-length-sum>)[
