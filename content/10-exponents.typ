#[
  ...
  #source(158, "145")
  = 10. The Exponents of the Weyl Group <ch:10>
  ...
  referring to #book-ref("cor", "10-2-4", [10.2.4]) and #book-ref(
    "p",
    "10-2-5",
    [10.2.5],
  ) for a knowledge of the numbers $d_1,dots,d_l$ in the individual groups. The
  equivalent definitions of these integers are nevertheless of considerable
  interest, and it is probable that their significance is not yet fully
  understood.

  == 10.1 A Theorem of Weyl <sec:10-1>

  We prove first a well known factorization theorem of Weyl. In order to do this
  we derive some properties of the vector
  ...
  $ <eq:10-1-3-duality>
  #block(sticky: true)[
    as in section #book-ref("sec", "7-1", [7.1]). Thus
  ]
  ...
  #block(sticky: true)[
    #smallcaps[Proof.] Let $Omega$ be the set of roots in $Phi^+$ which are not
    in $w(Phi^+)$. Then
  ]
  $
  ...
  #block(width: 100%, breakable: false)[
    ...
    _is either in one of the reflecting hyperplanes or is a transform $w(S)$ of
    $S$ by some element of the Weyl group._
  ]
  ...
  #source(161, "148")
  for some other subset $Omega_1$ of $Phi^+$. (This follows from the definition
  of $S$ and the fact that $w$ permutes the roots.) Thus by #book-ref(
    "l",
    "10-1-5",
    [10.1.5],
  ...
  of the fundamental weights. $Q$ is an additive abelian group. It will be more
  convenient in the following discussion to regard $Q$ as a multiplicative group
  instead, and so we define $e(Q)$ to be a multiplicative group isomorphic to
  $Q$. The elements of $e(Q)$ have form
  ...
  $W$-module. We consider the alternating elements of $A$, viz., those which
  satisfy
  ...
  #block(sticky: true)[
    ...
    #smallcaps[Lemma 10.1.7.] _The image of $A$ under $theta$ is the set of all
    alternating elements of $A$._
  ]
  ...
  It follows that $w(a)=det w dot a$ for all $w in W$ and so $a$ is alternating.
...
#[
  ...
  == 10.2 A Theorem of Macdonald <sec:10-2>
  ...
  does not lie in any reflecting hyperplane, it has the form $w'(S)$, where $w'$
  is an element of $W$ with $l(w')=abs(Omega)$ (see #book-ref(
    "l",
    "10-1-6",
    [10.1.6],
  ...
  is the field of fractions of the rational group algebra of $e(Q)$, and a
  factorization of this polynomial has been obtained in $F[t]$. However, one can
  make use of this factorization in $F[t]$ to obtain a factorization of
  ...
  #smallcaps[Proof.] Let $P$ be the additive group generated by the fundamental
  roots $p_1,dots,p_l$ and $e(P)$ be the corresponding multiplicative group. Let
  ...
  $Q$ (see section #book-ref("sec", "7-1", [7.1])), $B$ is a subalgebra of $A$.
  The above homomorphism from $e(P)$ into $chevron(t)$ extends to an algebra
  homomorphism from $B$ into $QQ[t,1 slash t]$, the set of rational combinations
  of the powers of $t$. ($QQ[t,1 slash t]$ is the rational group algebra of the
  infinite cyclic group $chevron(t)$.) This algebra homomorphism may itself be
  extended to an algebra homomorphism from the polynomial ring $B[t]$ into
  $QQ[t,1 slash t]$ under which $t$ is mapped into $t$. For since $t$ is an
  indeterminate over $B$, its image may be chosen arbitrarily in
  $QQ[t,1 slash t]$ and an algebra homomorphism is then uniquely defined. We
  denote this homomorphism by $psi$. Then $psi:B[t] arrow QQ[t,1 slash t]$
  satisfies
  $
  ...
  However if $w!=1$ there is some $r in Phi^+$ such that $h(w(r))=-1$. Thus the
  contributions to the left-hand side from all non-identity elements of $W$ are
  zero. Hence
  $
  ...
  section #book-ref("sec", "3-6", [3.6])) shows that
  ...
  #book-ref(
    ...
    "3-6",
    [3.6],
  ) and form the dual partition. #qed

  It will be observed that the integers $m_1,dots,m_l$ determined in #book-ref(
    "p",
    "10-2-5",
    [10.2.5],
  ...
  also explain another property of the integers $m_i$ observable from #book-ref(
    "p",
    "10-2-5",
    [10.2.5],
...
#[
  ...
  == 10.3 The Class of Coxeter Elements <sec:10-3>
  ...
  #block(sticky: true)[
    #smallcaps[Proof.] It is clear that any conjugate of a Coxeter element is a
    Coxeter element. For
  ]
  $
  ...
  Coxeter elements corresponding to this. $w_(p_1) w_(p_2) dots w_(p_l)$ is one
  such element and the others are obtained by changing the order of the factors.
  The fact that they are all conjugate follows from the following lemma.

  #source(170, "157")
  #block(sticky: true)[
    #metadata((kind: "anchor")) <l:10-3-2>
    #smallcaps[Lemma 10.3.2.] _Suppose the fundamental roots $p_1,dots,p_l$ are
    written round a circle, as shown in #figure-ref("3"). Then any permutation
    of $p_1,dots,p_l$ may be obtained by performing a succession of interchanges
    $(p_i p_j)$, where $p_i,p_j$ are not joined in the Dynkin diagram and are
    adjacent on the circle, and then reading clockwise around the circle
    beginning from a suitable point._
  ]
  ...
  $l>=3$. Now the Dynkin diagram contains some node joined to at most one other
  node. Let $p_l$ correspond to such a node. On omitting $p_l$ from the circle
  we may by induction obtain any permutation of $p_1,dots,p_(l-1)$ by
  interchanging adjacent pairs not joined in the Dynkin diagram and then reading
  round from a suitable point. We show that all these interchanges can be
  carried out even when $p_l$ is present. It is sufficient to show that
  ...
  interchanging $p_i,p_l$ and then $p_i,p_j$ we have succeeded in interchanging
  the order of $p_i,p_j$ when $p_l$ is removed. Thus we may make the required
  arrangement of all the roots other than $p_l$. But then $p_l$, since it is
  joined
  #source(171, "158")
  ...
  any starting point on the circle, a Coxeter element $w_(r_1) dots w_(r_l)$ is
  determined. $r_1$ is the starting point, and $r_1,r_2,dots,r_l$ appear
  ...
  element is unchanged since the corresponding reflections commute. (If we have
  interchanged the first and last terms, a conjugate Coxeter element is
...
#[
  ...
  == 10.4 A Dihedral Subgroup of the Weyl Group <sec:10-4>

  We shall obtain further information about the Coxeter elements by showing that
  each Coxeter element can be embedded in a dihedral subgroup of $W$ which
  operates faithfully on a certain 2-dimensional subspace of $frak(V)$.

  #block(sticky: true)[
    #metadata((kind: "anchor")) <l:10-4-1>
    #smallcaps[Lemma 10.4.1.] _The set of fundamental roots $p_1,dots,p_l$ may
    be divided into two disjoint subsets, each of which contains roots which are
    all orthogonal to one another._
  ]

  #smallcaps[Proof.] Remove a node from the Dynkin diagram which is joined to at
  most one other node, and use induction on the number of nodes. #qed

  #block(sticky: true)[
    We suppose that the fundamental system $Pi={p_1,dots,p_l}$ is decomposed as
    in #book-ref("l", "10-4-1", [10.4.1]) into two subsets
  ]
  ...
  #block(sticky: true)[
    ...
    $I-M$ and $u$ a corresponding eigenvector. Let $a,b$ be the vectors defined
    above and $Gamma$ be the circle with centre the origin passing through $a$
    and $b$ (see #figure-ref("5")). Then the group
    ...
    reflection in the line $O a$ and $w$ is a rotation around $Gamma$ through an
    angle $2theta$, where $cos theta=lambda$._
  ]
  ...
  $i=1,dots,k$. Thus $lambda a-b$ is a linear combination of $f_(k+1),dots,f_l$,
  so also a linear combination of $p_(k+1),dots,p_l$. Hence
  $w_2 (lambda a-b)=-(lambda a-b)$. However, $w_2 (a)=a$ and, since
  ...
  $lr(chevron.l w_1, w_2 chevron.r)$ therefore operates on $Gamma$ in an obvious
  way.
  #qed
]
...
#[
  ...
  == 10.5 Eigenvalues of the Coxeter Elements <sec:10-5>

  Before applying the results of section #book-ref("sec", "10-4", [10.4]) to
  give further information about the Coxeter elements, we state and prove a
  classical result on real
  #source(175, "162")
  symmetric matrices which we shall need, known as the Frobenius–Perron theorem.
  ...
  #smallcaps[Proof.] The eigenvalues of $M$ are real since $M$ is symmetric, and
  non-negative since $M$ is positive semi-definite. There is an orthogonal
  matrix $T$ such that $T M T'$ is a diagonal matrix whose coefficients are the
  ...
  that $x M=0$. $frak(V)_0$ is also the set of $x in RR^l$ such that $x M x'=0$,
  as is easily seen by considering the diagonal matrix $T M T'$. We shall show
  that $dim frak(V)_0<=1$. Suppose $dim frak(V)_0>0$ and let
  ...
  If $i in I$ we have $abs(alpha_i)!=0$ and hence $m_(i j)=0$. Thus $m_(i j)=0$
  for all $i in I,j in J$. However, $M$ is indecomposable and $I$ is non-empty,
  therefore $J$ must be empty. Thus each coefficient of $x$ is non-zero. Since
  this holds for each non-zero vector in $frak(V)_0$ we must have
  $dim frak(V)_0<=1$. Furthermore if $dim frak(V)_0=1$ then $frak(V)_0$ contains
  a vector whose coefficients are all positive.
  ...
  diagonal
  #source(176, "163")
  matrix $T M T'$. $M-mu I$ is singular, so its null-space has dimension $1$ and
  contains a vector whose coefficients are all positive. Thus $mu$ occurs as
  eigenvalue of $M$ with multiplicity $1$, and has an eigenvector whose
  coefficients are all positive. #qed

  We apply the Frobenius–Perron theorem to the situation discussed in #book-ref(
    "p",
    "10-4-3",
    [10.4.3],
  ). Let $M=(m_(i j))$ be the matrix defined by $m_(i j)=(f_i,f_j)$. Then $M$ is
  positive definite, since the Killing form is positive definite (section
  #book-ref("sec", "3-3", [3.3])). Moreover $m_(i j)<=0$ when $i!=j$ by
  #book-ref(
    "cor",
    "2-1-4",
    [2.1.4],
  ...
  #block(sticky: true)[
    ...
    #smallcaps[Corollary 10.5.2.] _If the vectors $a,b$ are chosen as in section
    #book-ref("sec", "10-4", [10.4]) with respect to the largest eigenvalue of
    $I-M$, then each point of the circle $Gamma$ lying strictly between $a$ and
    $b$ is in the fundamental chamber._
  ]
  ...
  Let $Gamma$ be the circle defined in section #book-ref("sec", "10-4", [10.4])
  ...
  #source(177, "164")
  #figure(
    [#block[#coxeter-plane(reflected: true) #align(center)[Figure 6]] <fig:6>],
    caption: none,
    placement: top,
  )
  ...
  $overline(C)$, the closure of the fundamental chamber (#book-ref(
    ...
    "10-5-2",
    [10.5.2],
  )). Thus $w^i (a)=a$ by #book-ref("cor", "2-3-5", [2.3.5]). On the other hand,
  if $w^i (a)$ lies between $a$ and $c$ it is evident that $w^i (b)$ lies
  between $a$ and $b$. Thus both $b$ and $w^i (b)$ lie in $overline(C)$, and so
  $w^i (b)=b$. In either case we have $w^i=1$ since $lr(chevron.l w chevron.r)$
  operates faithfully on $Gamma$. Hence $i=h$ and $theta=pi slash h$.

  #block(sticky: true)[Now]
  $ a=sum_(i=1)^k xi_i hat(f)_i $ <eq:10-5-3-a>
  and so $a$ is orthogonal to the roots $p_(k+1),dots,p_l$ and their negatives.
  ...
  #source(178, "165")
  is a fundamental system. Since $p_(k+1),dots,p_l$ are mutually orthogonal, the
  only such roots are $plus.minus p_i$. Thus $a$ lies in exactly $l-k$
  ...
  $w^i (b)$ lie in exactly $k$ reflecting hyperplanes. However, each reflecting
  hyperplane intersects the plane of $Gamma$ in a line meeting $Gamma$ at one of
  the transforms $w^i (a)$ or $w^i (b)$. For otherwise there would be a
  reflecting hyperplane intersecting $Gamma$ at a point strictly between $a$ and
  $b$, contrary to the fact that such points lie in the fundamental chamber. Now
  there are $h$ transforms of $a$ and $h$ transforms of $b$ alternating round
  $Gamma$. Thus the total number of reflecting hyperplanes is
  ...
  $2pi slash h$, $w$ has an eigenvalue $upright(e)^(2pi i slash h)$ in the plane
  of $Gamma$. #qed

  #block(sticky: true)[
    #metadata((kind: "anchor")) <cor:10-5-5>
    #smallcaps[Corollary 10.5.5.] _A Coxeter element $w$ has an eigenvector $v$
    with eigenvalue $upright(e)^(2pi i slash h)$ such that $v$ is not orthogonal
    to any root._
  ]

  #smallcaps[Proof.] Let $v$ be an eigenvector with eigenvalue
  $upright(e)^(2pi i slash h)$ lying in the plane of $Gamma$. Since $w$ operates
  as a rotation in this plane $v$ is not real, so $v!=overline(v)$. Suppose
  $(v,r)=0$, where $r in Phi$. Then $(overline(v),r)=0$ since $r$ is real, and
  it follows that $r$ is orthogonal to every vector in the plane of $Gamma$.
  Since this plane contains points in the fundamental chamber we have a
  contradiction. Thus $v$ is not orthogonal to any root. #qed
  ...
  #block(sticky: true)[
    #smallcaps[Proof.] This is much easier to prove than the preceding results.
  ...
  #source(179, "166")
  Now $w_(p_2) dots w_(p_l) (v)-v$ is a linear combination of $p_2,dots,p_l$ and
  $w_(p_1) (v)-v$ is a scalar multiple of $p_1$. Since $p_1,p_2,dots,p_l$ are
...
#[
  ...
  == 10.6 A Theorem of Coleman <sec:10-6>
  ...
  #smallcaps[Proof.] The eigenvalues of a Coxeter element $w$ are $h$th roots of
  unity, where $h$ is the order of $w$, so are powers of $zeta$. By #book-ref(
    "p",
    "10-5-6",
    [10.5.6],
  ) $zeta^0$ is not an eigenvalue, so the eigenvalues are of form $zeta^(m_i)$,
  where $0<m_i<h$. Let $f_1,dots,f_l$ be a basis for the complexification
  $frak(V)_CC$ of $frak(V)$ such that
  ...
  where $e_1,dots,e_l$ is an orthonormal basis of $frak(V)$. Then $y_1,dots,y_l$
  are linear functions in $x_1,dots,x_l$ and vice versa. Now we may assume that
  $m_1=1$ by #book-ref("cor", "10-5-4", [10.5.4]) and that $f_1$ is not
  orthogonal to any root by #book-ref("cor", "10-5-5", [10.5.5]).
  #block(sticky: true)[Let]
  $
  ...
  expressed in terms of $y_1,dots,y_l$. By #book-ref("th", "9-3-5", [9.3.5]) $J$
  factorizes into a product
  #source(180, "167")
  of linear factors representing the reflecting hyperplanes. Since $f_1$ is not
  in any reflecting hyperplane, $J!=0$ at $(y_1,0,dots,0)$. Thus we may choose
  the numbering of the invariants $I_1,dots,I_l$ so that
  ...
  #block(sticky: true)[
    Since for any polynomial $P(y)$ we have $(w(P))(y)=P(w^(-1) (y))$ it follows
    that
  ]
  ...
  Now $zeta^(h-m_i)$ occurs as an eigenvalue of $w$ whenever $zeta^(m_i)$ does,
  since $w$ is a real transformation. We now renumber the basis vectors
  ...
  Now the numbers $h-m_i$, $i=1,dots,l$, are a permutation of the numbers $m_i$,
  $i=1,dots,l$. (We have again used #book-ref("p", "10-5-6", [10.5.6]) here.)
  Thus
  ...
  #block(sticky: true)[
    ...
    #smallcaps[Corollary 10.6.2.] _Let $k_i$ be the number of positive roots of
    height $i$ and let $(m_1,m_2,dots,m_l)$ be the partition of $N$ dual to the
    partition $(k_1,k_2,dots)$. Then the eigenvalues of a Coxeter element are_
  ]
  $ zeta^(m_1),zeta^(m_2),dots,zeta^(m_l), $ <eq:10-6-2-spectrum>
  _where $zeta=upright(e)^(2pi i slash h)$._

  #smallcaps[Proof.] This follows from #book-ref("cor", "10-2-4", [10.2.4]) and
  #book-ref("th", "10-6-1", [10.6.1]). #qed
  ...
  also be defined in terms of the topology of the corresponding Lie group, i.e.
  in terms of the Betti numbers of the compact Lie group with Weyl group $W$.
  The Betti numbers are the coefficients of the Poincaré polynomial of this Lie
  group, which factorizes as
  ...
  G. Lusztig has pointed out that Macdonald’s identity #book-ref(
    ...
    "10-2-1",
    [10.2.1],
  ) can be derived analytically by using a Lefschetz fixed point formula due to
  Atiyah and Bott.
]
