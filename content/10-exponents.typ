...
#let chapter = [
  #[
    ...
    reading, referring to #book-ref("cor", "degrees-and-exponents") and
    #book-ref("p", "exponent-table") for a knowledge of the numbers
    $d_1,dots,d_l$ in the individual groups. The equivalent definitions of these
    integers are nevertheless of considerable interest, and it is probable that
    their significance is not yet fully understood.
    ...
    $ S=1/2 sum_(r in Phi^+) r. $ <eq:weyl-exponent-theorem-half-sum>

    #lemma(<l:simple-reflection-weyl-vector>)[
      #block(sticky: true)[
    ...
    #lemma(<l:weyl-vector-in-chamber>)[
      #block(sticky: true)[
    ...
    #proof[
      #block(sticky: true)[
        It follows from #book-ref("l", "simple-reflection-weyl-vector") that
      ]
      $ frac(2(p_i,S), lr((p_i,p_i)))=1 $ <eq:weyl-vector-in-chamber-pairing>
      for all $p_i in Pi$. Thus $(p_i,S)>0$ for all $p_i$.
    ]

    #lemma(<l:weyl-vector-fundamental-weights>)[
      #block(sticky: true)[
        Let $q_1,dots,q_l$ be the fundamental weights. Then $S=q_1+dots+q_l$.
      ]
    ]
    ...
    #lemma(<l:weyl-vector-inversion-sum>)[
    ...
    #lemma(<l:dominant-root-subset-sum>)[
      #block(width: 100%, breakable: false)[
    ...
    #lemma(<l:root-subset-sum-orbit>)[
      #block(width: 100%, breakable: false)[
        Let $Omega$ be a subset of $Phi^+$. Then
        $ S-sum_(r in Omega) r $ <eq:root-subset-sum-orbit-weight>
        is either in one of the reflecting hyperplanes or is a transform $w(S)$
        of $S$ by some element of the Weyl group.
      ]
    ]
    ...
    #proof[
      ...
      #block(sticky: true)[
        lies in some chamber. By #book-ref(
          "cor",
      ...
      #book-ref("l", "dominant-root-subset-sum") $Omega_1$ is empty and so
      #block(width: 100%, breakable: false)[
    ...
    #lemma(<l:alternating-group-algebra>)[
      #block(sticky: true)[
    ...
    #theorem(<th:weyl-denominator-formula>)[
      #block(width: 100%, breakable: false)[
        $
    ...
    #proof[
      ...
      #source(163, "150")
      #block(sticky: true)[
        We show that $a$ is an alternating element of $A$. For each $p_i in Pi$
        we have
      ]
      $
      ...
      which are equal and opposite. Thus we need only consider subsets $Omega$
      of $Phi^+$ such that
      ...
      some $w' in W$ by #book-ref("l", "root-subset-sum-orbit"). In such a case
      we have $abs(Omega)=l(w')$ by #book-ref("l", "weyl-vector-inversion-sum").
  ...
  #[
    ...
    #proof[
      ...
      #block(sticky: true)[
        As in the proof of #book-ref("th", "weyl-denominator-formula") we have
      ]
      $
        theta lr((e lr((S-sum_(r in Omega) r))))=0
      ...
      $w'$ is an element of $W$ with $l(w')=abs(Omega)$ (see #book-ref(
        "l",
        "root-subset-sum-orbit",
      ) and #book-ref("l", "weyl-vector-inversion-sum")). #block(
        sticky: true,
      ...
      $a=theta(e(S))$ by #book-ref("th", "weyl-denominator-formula") and so the
      result follows.
    ]
    ...
    #theorem(<th:poincare-root-height-product>)[
      #block(width: 100%, breakable: false)[
        $
          sum_(w in W) t^(l(w))=product_(r in Phi^+)
          (t^(h(r)+1)-1)/(t^(h(r))-1),
        $ <eq:poincare-root-height-product-height-product>
        where $h(r)$ is the height of the root $r$.
      ]
    ]

    #proof[
      Let $P$ be the additive group generated by the fundamental roots
      $p_1,dots,p_l$ and $e(P)$ be the corresponding multiplicative group. Let
      $h$ be the homomorphism from $P$ into $ZZ$ taking value 1 at each
      #source(167, "154")
      ...
      of $Q$ (see section #book-ref("sec", "diagonal-subgroup")), $B$ is a
      subalgebra of $A$. The above homomorphism from $e(P)$ into $chevron(t)$
      extends to an algebra homomorphism from $B$ into $QQ[t,1 slash t]$, the
      set of rational combinations of the powers of $t$. ($QQ[t,1 slash t]$ is
      the rational group algebra of the infinite cyclic group $chevron(t)$.)
      This algebra homomorphism may itself be extended to an algebra
      homomorphism from the polynomial ring $B[t]$ into $QQ[t,1 slash t]$ under
      which $t$ is mapped into $t$. For since $t$ is an indeterminate over $B$,
      its image may be chosen arbitrarily in $QQ[t,1 slash t]$ and an algebra
      homomorphism is then uniquely defined. We denote this homomorphism by
      $psi$. Then $psi:B[t] arrow QQ[t,1 slash t]$ satisfies
      $
      ...
      denominators to obtain an identity in $A[t]$, and then observe that all
      the elements of $Q$ appearing in this identity are in $P$. We now apply
      the algebra homomorphism $psi$ to both sides, and obtain an identity
      $
        sum_(w in W) lr(
          (product_(r in Phi^+)
    ...
    #theorem(<th:poincare-exponent-product>)[
      #block(width: 100%, breakable: false)[
        $
    ...
    #proof[
      ...
      $t^(m_l+1)-1$ in the numerator and $(t-1)^l$ in the denominator.
    ]

    #corollary(<cor:degrees-and-exponents>)[
      #block(sticky: true)[
    ...
    #proof[
      This follows from #book-ref("th", "poincare-polynomial-factorization") and
      #book-ref("th", "poincare-exponent-product").
    ]

    It is easy to determine the integers $m_1,dots,m_l$ in the individual cases
    by inspecting the root systems.

    #proposition(<p:exponent-table>)[
      #block(sticky: true)[
    ...
    #proof[
      Calculate the numbers $k_1,k_2,dots$ from section #book-ref(
        "sec",
        "description-of-simple-lie-algebras",
      ) and form the dual partition.
    ]

    It will be observed that the integers $m_1,dots,m_l$ determined in
    #book-ref("p", "exponent-table") satisfy a condition of duality. If the
    $m_i$ are arranged in increasing order we have
    ...
    #book-ref("p", "exponent-table"), the fact that they exhibit a definite
    tendency to be prime.
  ]

  #[
    #import "main-defs.typ": book-ref, figure-ref, source
    ...
    #theorem(<th:coxeter-elements-conjugate>)[
      #block(sticky: true)[
    ...
    #proof[
      #block(sticky: true)[
        It is clear that any conjugate of a Coxeter element is a Coxeter
        element. For
      ]
      $
        w w_(r_1) dots w_(r_l) w^(-1)=w_(w(r_1)) dots w_(w(r_l))
      $ <eq:coxeter-elements-conjugate-conjugation>
      and if $r_1,dots,r_l$ is a fundamental system so is $w(r_1),dots,w(r_l)$.
      In showing that any two Coxeter elements are conjugate it is sufficient to
      consider Coxeter elements corresponding to a fixed fundamental system,
      since any two fundamental systems are equivalent under the Weyl group
      (#book-ref("th", "fundamental-systems-conjugate")). We take the
      fundamental system $Pi={p_1,dots,p_l}$ and consider the Coxeter elements
      corresponding to this. $w_(p_1) w_(p_2) dots w_(p_l)$ is one such element
      and the others are obtained by changing the order of the factors. The fact
      that they are all conjugate follows from the following lemma.

      #source(170, "157")
      #lemma(<l:circular-order-interchanges>)[
        #block(sticky: true)[
          Suppose the fundamental roots $p_1,dots,p_l$ are written round a
          circle, as shown in #figure-ref("3"). Then any permutation of
          $p_1,dots,p_l$ may be obtained by performing a succession of
          interchanges $(p_i p_j)$, where $p_i,p_j$ are not joined in the Dynkin
          diagram and are adjacent on the circle, and then reading clockwise
          around the circle beginning from a suitable point.
        ]
      ]
      ...
      #proof[
        ...
        interchanges have to be carried out at all. Thus we assume $l>=3$. Now
        the Dynkin diagram contains some node joined to at most one other node.
        Let $p_l$ correspond to such a node. On omitting $p_l$ from the circle
        we may by induction obtain any permutation of $p_1,dots,p_(l-1)$ by
        interchanging adjacent pairs not joined in the Dynkin diagram and then
        reading round from a suitable point. We show that all these interchanges
        can be carried out even when $p_l$ is present. It is sufficient to show
        that $p_i,p_j$ can be interchanged if $p_i,p_j$ are not joined in the
        Dynkin diagram and if $p_l$ is adjacent to both. (See #figure-ref("4").)
        ...
        circle by a succession of steps in either a clockwise or an
        anticlockwise direction into its required position.
      ]

      Now given any arrangement of the roots $p_1,dots,p_l$ around the circle
      and any starting point on the circle, a Coxeter element
      $w_(r_1) dots w_(r_l)$ is determined. $r_1$ is the starting point, and
      $r_1,r_2,dots,r_l$ appear clockwise around the circle starting from $r_1$.
      If we take the same arrangement around the circle, but a different
      starting point, we obtain a conjugate Coxeter element. If we change the
      arrangement around the circle by interchanging adjacent roots not linked
      in the Dynkin diagram, the Coxeter element is unchanged since the
      corresponding reflections commute. (If we have interchanged the first and
      last terms, a conjugate Coxeter element is obtained.) Thus #book-ref(
        "l",
        "circular-order-interchanges",
      ) shows that any Coxeter element of the form $w_(r_1) dots w_(r_l)$ can be
      obtained from $w_(p_1) dots w_(p_l)$ by a succession of operations which
      either leave the element unchanged or give a conjugate element. This
      completes the proof of #book-ref("th", "coxeter-elements-conjugate").
    ]
  ...
  #[
    #import "main-defs.typ": book-ref, figure-ref, mathclap, source
    ...
    #lemma(<l:orthogonal-bipartition>)[
      #block(sticky: true)[
        The set of fundamental roots $p_1,dots,p_l$ may be divided into two
        disjoint subsets, each of which contains roots which are all orthogonal
        to one another.
      ]
    ]
    ...
    #block(sticky: true)[
      ...
      as in #book-ref("l", "orthogonal-bipartition") into two subsets
    ]
    ...
    non-positive (#book-ref("cor", "fundamental-root-inner-products")). Thus
    $I-M$ is a symmetric matrix whose coefficients are all non-negative. This
    matrix plays a useful role in deriving the properties of the Coxeter
    elements.
    ...
    #proof[
      ...
      #block(sticky: true)[
        where $A$ is a certain $k times (l-k)$ matrix and $A'$ is the transpose
        of $A$. $M^(-1)$, which is also symmetric, may be written in the form
      ]
      ...
      #block(sticky: true)[
        The equations (#book-ref("eq", "coxeter-plane-angle-star", [$*$])) now
        show
      ]
      $
    ...
    #proposition(<p:dihedral-circle-action>)[
      #block(sticky: true)[
        ...
        #figure-ref("5")). Then the group $lr(chevron.l w_1, w_2 chevron.r)$
        operates on the points of $Gamma$. In particular $w_1$ is the reflection
        in the line $O b$, $w_2$ is the reflection in the line $O a$ and $w$ is
        a rotation around $Gamma$ through an angle $2theta$, where
        $cos theta=lambda$.
      ]
    ]
    ...
    #proof[
      It was shown in #book-ref("l", "coxeter-plane-angle") that
      ...
      $cos theta=lambda$, $lambda a$ is the projection of $b$ on $O a$. Thus
      $w_2$ leaves $Gamma$ invariant and operates on it as the reflection in the
      line $O a$.

      Similarly it was shown that $C' u^1-lambda D u^2=0$ and this implies that
      $(hat(f)_i,a-lambda b)=0$ for $i=k+1,dots,l$. Thus $a-lambda b$ is a
      linear combination of $p_1,dots,p_k$ and so
      $w_1 (a-lambda b)=-(a-lambda b)$. Also $w_1 (b)=b$ and $lambda b$ is the
      projection of $a$ on $O b$. Thus $w_1$ leaves $Gamma$ invariant and
      operates on it as the reflection in the line $O b$.

      Finally, since $O a$ and $O b$ are inclined at an angle $theta$,
      $w=w_1 w_2$ is a rotation through $2theta$. The whole dihedral group
  ...
  #[
    ...
    #proposition(<p:smallest-cartan-eigenvalue>)[
      #block(sticky: true)[
        Let $M$ be a real symmetric matrix such that $m_(i j)<=0$ for all
        $i!=j$, and suppose that $M$ is positive semi-definite and
        indecomposable. Then the eigenvalues of $M$ are all real and
        non-negative. The smallest eigenvalue has multiplicity $1$ and has an
        eigenvector whose coefficients are all positive.
      ]
    ]

    #proof[
      The eigenvalues of $M$ are real since $M$ is symmetric, and non-negative
      since $M$ is positive semi-definite. There is an orthogonal matrix $T$
      such that $T M T'$ is a diagonal matrix whose coefficients are the
      eigenvalues of $M$, by a well-known theorem of linear algebra.
      ...
      $x=(alpha_1,alpha_2,dots,alpha_l)$ be a non-zero vector in $frak(V)_0$.
      Let $y=(abs(alpha_1),abs(alpha_2),dots,abs(alpha_l))$. Then we have
      ...
      $alpha_i=0$. Suppose $j in J$. Then all the terms $abs(alpha_i) m_(i j)$
      in the above sum are non-positive, and so $abs(alpha_i) m_(i j)=0$ for all
      $i$. If $i in I$ we have $abs(alpha_i)!=0$ and hence $m_(i j)=0$. Thus
      $m_(i j)=0$ for all $i in I,j in J$. However, $M$ is indecomposable and
      $I$ is non-empty, therefore $J$ must be empty. Thus each coefficient of
      $x$ is non-zero. Since this holds for each non-zero vector in $frak(V)_0$
      we must have $dim frak(V)_0<=1$. Furthermore if $dim frak(V)_0=1$ then
      $frak(V)_0$ contains a vector whose coefficients are all positive.
      ...
      and contains a vector whose coefficients are all positive. Thus $mu$
      occurs as eigenvalue of $M$ with multiplicity $1$, and has an eigenvector
      whose coefficients are all positive.
    ]

    We apply the Frobenius–Perron theorem to the situation discussed in
    #book-ref("p", "dihedral-circle-action"). Let $M=(m_(i j))$ be the matrix
    defined by $m_(i j)=(f_i,f_j)$. Then $M$ is positive definite, since the
    Killing form is positive definite (section #book-ref(
      "sec",
      "roots-of-simple-lie-algebras",
    )). Moreover $m_(i j)<=0$ when $i!=j$ by #book-ref(
      "cor",
      "fundamental-root-inner-products",
    ).

    #corollary(<cor:coxeter-circle-chamber-arc>)[
      #block(sticky: true)[
        If the vectors $a,b$ are chosen as in section #book-ref(
          "sec",
          "dihedral-subgroup",
        ) with respect to the largest eigenvalue of $I-M$, then each point of
        the circle $Gamma$ lying strictly between $a$ and $b$ is in the
        fundamental chamber.
      ]
    ]

    #proof[
      The eigenvector $(xi_1,dots,xi_l)$ corresponding to the smallest
      eigenvalue of $M$ corresponds to the largest eigenvalue of $I-M$. Thus we
      may assume each $xi_i>0$.
      ...
      #block(sticky: true)[
        thus every point $c$ on $Gamma$ strictly between $a$ and $b$ has the
        form
      ]
      $
    ...
    #theorem(<th:coxeter-number-root-count>)[
      #block(sticky: true)[
    ...
    #proof[
      Let $w$ be a Coxeter element and $h$ be the order of $w$. Let $Gamma$ be
      the circle defined in section #book-ref(
        "sec",
        "dihedral-subgroup",
      ...
      operates trivially on $Gamma$ it fixes some point in the fundamental
      chamber by #book-ref("cor", "coxeter-circle-chamber-arc"), so is the
      identity by #book-ref("cor", "simply-transitive-chamber-action"). Thus $w$
      has order $h$ on $Gamma$.
      ...
      Let $c$ be the point on $Gamma$ which is the reflection of $b$ in $O a$
      (see #figure-ref("6")). Then the angle between $O b$ and $O c$ is
      $2theta$. Since $w$ is a rotation through $2theta$, there exists an
      integer $i>0$ such that $w^i (a)$ lies on the arc $b c$. Let $i$ be the
      least such positive integer. If $w^i (a)$ lies between $a$ and $b$, both
      $a$ and $w^i (a)$ lie in $overline(C)$, the closure of the fundamental
      chamber (#book-ref("cor", "coxeter-circle-chamber-arc")). Thus $w^i (a)=a$
      by #book-ref("cor", "closed-chamber-fundamental-region"). On the other
      hand, if $w^i (a)$ lies between $a$ and $c$ it is evident that $w^i (b)$
      lies between $a$ and $b$. Thus both $b$ and $w^i (b)$ lie in
      $overline(C)$, and so $w^i (b)=b$. In either case we have $w^i=1$ since
      $lr(chevron.l w chevron.r)$ operates faithfully on $Gamma$. Hence $i=h$
      and $theta=pi slash h$.
      ...
      negatives. We show that these are the only roots orthogonal to $a$.
      Suppose
      ...
      #block(sticky: true)[
        Since each $xi_i>0$ and all $eta_i$ have the same sign, this implies
        that
      ]
      $
      ...
      $Gamma$ at one of the transforms $w^i (a)$ or $w^i (b)$. For otherwise
      there would be a reflecting hyperplane intersecting $Gamma$ at a point
      strictly between $a$ and $b$, contrary to the fact that such points lie in
      the fundamental chamber. Now there are $h$ transforms of $a$ and $h$
      transforms of $b$ alternating round $Gamma$. Thus the total number of
      reflecting hyperplanes is
      $
        1/2 [(l-k)h+k h]=1/2 l h
      $ <eq:coxeter-number-root-count-hyperplane-count>
      since each hyperplane meets $Gamma$ in two points. Thus $N=1/2 l h$, and
      so $h=2N slash l$, as required.
    ]

    #corollary(<cor:primitive-coxeter-eigenvalue>)[
      #block(sticky: true)[
    ...
    #corollary(<cor:regular-coxeter-eigenvector>)[
      #block(sticky: true)[
        A Coxeter element $w$ has an eigenvector $v$ with eigenvalue
        $upright(e)^(2pi i slash h)$ such that $v$ is not orthogonal to any
        root.
      ]
    ]

    #proof[
      Let $v$ be an eigenvector with eigenvalue $upright(e)^(2pi i slash h)$
      lying in the plane of $Gamma$. Since $w$ operates as a rotation in this
      plane $v$ is not real, so $v!=overline(v)$. Suppose $(v,r)=0$, where
      $r in Phi$. Then $(overline(v),r)=0$ since $r$ is real, and it follows
      that $r$ is orthogonal to every vector in the plane of $Gamma$. Since this
      plane contains points in the fundamental chamber we have a contradiction.
      Thus $v$ is not orthogonal to any root.
    ]

    #proposition(<p:coxeter-element-no-fixed-vector>)[
      #block(sticky: true)[
    ...
    #proof[
      #block(sticky: true)[
        This is much easier to prove than the preceding results. Suppose a
        Coxeter element $w$ fixes a vector $v$. Then
      ]
      $
      ...
      and $w_(p_1) (v)-v$ is a scalar multiple of $p_1$. Since
      $p_1,p_2,dots,p_l$ are linearly independent we have
      $
      ...
      Repeating the argument we see that $v$ is orthogonal to
      $p_1,p_2,dots,p_l$, hence $v=0$. Thus $w$ fixes no non-zero vector and so
      has no eigenvalue $1$.
    ]
  ...
  #[
    ...
    #theorem(<th:coxeter-eigenvalues-invariant-degrees>)[
      #block(sticky: true)[
      ...
      where $zeta=upright(e)^(2pi i slash h)$ and $m_1,m_2,dots,m_l$ are
      positive integers less than $h$. Then the degrees of the basic invariants
    ...
    #proof[
      The eigenvalues of a Coxeter element $w$ are $h$th roots of unity, where
      $h$ is the order of $w$, so are powers of $zeta$. By #book-ref(
        "p",
        "coxeter-element-no-fixed-vector",
      ...
      $y_1,dots,y_l$ are linear functions in $x_1,dots,x_l$ and vice versa. Now
      we may assume that $m_1=1$ by #book-ref(
        "cor",
        "primitive-coxeter-eigenvalue",
      ) and that $f_1$ is not orthogonal to any root by #book-ref(
        "cor",
        "regular-coxeter-eigenvector",
      ).
      #block(sticky: true)[Let]
      $
      ...
      expressed in terms of $y_1,dots,y_l$. By #book-ref(
        "th",
      ...
      does, since $w$ is a real transformation. We now renumber the basis
      vectors $f_1,dots,f_l$ so that the eigenvalue $zeta^(m_i)$ is replaced by
      ...
      $m_i$, $i=1,dots,l$. (We have again used #book-ref(
        "p",
        "coxeter-element-no-fixed-vector",
      ) here.) Thus
      $
      ...
      #block(sticky: true)[
        But $1/2 l h=N$ by #book-ref("th", "coxeter-number-root-count"), and so
      ]
      $
        sum_(i=1)^l m_i=N.
      ...
      by #book-ref("th", "degree-product-and-sum"). Since
      $d_i-1 equiv m_i mod h$ and $0<m_i<h$, these equations imply that
      $d_i-1=m_i$ for $i=1,dots,l$.
    ]
    ...
    #proof[
      This follows from #book-ref("cor", "degrees-and-exponents") and #book-ref(
        "th",
        "coxeter-eigenvalues-invariant-degrees",
      ).
    ]
    ...
    #corollary(<cor:coxeter-number-highest-root>)[
      #block(sticky: true)[
        The following three integers are equal:
      ]
      #enum(
        numbering: "(i)",
