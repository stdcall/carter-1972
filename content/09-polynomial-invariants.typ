#[
  ...
  #source(136, "123")
  = 9. Polynomial Invariants of the Weyl Group <ch:9>

  == 9.1 The Algebra of Polynomial Invariants <sec:9-1>
  ...
  see in the #book-ref("ch", "10", [following chapter], highlight: false), but
  we describe them first in terms of polynomial invariants as this appears to be
  the approach which generalizes most readily to the twisted groups to be
  considered in chapters #book-ref("ch", "13", [13]), #book-ref(
    "ch",
    "14",
    [14],
  ).
  ...
  as a map from $frak(V)$ to $RR$. Let $frak(S)$ be the algebra of all such
  polynomial functions on $frak(V)$. $frak(S)$ is independent of the basis
  chosen for $frak(V)$. For the homogeneous polynomials of degree one in
  ...
  $(l+1)$-dimensional Euclidean space with orthonormal basis $e_0,e_1,dots,e_l$
  whose elements satisfy $x_0+x_1+dots+x_l=0$. The elements of $W$ operate on
  $frak(V)$ by permuting the coordinates $x_0,x_1,dots,x_l$ in all possible
  ways. Thus the polynomial invariants are the symmetric polynomials in
  $x_0,x_1,dots,x_l$. These are all polynomials in the elementary symmetric
  polynomials. Since $x_0+x_1+dots+x_l=0$ we have
...
#[
  ...
  == 9.2 A Theorem of Chevalley <sec:9-2>
  ...
  Since $Av Q_i in frak(I)$, this means that $J_1$ is in the ideal of $frak(I)$
  generated by $J_2,dots,J_k$, a contradiction.
  ...
  Now $P_i$ is homogeneous, hence $w_r (P_i)$ is homogeneous of the same degree.
  Thus $w_r (P_i)-P_i$ is also homogeneous, and it follows that $overline(P)_i$
  is homogeneous. Moreover $deg overline(P)_i<deg P_i$. Now we have
  ...
  But $deg overline(P)_1<deg P_1$ and so $overline(P)_1 in frak(S)frak(I)^+$ by
  induction. Thus
  $ w_r (P_1)-P_1 in frak(S)frak(I)^+. $ <eq:9-2-1-reflection-congruence>

  Now $w(frak(I)^+)=frak(I)^+$ for each $w in W$, hence
  $w(frak(S)frak(I)^+)=frak(S)frak(I)^+$. Thus each $w in W$ operates naturally
  on the quotient ring $frak(S)\/frak(S)frak(I)^+$. We have seen that $w_r$
  operates trivially on $P_1$ in this quotient ring for each $r in Phi$. Since
  the $w_r$ generate $W$, each $w in W$ operates trivially on $P_1$ in the
  quotient ring. Thus
  ...
  #source(140, "127")
  #smallcaps[Proof.] Suppose $P(I_1,dots,I_n)=0$ with $P!=0$. We may assume, by
  comparing terms of a given degree, that all monomials in $I_1,dots,I_n$ which
  occur in $P$ have the same degree $d$ in $x_1,dots,x_l$.
  ...
  Then $P_i (I_1,dots,I_n)$, $i=1,dots,n$, are elements of $frak(I)$ and not all
  the $P_i$ are zero. Let $frak(K)$ be the ideal of $frak(I)$ generated by
  ...
  by comparing terms of the same degree in $x_1,dots,x_l$ on both sides, we may
  assume each $Q_(i,j)$ is homogeneous of degree $deg P_i-deg P_j$. Thus
  ...
  $ <eq:9-2-2-regrouped>
  We now apply #book-ref("l", "9-2-1", [9.2.1]). $P_1,dots,P_m$ are in $frak(I)$
  ...
  #block(sticky: true)[
    We now multiply this polynomial by $x_k$ and sum over $k$. For a homogeneous
    polynomial $I_j$ in $x_1,dots,x_l$ we have, by Euler's formula,
  ]
  $
  ...
  homogeneous of degree $deg I_1$. Comparing terms of this degree on both sides
  we obtain
  $
  ...
  including $i=1$ (since the monomials in $I_1 R_1$ have too large a degree). It
  follows now that $I_1$ is in the ideal of $frak(S)$ generated by
  ...
  #smallcaps[Proof.] It is sufficient to prove this for homogeneous polynomials
  in $frak(I)$. Let $J in frak(I)$ be homogeneous. We use induction on $deg J$,
  the result being clear if $deg J=0$. Suppose $deg J>0$. Then $J in frak(I)^+$
  and in particular $J in frak(S)frak(I)^+$. Thus we have
  ...
  than $deg J$. Thus they are polynomials in $I_1,dots,I_n$ by induction, and so
  $J$ is also. #qed
  ...
  $I_1,dots,I_n$ is called a set of _basic polynomial invariants_ of $W$. We now
  determine the number of invariants in a basic set.

  #block(sticky: true)[
    #metadata((kind: "anchor")) <th:9-2-5>
    #smallcaps[Theorem 9.2.5.] _The number of invariants in a basic set is equal
    to the rank of the Weyl group._
  ]

  #source(142, "129")
  #smallcaps[Proof.] In the above notation we must show that $n=l$. Let
  $ K=RR(x_1, dots, x_l) $ <eq:9-2-5-rational-field>
  be the field of rational functions in $x_1,dots,x_l$ over $RR$. Similarly let
  $k=RR(I_1, dots, I_n)$ be the field of rational functions in $I_1,dots,I_n$
  over $RR$. Then we have
  ...
  #block(width: 100%)[
    ...
    under each element of $W$ so are in $frak(I)$ and therefore in $k$. Thus $K$
    is generated over $k$ by a finite number of algebraic elements over $k$, and
    so
...
#[
  ...
  == 9.3 The Degrees of the Basic Invariants <sec:9-3>

  Now the set $I_1,dots,I_l$ of basic polynomial invariants of $W$ is not
  uniquely determined. It is not difficult to show, however, that the degrees of
  the polynomials in a basic set are uniquely determined.

  #block(sticky: true)[
    #metadata((kind: "anchor")) <p:9-3-1>
    #smallcaps[Proposition 9.3.1.] _Let $I_1,dots,I_l$; $I'_1,dots,I'_l$ be two
    sets of basic
    #source(143, "130")
  ...
  #smallcaps[Proof.] Each of $I'_1,dots,I'_l$ is expressible as a polynomial in
  $I_1,dots,I_l$ and conversely. Consider the matrices
  ...
  By symmetry we must have equality. Thus $deg I_i=deg I'_i$ for each $i$. #qed

  We shall denote the degrees of the basic invariants by $d_1,d_2,dots,d_l$, and
  shall derive some properties of this set of integers.

  #block(width: 100%, breakable: false)[
    #metadata((kind: "anchor")) <l:9-3-2>
    #smallcaps[Lemma 9.3.2.] _Let $M$ be any finite-dimensional $W$-module over
    a field of characteristic $0$. Then the dimension of the subspace formed by
    the $W$-invariants of $M$ (i.e. the elements $x in M$ such that $w(x)=x$ for
    all $w in W$) is equal to the trace of the linear transformation_
  ...
  #source(144, "131")
  Then $T^2=T$. Let $M_0$ be the set of $x in M$ with $T(x)=0$ and $M_1$ be the
  set of $x in M$ with $T(x)=x$. Then $M=M_0 ⊕ M_1$ since $T$ is idempotent. The
  trace of $T$ is the dimension of $M_1$. Also the $W$-invariants of $M$ are
  just the elements which lie in $M_1$. For if $x$ is $W$-invariant,
  ...
  #source(145, "132")
  #smallcaps[Proof.] We have earlier defined the action of $W$ on the dual space
  $hat(frak(V))$ of $frak(V)$ and on the ring $frak(S)$ of polynomial functions
  on $frak(V)$. In the present proof we take $frak(V),hat(frak(V)),frak(S)$ over
  the complex field instead of the real field as before.

  $lambda_1,dots,lambda_l$ are the eigenvalues of $w$ on $frak(V)$, thus
  $lambda_1^(-1),dots,lambda_l^(-1)$ are the eigenvalues of $w$ on
  $hat(frak(V))$. However these eigenvalues are roots of unity and $w$ is a real
  transformation, hence the eigenvalues of $w$ on $hat(frak(V))$ are
  ...
  $frak(S)_n$, the space of polynomial functions which are homogeneous of degree
  $n$. Since $w(y_i)=lambda_i y_i$ we have
  $
  ...
  #block(sticky: true)[
    ...
    coefficient of $t^n$ in the power series $Av 1\/det(1-t w)$ is the trace of
    the linear transformation
  ]
  $ 1/abs(W) sum_(w in W) w $ <eq:9-3-3-trace-average>
  on $frak(S)_n$. By #book-ref("l", "9-3-2", [9.3.2]) this is the dimension of
  the space $frak(S)_n inter frak(I)=frak(I)_n$. Now $frak(I)_n$ has as a basis
  the set of all polynomials $I_1^(e_1)I_2^(e_2)dots I_l^(e_l)$ of degree $n$,
  where $I_1,dots,I_l$ are a set of basic invariants. Since $deg I_i=d_i$ the
  number of such polynomials is the number of solutions of the equation
  ...
  are the elements $w_r$ with $r in Phi$. For suppose $W$ contains a reflection
  $w_H$ in a hyperplane $H$ not orthogonal to any root. $H$ contains a vector
  $v$ not orthogonal to any root, and $w_H (v)=v$. $v$ lies in a chamber $C$
  (see section #book-ref("sec", "2-3", [2.3])), and since $w_H (C)$ is also a
  chamber containing $v$ we have $w_H (C)=C$. Hence $w_H=1$ by #book-ref(
    "cor",
    "2-3-2",
    [2.3.2],
  ) and we have a contradiction. Therefore $W$ contains exactly $N$ reflections,
  where $N=abs(Phi^+)$.
  ...
  To obtain the second result we first differentiate and then put $t=1$. We have
  $
    & product_(i=1)^l 1/(1+t+dots+t^(d_i-1))
      lr(
  ...
  By #book-ref("th", "9-3-4", [9.3.4]) $J$ has degree $N$. We now show that $J$
  ...
  open neighbourhoods of $x,T(x)$ in $1$–$1$ correspondence under $T$. (See, for
  example Loomis and Sternberg #bib-ref("loomisSternberg1968").) Now suppose $x$
  lies in a reflecting hyperplane $H_r$, $r in Phi$. Then any open neighbourhood
  of $x$ contains points $a,b$ such that $a!=b$ but $w_r (a)=b$. Then we have
  ...
  Since $H_r$ is an irreducible polynomial, $H_r$ divides $J$. This is true for
  all $r in Phi^+$, thus
...
#[
  ...
  == 9.4 A Theorem of Solomon <sec:9-4>
  ...
  #block(sticky: true)[
    ...
    #smallcaps[Lemma 9.4.1.] _Let $w$ be an element of $W_J$. Then $l(w)$ is the
    same whether $w$ is regarded as an element of the Weyl group $W$ or the Weyl
    group $W_J$._
  ]

  #smallcaps[Proof.] Suppose $r$ is a positive root not in $Phi_J$. Then $r$ is
  a positive combination of roots in $Pi$ involving some root in $Pi-J$. If
  ...
  roots are in $Phi_J$. The result follows by #book-ref("th", "2-2-2", [2.2.2]).
  #qed
  ...
  section #book-ref("sec", "2-6", [2.6]). By #book-ref("p", "2-6-3", [2.6.3])
  each element of the Coxeter complex can be transformed under $W$ into just one
  element of the form
  $
    C_J=lr(
      {v; vec(
  ...
  are fixed by $w$. By #book-ref("p", "2-6-1", [2.6.1]) $n_J (w)$ is equal to
  the number of left cosets $x W_J$ fixed by $w$ under left multiplication.
  ...
  #block(sticky: true)[
    ...
    #smallcaps[Lemma 9.4.2.] _Let $1_(W_J)$ be the unit character of $W_J$. Thus
    $1_(W_J) (w)=1$ for all $w in W_J$. Then $1_(W_J)^W (w)=n_J (w)$ for all
    $w in W$._
  ]
  ...
  Coxeter complex defined in terms of the reflecting hyperplanes, but a complex
  $cal(K)$ defined similarly by any finite set of hyperplanes of $frak(V)$. The
  dimension of an element $frak(K) in cal(K)$ is defined as the dimension of the
  smallest subspace of $frak(V)$ containing $frak(K)$.

  #block(width: 100%, breakable: false)[
    #metadata((kind: "anchor")) <l:9-4-4>
    #smallcaps[Lemma 9.4.4.] _Let $n_i$ be the number of elements of $cal(K)$ of
    dimension $i$. Then_
    $ sum_i (-1)^i n_i=(-1)^(dim frak(V)). $ <eq:9-4-4-euler-sum>
  ]

  #smallcaps[Proof.] We use induction on the number of hyperplanes. Suppose the
  result proved for a system of $n$ hyperplanes and that an additional
  hyperplane $H$ is then added. Each element of $cal(K)$ of dimension $i$ which
  is cut in two by $H$ has corresponding to it an element of dimension $i-1$ in
  $H$ separating the two parts. Thus the sum
  $ sum_i (-1)^i n_i $ <eq:9-4-4-unchanged-sum>
  remains unchanged. #qed

  #smallcaps[Proof of #book-ref("p", "9-4-3", [9.4.3]).] Let $U$ be the subspace
  of $frak(V)$ of elements fixed by $w$. The elements of the Coxeter complex
  which are fixed by $w$ are just the ones which lie in $U$, by #book-ref(
    "cor",
    "2-6-2",
    [2.6.2],
  ...
  #smallcaps[Proof.] Let $D_J$ be the set of distinguished coset representatives
  of $W_J$ in $W$ defined in #book-ref("th", "2-5-8", [2.5.8]). Define
  $
  ...
  This is $0$ unless $J_w$ is the empty set, when it is $1$. However if $J_w$ is
  empty $w$ transforms every positive root into a negative root, so $w=w_0$ by
  #book-ref("p", "2-2-6", [2.2.6]). Hence the above sum is $t^(l(w_0))=t^N$ and
  the result is proved. #qed
  ...
  said to be alternating if $w(P)=det w dot P$ for all $w in W$, and the set of
  alternating polynomials is denoted by $hat(frak(I))$. As before, $H_r$ denotes
  the linear form representing the hyperplane orthogonal to the root $r$,
  defined by $H_r (x)=(r,x)$.

  #block(width: 100%, breakable: false)[
    #metadata((kind: "anchor")) <l:9-4-6>
    #smallcaps[Lemma 9.4.6.] _A polynomial $P in frak(S)$ is alternating if and
    only if it is the product of an invariant polynomial with_
  ...
  #block(sticky: true)[
    ...
    $r in Phi$. Let $x$ be an element in the hyperplane orthogonal to $r$. Then
  ...
  Let $frak(S)_n$ be the set of homogeneous polynomial functions on $frak(V)$ of
  degree $n$, let $frak(I)_n=frak(I) inter frak(S)_n$ and
  ...
  However $det w dot w(P)=P$ if and only if $w(P)=det w dot P$; thus we require
  the dimension of the space of alternating elements of $frak(S)_n$, viz.,
  $dim hat(frak(I))_n$. Hence
  $ sum_J (-1)^abs(J) dim (frak(I)_J)_n=dim hat(frak(I))_n. $
  <eq:9-4-7-result>
  #qed

  We are now able to show that the polynomial $overline(P)_W (t)$ satisfies the
  analogue of #book-ref("th", "9-4-5", [9.4.5]).
  ...
  #smallcaps[Proof.] We prove this result by comparing the coefficients of $t^n$
  in the power series expansion of the rational functions
  $
  ...
  #source(156, "143")
  where $d'_1,dots,d'_(abs(J))$ are the degrees of the basic invariants of $W_J$
  on $frak(V)_J$, the subspace of $frak(V)$ spanned by roots in $J$. Since $W_J$
  operates as the identity on the subspace of $frak(V)$ orthogonal to
  $frak(V)_J$, the degrees of the basic polynomial invariants of $W_J$ on
  $frak(V)$ are $d'_1,dots,d'_(abs(J)),1,dots,1$. Thus the coefficient of $t^n$
  in
  ...
  #block(width: 100%, breakable: false)[
    ...
    #smallcaps[Theorem 9.4.10.] _Let $G=frak(L)(q)$ be a finite Chevalley group.
    Then_
    $ abs(G)=1/d q^N (q^(d_1)-1)(q^(d_2)-1)dots(q^(d_l)-1), $
    <eq:9-4-10-group-order>
    _where $d$ is defined as in section #book-ref("sec", "8-6", [8.6]), $N$ is
    the number of positive roots of $frak(L)$, and $d_1,dots,d_l$ are the
    degrees of the basic polynomial invariants of $W$._
  ]
