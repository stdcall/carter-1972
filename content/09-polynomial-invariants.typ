...
#let chapter = [
  #[
    ...
    #example(<ex:type-a-polynomial-invariants>)[
      The Weyl group of type $A_l$ is isomorphic to the symmetric group
      $S_(l+1)$. It is best described operating on the subspace of an
      $(l+1)$-dimensional Euclidean space with orthonormal basis
      $e_0,e_1,dots,e_l$ whose elements satisfy $x_0+x_1+dots+x_l=0$. The
      elements of $W$ operate on $frak(V)$ by permuting the coordinates
      $x_0,x_1,dots,x_l$ in all possible ways. Thus the polynomial invariants
      are the symmetric polynomials in $x_0,x_1,dots,x_l$. These are all
      polynomials in the elementary symmetric polynomials. Since
      $x_0+x_1+dots+x_l=0$ we have $frak(I)=RR[I_1,I_2,dots,I_l]$, where $I_i$
      is the elementary symmetric polynomial of degree $i+1$.
    ]

    #example(<ex:type-b-polynomial-invariants>)[
      The Weyl group of type $B_l$ operates on a Euclidean space with
      orthonormal basis $e_1,dots,e_l$ by permuting the coordinates
      ...
      $frak(I)=RR[I_1,I_2,dots,I_l]$ where $I_i$ is the $i$th elementary
      symmetric polynomial in $x_1^2,dots,x_l^2$.
    ]
    ...
    #lemma(<l:averaging-is-invariant>)[
      #block(sticky: true)[
  ...
  #[
    ...
    #lemma(<l:invariant-ideal-relation>)[
      #block(sticky: true)[
    ...
    #proof[
      ...
      $deg P_1=0$, $P_1$ is constant. Since $J_1$ is not in the ideal of
      $frak(S)$ generated by $J_2,dots,J_k$, we must have $P_1=0$. Thus
      ...
      $overline(P)_i$ is homogeneous. Moreover $deg overline(P)_i<deg P_i$. Now
      we have
    ...
    #theorem(<th:basic-invariants-algebraic-independence>)[
      #block(width: 100%, breakable: false)[
    ...
    #proof[
      ...
      // derivatives.
      Choose such a relation with $d$ as small as possible. Let
      $P_i=∂ P\/∂ I_i$. Then $P_i (I_1,dots,I_n)$, $i=1,dots,n$, are elements of
      $frak(I)$ and not all the $P_i$ are zero. Let $frak(K)$ be the ideal of
      $frak(I)$ generated by $P_1,P_2,dots,P_n$. We may choose the notation so
      that $P_1,dots,P_m$ but no proper subset generate $frak(K)$ as an ideal of
      $frak(I)$. Then there exist polynomials $Q_(i,j) in frak(I)$ such that
      $
        P_i=sum_(j=1)^m Q_(i,j)P_j, quad i>m.
      $ <eq:basic-invariants-algebraic-independence-minimal-generators>

      Now each $P_i$ is homogeneous in $x_1,dots,x_l$ of degree $d-deg I_i$.
      Thus, by comparing terms of the same degree in $x_1,dots,x_l$ on both
      sides, we may assume each $Q_(i,j)$ is homogeneous of degree
      $deg P_i-deg P_j$. Thus $deg Q_(i,j)=deg I_j-deg I_i$.
      ...
      We now apply #book-ref("l", "invariant-ideal-relation"). $P_1,dots,P_m$
      are in $frak(I)$ and $P_1$ is not in the ideal of $frak(I)$ generated by
      ...
      #block(sticky: true)[
        ...
        #book-ref("l", "invariant-ideal-relation") we have
      ]
      $
        (∂ I_1)/(∂ x_k)
      ...
      including $i=1$ (since the monomials in $I_1 R_1$ have too large a
      degree). It follows now that $I_1$ is in the ideal of $frak(S)$ generated
      by $I_2,dots,I_n$ and we have a contradiction. This completes the proof.
    ]

    #theorem(<th:basic-invariants-generate>)[
      #block(sticky: true)[
    ...
    #proof[
      It is sufficient to prove this for homogeneous polynomials in $frak(I)$.
      Let $J in frak(I)$ be homogeneous. We use induction on $deg J$, the result
      being clear if $deg J=0$. Suppose $deg J>0$. Then $J in frak(I)^+$ and in
      ...
      $Av P_1,dots,Av P_n$ are homogeneous polynomials in $frak(I)$ of degree
      less than $deg J$. Thus they are polynomials in $I_1,dots,I_n$ by
      induction, and so $J$ is also.
    ]

    #corollary(<cor:polynomial-invariant-ring>)[
      #block(sticky: true)[
    ...
    #proof[
      This follows from #book-ref(
        "th",
        "basic-invariants-algebraic-independence",
      ) and #book-ref("th", "basic-invariants-generate").
    ]
    ...
    #theorem(<th:number-of-basic-invariants>)[
      #block(sticky: true)[
    ...
    #proof[
      ...
      Also by #book-ref("th", "basic-invariants-algebraic-independence") we have
      ...
      #block(width: 100%)[
        ...
        polynomials in $w(x_i)$ for all $w in W$. These coefficients are
        invariant under each element of $W$ so are in $frak(I)$ and therefore in
        $k$. Thus $K$ is generated over $k$ by a finite number of algebraic
        elements over $k$, and so
  ...
  #[
    ...
    #proposition(<p:invariant-degrees-unique>)[
      #block(sticky: true)[
    ...
    #proof[
      ...
      $I'_1,dots,I'_l$, involves $I'_i$ and so $deg I_i>=deg I'_i$. In
      particular
      $
    ...
    #proof[
      ...
      of $frak(V)$ and on the ring $frak(S)$ of polynomial functions on
      $frak(V)$. In the present proof we take $frak(V),hat(frak(V)),frak(S)$
      over the complex field instead of the real field as before.
      ...
      non-negative integers $k_1,dots,k_l$ with $k_1+dots+k_l=n$ form a basis
      for $frak(S)_n$, the space of polynomial functions which are homogeneous
      of degree $n$. Since $w(y_i)=lambda_i y_i$ we have
      $
      ...
      #block(sticky: true)[
        ...
        $lambda_1^(k_1)dots lambda_l^(k_l)$. Hence the coefficient of $t^n$ in
        the power series $1\/det(1-t w)$ is the sum of the eigenvalues of $w$ on
      ...
      on $frak(S)_n$. By #book-ref("l", "averaging-trace") this is the dimension
      of the space $frak(S)_n inter frak(I)=frak(I)_n$. Now $frak(I)_n$ has as a
    ...
    #source(146, "133")
    #theorem(<th:degree-product-and-sum>)[
      #block(width: 100%, breakable: false)[
        #grid(
          columns: (auto, 1fr),
          column-gutter: 0.6em,
    ...
    #proof[
      Consider $det(1-t w)$. This is $(1-t)^l$ if $w=1$, $(1-t)^(l-1)(1+t)$ if
      $w$ is a reflection, and a polynomial not divisible by $(1-t)^(l-1)$
      otherwise. Now the only elements of $W$ which are reflections are the
      elements $w_r$ with $r in Phi$. For suppose $W$ contains a reflection
      $w_H$ in a hyperplane $H$ not orthogonal to any root. $H$ contains a
      vector $v$ not orthogonal to any root, and $w_H (v)=v$. $v$ lies in a
      chamber $C$ (see section #book-ref(
        "sec",
      ...
      $w_H (C)=C$. Hence $w_H=1$ by #book-ref(
        "cor",
        "simply-transitive-chamber-action",
      ) and we have a contradiction. Therefore $W$ contains exactly $N$
      reflections, where $N=abs(Phi^+)$.

      #block(sticky: true)[
        We now apply #book-ref("p", "molien-series"). Multiplying both sides by
        $(1-t)^l$ we have
      ]
      $
      ...
      second from the $N$ reflections, and the third from the remaining elements
      of $W$.)
    ...
    By #book-ref("th", "degree-product-and-sum") $J$ has degree $N$. We now show
    that $J$ has a factorization into linear factors.

    #theorem(<th:jacobian-factorization>)[
      #block(width: 100%, breakable: false)[
    ...
    #proof[
      ...
      suppose $x$ lies in a reflecting hyperplane $H_r$, $r in Phi$. Then any
      open neighbourhood of $x$ contains points $a,b$ such that $a!=b$ but
      $w_r (a)=b$. Then we have
      ...
      for $i=1,dots,l$. Thus $T(b)=T(a)$ and so $J=0$ at $x$. Hence the
      polynomial $J$ vanishes at each point at which the linear polynomial $H_r$
      vanishes. Since $H_r$ is an irreducible polynomial, $H_r$ divides $J$.
  ...
  #[
    ...
    #source(149, "136")
    #lemma(<l:parabolic-length-compatibility>)[
      #block(sticky: true)[
    ...
    #proof[
      ...
      Repeating this argument we see that $w(r)$ is a positive root not in
      $Phi_J$ for all $w in W_J$. Thus all positive roots transformed by $w$
      into negative roots are in $Phi_J$. The result follows by #book-ref(
        "th",
        "length-and-inversions",
      ).
    ]

    Lemma #book-ref("l", "parabolic-length-compatibility") shows that we may
    write $l(w)$ for $w in W_J$ without ambiguity.
    ...
    which are fixed by $w$. By #book-ref("p", "fundamental-face-stabilizer")
    $n_J (w)$ is equal to the number of left cosets $x W_J$ fixed by $w$ under
    left multiplication.
    ...
    #lemma(<l:induced-character-face-count>)[
      #block(sticky: true)[
        Let $1_(W_J)$ be the unit character of $W_J$. Thus $1_(W_J) (w)=1$ for
        all $w in W_J$. Then $1_(W_J)^W (w)=n_J (w)$ for all $w in W$.
      ]
    ]
    ...
    #proof[
      ...
      $x w x^(-1) in W_J$ if and only if $x w x^(-1)(C_J)=C_J$ by #book-ref(
        "p",
        "fundamental-face-stabilizer",
      ), and this holds if and only if $w$ fixes $x^(-1)(C_J)$. Now there are
      $n_J (w)$ such elements $x^(-1)(C_J)$ fixed by $w$ and for each one of
      them the element $x$ may be chosen in $abs(W_J)$ ways. Thus
      $ 1_(W_J)^W (w)=1/abs(W_J) dot abs(W_J)n_J (w)=n_J (w). $
      <eq:induced-character-face-count-face-count>
    ]

    #proposition(<p:alternating-face-character>)[
      #block(width: 100%, breakable: false)[
        $
    ...
    #lemma(<l:hyperplane-complex-euler-sum>)[
      #block(width: 100%, breakable: false)[
    ...
    #proof[
      We use induction on the number of hyperplanes. Suppose the result proved
      for a system of $n$ hyperplanes and that an additional hyperplane $H$ is
      then added. Each element of $cal(K)$ of dimension $i$ which is cut in two
      by $H$ has corresponding to it an element of dimension $i-1$ in $H$
      separating the two parts. Thus the sum
      $ sum_i (-1)^i n_i $ <eq:hyperplane-complex-euler-sum-unchanged-sum>
      remains unchanged.
    ]

    #proof(of: [ #book-ref("p", "alternating-face-character")])[
      Let $U$ be the subspace of $frak(V)$ of elements fixed by $w$. The
      elements of the Coxeter complex which are fixed by $w$ are just the ones
      which lie in $U$, by #book-ref("cor", "face-stabilizer-fixes-pointwise").
      We now apply #book-ref("l", "hyperplane-complex-euler-sum") to $U$. $n_i$
      is the number of elements of the Coxeter complex which have dimension $i$
      and lie in $U$. Since $dim C_J=l-abs(J)$ we have
      $
        n_i=sum_(#mathclap($abs(J)=l-i$)) n_J (w).
      $ <eq:alternating-face-character-dimension-count>
      #source(151, "138")
      Thus, by #book-ref("l", "hyperplane-complex-euler-sum"),
      $
    ...
    #theorem(<th:alternating-length-polynomial-identity>)[
      #block(width: 100%, breakable: false)[
        $ sum_J (-1)^abs(J) (P_W (t))/(P_(W_J) (t))=t^N. $
    ...
    #proof[
      Let $D_J$ be the set of distinguished coset representatives of $W_J$ in
      $W$ defined in #book-ref("th", "parabolic-coset-factorization"). Define
      $
        P_(D_J) (t)=sum_(#mathclap($w in D_J$)) t^(l(w)).
      $ <eq:alternating-length-polynomial-identity-coset-polynomial>
      #block(sticky: true)[
        Then, by #book-ref("th", "parabolic-coset-factorization"), we have
      ]
      $
      ...
      This is $0$ unless $J_w$ is the empty set, when it is $1$. However if
      $J_w$ is empty $w$ transforms every positive root into a negative root, so
      $w=w_0$ by #book-ref("p", "longest-weyl-element"). Hence the above sum is
      $t^(l(w_0))=t^N$ and the result is proved.
    ]
    ...
    #lemma(<l:alternating-polynomial-factorization>)[
      #block(width: 100%, breakable: false)[
    ...
    #proof[
      ...
      #block(sticky: true)[
        Hence $P(x)=0$. Thus $P$ vanishes at all points for which $H_r$
        vanishes, therefore $H_r$ divides $P$. In fact
      ]
      $
        product_(r in Phi^+) H_r
    ...
    #proof[
      ...
      Now by #book-ref("l", "induced-character-face-count")
      $1_(W_J)^W (w)=n_J (w)$ and by #book-ref(
        "p",
        "alternating-face-character",
      ) we have
      $
        sum_J (-1)^abs(J) n_J (w)=det w.
      ...
      We now apply #book-ref("l", "averaging-trace").
      $ chi_(W_J) lr((1/abs(W_J) sum_(w in W_J) w)) $
    ...
    the analogue of #book-ref("th", "alternating-length-polynomial-identity").

    #proposition(<p:alternating-degree-polynomial-identity>)[
      #block(width: 100%, breakable: false)[
        $ sum_J (-1)^abs(J) (overline(P)_W (t))/(overline(P)_(W_J) (t))=t^N. $
    ...
    #proof[
      ...
      $dim frak(I)_n$ as in #book-ref("p", "molien-series"). Thus the
      ...
      #book-ref("l", "alternating-polynomial-factorization").
      ...
      Since $W_J$ operates as the identity on the subspace of $frak(V)$
      orthogonal to $frak(V)_J$, the degrees of the basic polynomial invariants
      of $W_J$ on $frak(V)$ are $d'_1,dots,d'_(abs(J)),1,dots,1$. Thus the
      coefficient of $t^n$ in
      ...
      is $dim (frak(I)_J)_n$, also as in #book-ref("p", "molien-series").
      However
      $ sum_J (-1)^abs(J) dim (frak(I)_J)_n=dim hat(frak(I))_n $
      <eq:alternating-degree-polynomial-identity-coefficients>
      by #book-ref("p", "alternating-invariant-dimension"). It follows that the
      rational functions
      $
        sum_J ((-1)^abs(J))/((1-t)^l overline(P)_(W_J) (t)),
    ...
    #theorem(<th:poincare-polynomial-factorization>)[
    ...
    #proof[
      ...
      #book-ref("th", "alternating-length-polynomial-identity") and #book-ref(
        "p",
        "alternating-degree-polynomial-identity",
      ) we have
      $
        (t^N-(-1)^abs(Pi))/(P_W (t))
    ...
    #source(157, "144")
    #theorem(<th:finite-chevalley-order>)[
      #block(width: 100%, breakable: false)[
        ...
        where $d$ is defined as in section #book-ref(
          "sec",
    ...
    #proof[
      This follows from #book-ref("p", "finite-order-length-sum") and #book-ref(
        "th",
        "poincare-polynomial-factorization",
      ).
    ]
