...
#[
  ...
  introduced by J. Tits. We shall describe these structures here and demonstrate
  the connection with groups with a #box[$(B,N)$-pair]. Closely connected to
  these geometries are somewhat simpler structures on which Weyl groups (or more
  generally Coxeter groups) operate as groups of automorphisms. #ix(
    "complex",
  )These structures are the Coxeter complexes, which we have described in
  earlier chapters. However, in order to describe the buildings we first need an
  axiomatic system which picks out the essential features of the Coxeter
  complex. A geometry satisfying these axioms will be called an _abstract
  Coxeter complex_. We shall introduce these ideas by means of a series of
  definitions of increasing complexity.

  == Chamber Complexes <sec:chamber-complexes>

  #nx("op-St-A", [$op("St") A$], group: "latin-s", after: "S-U-n-K-f")[The star
    of an element $A$ in a chamber complex]We consider a set $Theta$ endowed
  with a relation $subset.eq$ of partial #ix("simplex")ordering. $Theta$ is
  called a _simplex_ if $Theta$ is isomorphic to the set of all subsets of some
  set, partially ordered by inclusion. $Theta$ is called a _complex_ if:
  ...
  number of elements $B$ such that $B$ is minimal with respect to the properties
  $B subset.eq A$, $B != 0$. Thus the set of elements $B$ with $B subset.eq A$
  is isomorphic to the set of subsets of a set with cardinality $rank A$.

  A subset $Theta'$ of $Theta$ with the induced partial ordering is called a
  _subcomplex_ if, for all $A in Theta'$, $B in Theta$ with $B subset.eq A$, we
  have $B in Theta'$. There exist subsets of a complex $Theta$ which are
  complexes, although not subcomplexes of $Theta$. For example, let $A in Theta$
  and define $st A$ (the star of $A$) by:
  $ st A={B in Theta; B supset.eq A}. $ <eq:chamber-complexes-star>
  #source(288, "275")
  Then $st A$ is a complex contained in $Theta$ but not a subcomplex of $Theta$
  (unless $A=0$).
  ...
  #ix("chamber complex")A #ix("chamber")complex $Theta$ is called a _chamber
  complex_ if:
  ...
  condition on the codimension means that $C_(i-1)$ is either equal to $C_i$ or
  that $C_(i-1) != C_i$ and the intersection has codimension $1$ in each.
  ...
  #source(289, "276")
  It follows from @l:chamber-complex-codimension that the terms of a sequence
  $C_0,C_1,...,C_m$ of the type described above are all chambers. A sequence of
  chambers of this #ix("gallery")type will be called a _gallery_. Two chambers
  $C,C'$ are said to be _adjacent_ if $codim(C inter C')=1$. Thus a gallery is a
  sequence of chambers in which every pair of #ix(
    "adjacent chambers",
  )consecutive chambers are either identical or adjacent.

  We now consider maps from one chamber complex to another. Let $Theta,Theta'$
  be chamber complexes. A map $alpha:Theta -> Theta'$ #ix(
    "morphism of chamber complexes",
  )is called a _morphism_ of chamber complexes if:
  ...
  A chamber complex is said to be _thin_ if every element of codimension $1$ is
  contained in exactly two chambers and _thick_ if every element of codimension
  $1$ is contained in at least three chambers.

  #lemma()[
    #block(width: 100%, breakable: false)[
      Let $Theta,Theta'$ be two chamber complexes in which each element of
      codimension $1$ is contained in at most two chambers, and let $alpha,beta$
      be two morphisms of $Theta$ into $Theta'$ injective on the set of
      chambers. Suppose there exists a chamber $C in Theta$ such that
      $alpha(A)=beta(A)$ for all $A subset.eq C$. Then $alpha=beta$.
    ]
  ] <l:chamber-morphism-uniqueness>

  #proof[
    The elements $A$ satisfying $A subset.eq C$ will be called the _faces_ of
    $C$. Suppose there is a chamber $C' in Theta$ such that $alpha,beta$ do not
    coincide on the faces of $C'$. Let $Gamma$ be a gallery of minimal length
    joining $C$ to $C'$. Then
    $
...
#[
  ...
  #ix("folding")Let $Theta$ be a thin chamber complex. A _folding_ of $Theta$ is
  an endomorphism $alpha$ of $Theta$ satisfying:

  #enum(numbering: "(a)", [$alpha^2=alpha$ ($alpha$ is idempotent).], [Each
    chamber in $alpha(Theta)$ is the image under $alpha$ of exactly two chambers
    of $Theta$.])
  ...
  #lemma()[
    #block(width: 100%, breakable: false)[
      ...
      $C,C'$ of $Theta$ such that $C in alpha(Theta)$, $C' in.not alpha(Theta)$.
      If $C,C'$ are any two such chambers we have $alpha(C')=C$.
    ]
  ] <l:folding-boundary-chambers>
  ...
  #proof[
    ...
    joined by a gallery there exists a pair of adjacent chambers $C,C'$ in such
    a gallery with $C in alpha(Theta)$, $C' in.not alpha(Theta)$.
  ...
  A set of chambers in a chamber complex is called _convex_ if every gallery of
  minimal length joining two chambers in the set has all its terms in the set.
  ...
  #proof[
    ...
    also a gallery joining $C$ to $C'$. By @l:folding-boundary-chambers we have
    $alpha(C_j)=C_(j-1)$ and so
  ...
  ] <l:folding-complement-convex>

  #proof[
    To establish this result we introduce a map $abar$ on the set of chambers of
    $Theta$. If $C$ is a chamber not in $alpha(Theta)$ we define $abar(C)=C$;
    while if $C in alpha(Theta)$, $C$ is the image under $alpha$ of exactly two
    chambers. One of these is $C$ and the other is defined as $abar(C)$.
    ...
    #source(292, "279")
    Then $alpha(C_(i-1))=C_i$ by @l:folding-boundary-chambers and so
  ...
  #lemma()[
    #block(width: 100%, breakable: false)[
      Let $alpha$ be a folding of the thin chamber complex $Theta$ and $C,C'$ be
      adjacent chambers of $Theta$ such that $C in alpha(Theta)$,
  ...
  #proof[
    ...
    $alpha(Theta)$ and the other not. By @l:folding-boundary-chambers
    ...
    joining $D$ to $C$. By the proof of @l:folding-complement-convex
  ...
  ] <p:folding-uniqueness>

  #proof[
    We showed in @l:folding-boundary-chambers that $alpha(C')=C$. Let $beta$ be
    any folding of $Theta$ such that $beta(C')=C$. Then
    @l:folding-distance-split implies that, for any chamber
    #source(293, "280")
    ...
    form a convex set by @l:folding-complement-convex. Hence $abar(Theta)$ is a
    chamber complex. Similarly $bbar(Theta)$ is a chamber complex. By
    @l:folding-distance-split a chamber belongs to $abar(Theta)$ if and only if
    it belongs to $bbar(Theta)$, hence $abar(Theta)=bbar(Theta)$.
    ...
    $alpha,beta$ coincide on all faces of $C'$. By
    @l:chamber-morphism-uniqueness it follows that $alpha,beta$ coincide on
    $abar(Theta)$. But $alpha(Theta) union abar(Theta)=Theta$, thus $alpha,beta$
    coincide on $Theta$.
  ]
  ...
  #proposition()[
    #block(width: 100%, breakable: false)[
      ...
      that $beta(C)=C'$. Then $beta$ has the same property for any other pair of
      adjacent chambers of this type, viz., if $D,D'$ are adjacent and
      $alpha(D')=D$, then $beta(D)=D'$.
    ]
  ] <p:opposite-folding-boundary>

  #proof[
    By @l:folding-distance-split a chamber is in $alpha(Theta)$ if and only if
    it is not in $beta(Theta)$. Thus $D' in beta(Theta)$,
    $D in.not beta(Theta)$, and so $beta(D)=D'$ by @l:folding-boundary-chambers.
  ]

  The folding $beta$ of @p:opposite-folding-boundary which, when it exists, is
  uniquely determined #ix("opposite folding")by $alpha$, is called the _opposite
  folding_ of $alpha$.
  ...
  ] <p:reflection-from-opposite-foldings>
  ...
  #proof[
    ...
    $C$. By @l:chamber-morphism-uniqueness $rho^2$ is the identity. In
    particular $rho$ is invertible, so is an automorphism.
  ]
...
#[
  ...
  #ix("abstract Coxeter complex")#nx("Sigma", [$Sigma$], group: "greek-σ")[An
    abstract Coxeter complex]An abstract Coxeter complex is a thin chamber
  complex $Sigma$ such that, given any pair $C,C'$ of adjacent chambers, there
  exists a folding $alpha$ of $Sigma$ with $alpha(C')=C$.
  ...
  #proof[
    ...
    hence $C' in alpha(Sigma)$ by @l:folding-distance-split. Thus all faces of
    $C inter C'$ are in $alpha(Sigma)$. Consider the minimal non-zero faces of
    $C$. These are all faces of $C inter C_1$ except one, which we call $V$. Now
    $V subset.eq C$ and so $alpha(V) subset.eq C_1$, hence $alpha(V) != V$. Thus
    $V in.not alpha(Sigma)$, and so $V$ cannot be a face of $C inter C'$. It
    follows that $C inter C' subset.eq C inter C_1$. Hence $A$ is contained in
    $C_1$. Using induction we see that $A$ is contained in each $C_i$. Thus
    $st A$ is a chamber complex. $st_Sigma A$ is certainly thin, so it remains
    only to check the existence of all possible foldings.
    ...
    by @p:reflection-from-opposite-foldings. Thus the two chambers $D$ and
  ...
  showing first that such a complex can be ‘folded down’ into a single chamber.
  ...
  ] <p:chamber-retraction>

  _Note._ An idempotent #ix("retractions")morphism will be called a retraction.
  ...
  #proof[
    ...
    $C$ such that $gamma(Gamma)={C,C,...,C}$. We use induction on the length $m$
    of $Gamma$, the result being clear if $m=0$. If $C_1=C$ the result is clear
    by induction, so we assume $C_1 != C$. There is a folding $alpha$ of $Sigma$
    with $alpha(C_1)=C$. Thus $alpha(Gamma)={C,C,...,alpha(C_m)}$. By induction
    there exists an endomorphism $delta$ leaving invariant all faces of $C$ such
    that $delta alpha(Gamma)={C,C,...,C}$. Then $gamma=delta alpha$ has the
    required properties.
  ...
  The map $rho_C$ is called the retraction of $Sigma$ on to the simplex of faces
  of $C$.
  ...
  #ix(
    "type, of element in a building",
    sub: "of element in a chamber complex",
  )#lemma()[
    #block(width: 100%, breakable: false)[
      The equivalence relation defined on $Sigma$ is independent of the chamber
      $C$.
    ]
  ] <l:chamber-independent-types>

  #proof[
    Let $C'$ be another chamber of $Sigma$. We show that the equivalence
    relations for $C,C'$ are the same. Since any two chambers can be joined by a
    gallery we may assume $C,C'$ are adjacent. By
    @p:reflection-from-opposite-foldings there exists an automorphism $delta$ of
    $Sigma$ such that $delta(C)=C'$. Also $delta$ maps each face of $C inter C'$
    into itself. Thus $delta$ coincides with $rho_(C')$ on each face of
    $C inter C'$. Since $delta(C)=rho_(C')(C)$, $delta$ coincides with
    $rho_(C')$ on each face of $C$. Similarly $delta^(-1)$ coincides with
    $rho_C$ on each face of $C'$. Thus $delta rho_C$ is an endomorphism of
    $Sigma$ into $S(C')$:
    $
  ...
  #lemma()[
    #block(width: 100%, breakable: false)[
      ...
      type of each face of $C$. Then $gamma$ preserves the type of each element
      of $Sigma$.
    ]
  ] <l:type-preserving-endomorphism>
  ...
  Endomorphisms and automorphisms of the kind discussed #ix(
    "type-preserving automorphisms",
  )in @l:type-preserving-endomorphism will be called type-preserving.

  #source(297, "284")
  #lemma()[
    #block(width: 100%, breakable: false)[
      The automorphisms $rho$ defined in @p:reflection-from-opposite-foldings
      are type-preserving.
    ]
  ] <l:reflections-preserve-types>

  #proof[
    Let $alpha,beta$ be a pair of opposite foldings of $Sigma$ with respect to
    which $rho$ is defined. Let $C,C'$ be a pair of adjacent chambers such that
    $C in alpha(Sigma)$, $C' in beta(Sigma)$. Then $rho$ leaves invariant each
    face of $C inter C'$ and interchanges $C,C'$. Thus $rho$ preserves the type
    of $C$ and of each face of $C inter C'$, hence $rho$ preserves the type of
    each face of $C$. By @l:type-preserving-endomorphism $rho$ is
    type-preserving.
  ]

  #metadata((kind: "anchor")) <def:coxeter-complex-group>#nx(
    "W-Sigma",
    [$W(Sigma)$],
    group: "latin-w",
    definition: [@def:coxeter-complex-group],
  )[The group of type-preserving automorphisms of the abstract Coxeter complex
    $Sigma$] Let $W(Sigma)$ be the group generated by all the automorphisms of
  $Sigma$ of the kind described #ix("abstract Coxeter complex")in
  @p:reflection-from-opposite-foldings.
  ...
  ] <p:type-preserving-automorphism-group>

  #proof[
    By @l:reflections-preserve-types each element of $W(Sigma)$ is a
    type-preserving automorphism of $Sigma$. Conversely, let $delta$ be any
    type-preserving automorphism of $Sigma$. Let $C$ be a chamber of $Sigma$.
    Since any two chambers can be joined by a gallery it follows from
    @p:reflection-from-opposite-foldings and the fact that $Sigma$ is a Coxeter
    complex that there exists $gamma in W(Sigma)$ such that $gamma(C)=delta(C)$.
    Thus $gamma^(-1) delta$ is a type-preserving automorphism of $Sigma$ which
    fixes $C$. But a chamber $C$ has just one face of each type. Thus
    $gamma^(-1) delta$ fixes each face of $C$, so must be the identity, by
    @l:chamber-morphism-uniqueness. Hence $delta in W(Sigma)$.
  ]

  Let $C$ be a chamber of the abstract Coxeter complex $Sigma$. Each chamber
  adjacent to $C$ intersects $C$ in a face of codimension $1$. Now the number of
  faces of codimension $1$ in $C$ is rank $C$ (which may be finite or infinite),
  and each such face is contained in just one chamber other than $C$. Thus there
  are rank $C$ chambers of $Sigma$ adjacent to $C$. Each of these chambers gives
  rise to an involutary type-preserving automorphism of $Sigma$ as in
  @p:reflection-from-opposite-foldings. These #ix(
    "reflections",
  ...
  #theorem()[
    #block(width: 100%, breakable: false)[
      ...
      generate the group $W(Sigma)$. #ix("Coxeter group")Moreover $W(Sigma)$ is
      a Coxeter group with respect to this set of generators. #text(
        style: "normal",
      )[(cf. @def:coxeter-group.)]
    ]
  ] <th:abstract-coxeter-generators>
  ...
  #proof(qed: false)[
    ...
    fixing $C$, so must be the identity by @l:chamber-morphism-uniqueness. Thus
    $w in H$ and $H=W(Sigma)$.
    ...
    #source(299, "286")
    This will #ix("Matsumoto’s theorem")be sufficient, using a theorem of
    Matsumoto, to show #ix("Coxeter group")that $W(Sigma)$ is a Coxeter group.
    Let $w=w_(i_1) w_(i_2) dots w_(i_k)$ and write
    $
    ...
    Let $alpha$ be the folding with $alpha(C_1)=C$. By @l:folding-distance-split
    ...
    The proof of @th:abstract-coxeter-generators is completed by establishing
    the following result, due to Matsumoto.
  ]
  ...
  #theorem()[
    #block(sticky: true)[
      ...
      product of the involutary generators. A product $w_(i_1) dots w_(i_k)$ is
      called reduced if $l(w_(i_1) dots w_(i_k))=k$. Suppose $W$ satisfies the
      condition that, whenever $w_(i_2) dots w_(i_k)$ is reduced but
      $w_(i_1) dots w_(i_k)$ is not reduced, there exists an integer $j$ with
      $2<=j<=k$ such that $w_(i_2) dots w_(i_j)=w_(i_1) dots w_(i_(j-1))$. Then
      $W$ is generated by the $w_i$ as a Coxeter group.
    ]

    #text(style: "normal")[(The group $W(Sigma)$ considered above satisfies the
      hypotheses of this theorem.)]
  ] <th:exchange-condition-presentation>
...
#[
  ...
  #nx("Sigma-W-Pi", [$Sigma(W, Pi)$], group: "greek-σ")[The abstract Coxeter
    complex associated with a Weyl group $W$ and fundamental system $Pi$]We
  shall now show, as is #ix("Coxeter complex")to be expected, that the #ix(
    "abstract Coxeter complex",
  )Coxeter complex as defined in @sec:coxeter-complex[section] is an abstract
  Coxeter complex.
  ...
  #theorem()[
    ...
    #enum(
      numbering: "(i)",
      [The Coxeter complex $Sigma$ of $W$ (defined as in
        @sec:coxeter-complex[section]), with partial order $prec.eq$ defined by
        $K_1 prec.eq K_2$ if and only if $K_1$ is contained in the closure
        $overline(K)_2$ of $K_2$.],
  ...
  ] <th:geometric-and-coset-complexes>

  #proof[
    We show that $Sigma$ and $Sigma(W, Pi)$ are isomorphic as partially ordered
    sets, and then show that $Sigma(W, Pi)$ is an abstract Coxeter complex.
    ...
    #source(302, "289")
    closure $overline(C)$ are the elements $C_J$ defined in
    @sec:coxeter-complex[section]. The stabilizer of $C_J$ under the action of
    $W$ on $Sigma$ is $W_J$, by @p:fundamental-face-stabilizer. Moreover, each
    element of $Sigma$ can be expressed in the form $w(C_J)$ for exactly one
    subset $J$ of $Pi$, by @p:coxeter-face-orbit-representative. Let $w_1,w_2$
    be two elements of $W$. Then $w_1(C_J)=w_2(C_J)$ if and only if
    ...
    by @l:parabolic-length-compatibility. It follows that
    $
  ...
  We #ix("type-preserving automorphisms")determine next the group of
  type-preserving automorphisms of the abstract Coxeter complex $Sigma(W, Pi)$.

  #proposition()[
    #block(width: 100%, breakable: false)[
      The group of type-preserving automorphisms of $Sigma(W, Pi)$ is isomorphic
      to $W$.
    ]
  ] <p:coset-complex-automorphisms>

  #proof[
    Let $alpha$ be the folding of $Sigma(W, Pi)$ which maps $w’w_r$ to $w'$. The
    effect of $alpha$ on the chambers is given by:
    $
      alpha(w)=cases(w & "if" w in W_1, w_s w & "if" w in W_2.)
    $ <eq:coset-complex-automorphisms-positive-folding>
    #source(304, "291")
    (The notation is as in @th:geometric-and-coset-complexes.) By replacing $w'$
    by $w’w_r$ we may determine the opposite folding $beta$ of $alpha$. $beta$
    operates on the chambers by:
    $
    ...
    kind #ix("type-preserving automorphisms")generate the whole group of
    type-preserving automorphisms of $Sigma(W, Pi)$, by
    @p:type-preserving-automorphism-group. Since the Weyl group $W$ is generated
    by its reflections it follows that the group of type-preserving
    automorphisms is isomorphic to $W$.
  ]

  We now show that $Sigma(W, Pi)$ is, #ix("abstract Coxeter complex")to within
  isomorphism, the only abstract Coxeter complex whose group of type-preserving
  automorphisms is isomorphic to $W$.
  ...
  #theorem()[
    #block(sticky: true)[
      ...
      Coxeter group as in @th:abstract-coxeter-generators, has the same
      relations as $W$ when generated by a system of fundamental reflections.
      Then $Sigma$ is isomorphic to $Sigma(W, Pi)$.
    ]
  ] <th:coxeter-complex-uniqueness>

  #proof[
    Let $C$ be a chamber of $Sigma$. Then by @th:abstract-coxeter-generators
    ...
    Consider the complex $st C_J$. This is also #ix("Coxeter group")an abstract
    Coxeter complex by @l:residue-coxeter-complex. The group of type-preserving
    automorphisms of $st C_J$ is
    #source(305, "292")
    isomorphic to the subgroup $W_J$ of $W(Sigma)$ generated by the elements
    $rho_i$ for $i in J$, also by @th:abstract-coxeter-generators. In
    ...
    $w in H$ could not all be distinct. However, this contradicts
    @l:chamber-morphism-uniqueness. Thus $W_J$ is the stabilizer of $C_J$ in
    $W$.
  ...
  _Note._ We have shown that there is just one abstract Coxeter complex $Sigma$
  (to within isomorphism) whose group $W(Sigma)$ of type-preserving
  automorphisms is isomorphic to $W$, if $W$ is a Weyl group. In general
  $W(Sigma)$ is a Coxeter group. Now there are Coxeter groups which are not Weyl
  groups. (A classification of the finite Coxeter groups is given in Bourbaki
  #bib-ref("bourbaki1968").) Although such groups will not be discussed in the
  present volume it is possible to extend the result mentioned above to all
  Coxeter groups. Given any Coxeter group $W$ there is, to within isomorphism, a
  unique abstract Coxeter complex whose group of type-preserving automorphisms
  is isomorphic to $W$.
]
...
#[
  ...
  #ix("building")#nx("Omega", [$Omega$], group: "greek-ω")[A building]A
  _building_ is a pair $(Omega,cal(A))$ where $Omega$ is a chamber complex and
  $cal(A)$ is #ix("apartments")a set of subcomplexes, called _apartments_,
  ...
  It follows from these axioms that any two apartments of a building $Omega$ are
  isomorphic. For let $Sigma,Sigma'$ be apartments and $C,C'$ be chambers with
  $C in Sigma$ and $C' in Sigma'$. Let $Gamma={C=C_0,C_1,...,C_m=C'}$ be a
  gallery joining $C,C'$. There exist apartments $Sigma_1$ containing $C_0,C_1$;
  $Sigma_2$ containing $C_1,C_2$; etc. By @def:buildings-b4[B4] we have
  $
    Sigma tilde.eq Sigma_1 tilde.eq Sigma_2 tilde.eq ... tilde.eq Sigma_m
    tilde.eq Sigma'.
  $ <eq:buildings-apartments-isomorphic>

  #ix("building", sub: "associated with a group with $(B,N)$-pair")#nx(
    "Omega-G-B-N",
    [$Omega(G; B, N)$],
    group: "greek-ω",
  )[The building associated with a group $G$ with $(B,N)$-pair]#example(
    title: [The building
      $Omega(G; B, N)$],
  )[
    #block(sticky: true)[
    ...
    subsets $J$ of $I$, by @th:subgroups-containing-borel, and have the form
    $P_J=lr(⟨B,n_i;i in J⟩)$ by @p:parabolic-generated-by-double-coset. Let
    $Omega$ be the set of left cosets $g P_J$ for all $g in G$ and all subsets
    $J$ of $I$. We introduce a partial ordering on $Omega$ which is the reverse
    of set-theoretical inclusion. Let $Sigma_0$ be the subset of $Omega$ which
    ...
    $n_i B n_i=B$, which contradicts @def:buildings-bn5[BN5]. It follows that
    $g B$ and $g n_i B$ are not the only chambers containing $g P_J$. Thus
    $Omega$ is thick.

    Now $P_J=B union B n_i B$ and the only cosets of $B inter N$ in
    $N inter P_J$ are $B inter N$ and $n_i (B inter N)$ by
    @p:parabolic-generated-by-double-coset and @th:abstract-parabolic-lattice.
    Thus $B$ and $n_i B$ are the only chambers containing $P_J$ of the form
    $n B$ for $n in N$. It follows that $n B$ and $n n_i B$ are the only
    chambers in $Sigma_0$ containing $n P_J$. Thus $Sigma_0$ is thin, and hence
    all the other apartments of $Omega$ are thin also.
  ...
  ] <ex:building-from-bn-pair>

  #lemma()[
    #block(width: 100%, breakable: false)[
      Let $G$ be a #ix("$(B,N)$-pair")group with a $(B,N)$-pair. Then for each
      $n in N$ we have
      $
        P_J n P_K inter N=N_J n N_K.
      $ <eq:parabolic-double-coset-intersection-intersection>
    ]
  ] <l:parabolic-double-coset-intersection>

  (The notation is as in @p:abstract-bruhat-and-parabolics.)

  #proof[
    Axiom @def:buildings-bn4[BN4] applied repeatedly shows that, for any subset
    $N_0$ of $N$, we have
    $
    ...
    by @p:bruhat-double-coset-uniqueness. The reverse inclusion is obvious.
  ]

  We can now complete the argument to show that $Omega(G; B, N)$ is a building.
  We have $P_J n P_K=P_J n’P_K$ and so, by
  @l:parabolic-double-coset-intersection, we obtain $N_J n N_K=N_J n’N_K$. Thus
  $
  ...
  Let $overline(n) in N_J inter n’N_K n^(-1)$. We show that left multiplication
  by the element $p_J overline(n)$ gives an isomorphism from $Sigma_0$ to
  $g Sigma_0$ with the required properties. We have
  $
  ...
  Thus this isomorphism fixes $P_J$ and $n P_K$ and clearly fixes also all faces
  of these two elements, since the faces are larger subsets of $G$. Thus axiom
  @def:buildings-b4[B4] has been established, and so $Omega(G; B, N)$ is a
  building.
]
...
#[
  ...
  We have shown #ix("building")above that there is a building associated to each
  group with a $(B,N)$-pair, and have therefore established the existence of a
  large number of buildings. We shall now prove some further general properties
  #source(309, "296")
  of buildings, concentrating on the relationship #ix("apartments")between a
  ...
  containing $A$ and $C$, by axiom @def:buildings-b3[B3]. By
  @def:buildings-b4[B4] there is an isomorphism $Sigma' arrow.r Sigma$ which
  leaves invariant all faces of $C$. By @l:chamber-morphism-uniqueness there is
  only one such isomorphism. The image of $A$ under this isomorphism is an
  element of $Sigma$ which is independent of the choice of $Sigma'$, by
  @def:buildings-b4[B4]. This image will be called $retr_(Sigma,C)(A)$.
  ...
  #lemma()[
    #block(width: 100%, breakable: false)[
      ...
      #ix("retractions")is a retraction from $Omega$ onto $Sigma$.
    ]
  ] <l:building-apartment-retraction>

  #proof[
    The map is clearly a morphism, and since it acts as the identity on $Sigma$
    it is idempotent.
  ]
  ...
  #ix(
    ...
    sub: "on the apartments of a building",
  )#theorem()[
    #block(width: 100%, breakable: false)[
      The #ix("abstract Coxeter complex")apartments of a building are abstract
      Coxeter complexes.
    ]
  ] <th:building-apartments-coxeter>

  #proof[
    Let $Sigma$ be an apartment of a building $Omega$. We know that $Sigma$ is a
    thin chamber complex and so must prove the existence of all possible
    foldings. Let $C,C'$ be adjacent chambers of $Sigma$ and let $A=C inter C'$.
    Since $Omega$ is thick there is a third chamber $C''$ containing $A$. Let
    $Sigma'$ be an apartment containing $C,C''$. Let $alpha$ be the map of
    $Sigma$ into itself given by
    $
    ...
    centre $E$.#footnote[The book argues instead that $retr_(Sigma,C')$
      restricted to $Sigma'$ is an isomorphism. It is not: it maps both $C$ and
      $C''$ to $C$. The distance argument follows the proof of Theorem 4.131 in
      #cite(<ed:abramenko-brown>, form: "full"). — _Ed._] Indeed, choose an
    apartment containing $E$ and $D$. On this apartment the retraction is an
    isomorphism fixing $A$, and the preceding observation identifies the
    distances measured in the two apartments with $d(A,D)$ and
    $d(A,retr_(Theta,E)(D))$. Since $A$ is a face of both $C$ and $C'$, both
    retractions in the expression for $alpha$ preserve distance from $A$.
    Consequently
    $
      d(A,alpha(D))=d(A,D).
  ...
  #ix("type, of element in a building")#definition()[
    #block(sticky: true)[
      Let $A,A'$ be two elements of a building $Omega$. Then $A,A'$ are said to
      have the same type in $Omega$ if they have the same type in any apartment
      containing $A,A'$.
    ]
  ] <def:building-types>

  We observe that this condition is independent of the apartment chosen. For let
  $Sigma_1,Sigma_2$ be two apartments containing $A,A'$. Let $C,C'$ be chambers
  of $Sigma_1$ containing $A,A'$ respectively. Let $gamma$ be the retraction of
  $Sigma_1$ onto the simplex $S(C)$ defined in @p:chamber-retraction. Then
  $A,A'$ have the same type in $Sigma_1$ if and only if $gamma(A')=A$. Now by
  @def:buildings-b4[B4] there exists an isomorphism
  $delta:Sigma_1 arrow.r Sigma_2$ which leaves $A$ and $A'$ invariant. Thus the
  retraction of $Sigma_2$ onto the simplex $S(delta(C))$ is given by
  $ delta(X) arrow.r delta gamma(X), quad X in Sigma_1. $
  <eq:building-types-conjugate-retraction>
  $A$ and $A'$ have the same type in $Sigma_2$ if and only if $A'$ is mapped to
  $A$ under this retraction, which holds if and only if $gamma(A')=A$.
]
...
#[
  ...
  #ix("type-preserving automorphisms")We shall be concerned with groups of
  type-preserving automorphisms of a building $Omega$, and first give an example
  in the building $Omega(G; B, N)$ constructed from a group $G$ with a
  $(B,N)$-pair.

  #ix("$(B,N)$-pair", sub: "operating on a building")#proposition()[
    #block(sticky: true)[
      ...
      transitive on the pairs $(C,Sigma)$, where $C$ is a chamber and $Sigma$ is
      an apartment containing $C$.
    ]
  ] <p:bn-pair-building-action>

  #proof[
    We use the notation of @ex:building-from-bn-pair. The elements of $Omega$
    are cosets of the form $g P_J$. Consider the apartment $Sigma_0$ of $Omega$.
    The elements of $Sigma_0$ have form $n P_J$, where $n in N$, and the map
    ...
    chamber $B$. It is therefore the retraction described in
    @p:chamber-retraction. Thus $n_1 P_J$ and $n_2 P_K$ have the same type if
    and only if $J=K$. Similarly in the apartment $Sigma=g Sigma_0$ of $Omega$,
    the elements $g n_1 P_J$ and $g n_2 P_K$ have the same type if and only if
    $J=K$. We now consider any two elements $g_1 P_J$ and $g_2 P_K$ of $Omega$.
    They have the same type in $Omega$ if and only if they have the same type in
  ...
  #theorem()[
    #block(sticky: true)[
      ...
      automorphisms of $Omega$ which is transitive on the pairs $(C,Sigma)$ with
    ]
    #source(313, "300")
    ...
    automorphisms of each apartment $Sigma$ of $Omega$.
  ] <th:building-gives-bn-pair>

  #proof[
    We verify the last assertion first. $N$ operates on $Sigma_0$ as a group of
    type-preserving automorphisms. The transitivity hypothesis shows that $N$
    operates transitively on the chambers of $Sigma_0$. Thus $N$ induces on
    $Sigma_0$ the full group $W(Sigma_0)$ of type-preserving automorphisms, by
    @l:chamber-morphism-uniqueness. Thus we have an epimorphism
    $N arrow.r W(Sigma_0)$ with kernel $B inter N$. For a type-preserving
    automorphism of $Sigma_0$ which fixes $C_0$ must be the identity, again by
    @l:chamber-morphism-uniqueness. Thus $N\/(B inter N)$ is isomorphic to
    $W(Sigma_0)$. This is isomorphic to $W(Sigma)$ for any apartment $Sigma$,
    since any two apartments are isomorphic.
    ...
    codimension $1$ in $C_0$ (see @th:abstract-coxeter-generators).
  ...
  @def:buildings-bn4[BN4]. It was shown that some element of $B$ transforms the
  chamber $n b n_i C_0$ into a chamber of $Sigma_0$ containing
  $n C_0 inter n n_i C_0$, which must therefore be either $n C_0$ or
  $n n_i C_0$.

  #source(315, "302")
  The relation between the algebraic and geometric structures discussed in this
  chapter may be summarized in the following scheme.

  #block(width: 100%, breakable: false)[
    #align(center)[#building-correspondence()]
  ] <fig:type-preserving-automorphisms-structure-scheme>

  #ix("Tits’ theorems, on finite groups with $(B,N)$-pair")J. Tits has recently
  carried out #ix("building")a detailed investigation of the buildings of the
  various different types. In particular he has shown that a finite building
  whose associated Coxeter group is an indecomposable Weyl group of rank at
  least $3$ must be a building $Omega(G; B, N)$, where $G$ is a finite Chevalley
  group or twisted group. Tits is able to deduce from this that the only finite
  simple groups with a $(B,N)$-pair of rank at least $3$ are the finite
  Chevalley groups and twisted groups which have this property (see Tits
  #bib-ref("bib335-238")).
]
