#[
  ...
  #source(287, "274")
  = 15. Associated Geometrical Structures <ch:15>
  ...
  generally Coxeter groups) operate as groups of automorphisms. These structures
  are the Coxeter complexes, which we have described in earlier chapters.
  However, in order to describe the buildings we first need an axiomatic system
  which picks out the essential features of the Coxeter complex. A geometry
  satisfying these axioms will be called an _abstract Coxeter complex_. We shall
  introduce these ideas by means of a series of definitions of increasing
  complexity.

  == 15.1 Chamber Complexes <sec:15-1>

  We consider a set $Theta$ endowed with a relation $subset.eq$ of partial
  ordering. $Theta$ is called a _simplex_ if $Theta$ is isomorphic to the set of
  all subsets of some set, partially ordered by inclusion. $Theta$ is called a
  _complex_ if:
  ...
  #smallcaps[Proof.] Let $A in Theta$ be contained in two chambers $C,C'$. Then
  ...
  Now it is readily verified that $codim_C (C_0 inter C_1 inter dots inter C_m)$
  is finite and it follows that $codim_A B$ is finite also. Hence
  $
  ...
  sequence of chambers in which every pair of consecutive chambers are either
  identical or adjacent.

  We now consider maps from one chamber complex to another. Let $Theta,Theta'$
  be chamber complexes. A map $alpha:Theta -> Theta'$ is called a _morphism_ of
  ...
  #block(width: 100%, breakable: false)[
    ...
    which each element of codimension $1$ is contained in at most two chambers,
    and let $alpha,beta$ be two morphisms of $Theta$ into $Theta'$ injective on
    the set of chambers. Suppose there exists a chamber $C in Theta$ such that
    $alpha(A)=beta(A)$ for all $A subset.eq C$. Then $alpha=beta$._
  ]

  #smallcaps[Proof.] The elements $A$ satisfying $A subset.eq C$ will be called
  the _faces_ of $C$. Suppose there is a chamber $C' in Theta$ such that
  $alpha,beta$ do not coincide on the faces of $C'$. Let $Gamma$ be a gallery of
  minimal length joining $C$ to $C'$. Then
  ...
  $alpha(C_(i-1))$ and $alpha(C_i)$ since $alpha$ is injective on chambers. Thus
  $beta(C_i)=alpha(C_(i-1))$ or $beta(C_i)=alpha(C_i)$.
  ...
  $C_(i-1) inter C_i$ of codimension $1$ in $C_i$. Since the faces of $C_i$ form
  a simplex and $alpha,beta$ are isomorphisms between the simplexes determined
  by $C_i$ and $alpha(C_i)$ it follows that $alpha,beta$ coincide on all faces
  of $C_i$, and we have a contradiction.
  #qed
]
...
#[
  ...
  #smallcaps[Proof.] Since $alpha$ is idempotent there exist chambers $C_1,C_2$
  with $C_1 in alpha(Theta)$, $C_2 in.not alpha(Theta)$. Since $C_1,C_2$ can be
  joined by a gallery there exists a pair of adjacent chambers $C,C'$ in such a
  gallery with $C in alpha(Theta)$, $C' in.not alpha(Theta)$.
  ...
  Thus two consecutive terms of $alpha(Gamma)$ are the same, and by omitting one
  we could obtain a gallery joining $C$ to $C'$ of shorter length than $Gamma$,
  a contradiction.
  #qed
  ...
  $alpha$ of exactly two chambers. One of these is $C$ and the other is defined
  as $abar(C)$.
  ...
  $ abar(Gamma)={abar(C_0),abar(C_1),...,abar(C_m)} $ <eq:15-2-3-image-gallery>
  ...
  these we would obtain a gallery joining $C,C'$ which is shorter than $Gamma$,
  a contradiction.
  #qed
  ...
  Suppose $D in alpha(Theta)$ and $Gamma$ is a gallery of minimal length joining
  $D$ to $C'$. Then $Gamma$ contains two consecutive terms, one in
  $alpha(Theta)$ and the other not. By #book-ref("l", "15-2-1", [15.2.1])
  $alpha(Gamma)$ is a gallery joining $D$ to $C$ which contains two consecutive
  terms which are equal. Hence $dist C D<=dist C' D-1$, and we must have
  equality.

  If $D in.not alpha(Theta)$ we take a gallery $Gamma$ of minimal length joining
  $D$ to $C$. By the proof of #book-ref("l", "15-2-3", [15.2.3]) $abar(Gamma)$
  is a gallery joining $D$ to $C'$ which contains two consecutive terms which
  are equal. Hence $dist C' D<=dist C D-1$, and we again have equality.
  #qed
  ...
  $beta(Theta)$. Since $alpha(Theta),beta(Theta)$ are chamber complexes we have
  $alpha(Theta)=beta(Theta)$. $alpha$ and $beta$, being idempotent, both operate
  as the identity on $alpha(Theta)$.

  Let $abar(Theta)$ be the subcomplex of $Theta$ consisting of all faces of all
  chambers of the form $abar(D)$ for $D in Theta$. The chambers of form
  ...
  $alpha(Theta) union abar(Theta)=Theta$, thus $alpha,beta$ coincide on $Theta$.
  #qed
  ...
  $D' in beta(Theta)$, $D in.not beta(Theta)$, and so $beta(D)=D'$ by #book-ref(
    "l",
    "15-2-1",
    [15.2.1],
  ).
  ...
  exists, is uniquely determined by $alpha$, is called the _opposite folding_ of
  $alpha$.

  #block(width: 100%, breakable: false)[
    #metadata((kind: "anchor")) <p:15-2-7>
    #smallcaps[Proposition 15.2.7.] _Let $alpha,beta$ be opposite foldings of
    $Theta$. Then there exists an automorphism $rho$ of $Theta$ which coincides
    with $beta$ on $alpha(Theta)$ and with $alpha$ on $beta(Theta)$. Also,
    $rho^2$ is the identity._
  ]
  ...
  chambers such that $alpha(C')=C$ and $beta(C)=C'$. Then $rho^2$ fixes $C$ and
  also fixes each face of $C inter C'$, hence $rho^2$ fixes each face of $C$. By
  #book-ref(
    "l",
    "15-1-2",
    [15.1.2],
...
#[
  ...
  == 15.3 Abstract Coxeter Complexes <sec:15-3>

  An abstract Coxeter complex is a thin chamber complex $Sigma$ such that, given
  any pair $C,C'$ of adjacent chambers, there exists a folding $alpha$ of
  $Sigma$ with $alpha(C')=C$.

  #block(width: 100%, breakable: false)[
    #metadata((kind: "anchor")) <l:15-3-1>
    #smallcaps[Lemma 15.3.1.] _Let $Sigma$ be an abstract Coxeter complex and
    $A$ be an element of $Sigma$. Then $st_Sigma A$ is also an abstract Coxeter
    complex._
  ]

  #smallcaps[Proof.] $st_Sigma A$ is certainly a complex. To show it is a
  chamber complex, let $C,C'$ be chambers containing $A$. There exists a gallery
  $ Gamma={C=C_0,C_1,...,C_m=C'} $ <eq:15-3-1-gallery>
  in $Sigma$ joining $C,C'$, where $m=dist C C'$. Let $alpha$ be the folding of
  $Sigma$ such that $alpha(C)=C_1$. Then
  ...
  $alpha(V) != V$. Thus $V in.not alpha(Sigma)$, and so $V$ cannot be a face of
  $C inter C'$. It follows that $C inter C' subset.eq C inter C_1$. Hence $A$ is
  contained in $C_1$. Using induction we see that $A$ is contained in each
  $C_i$. Thus $st A$ is a chamber complex. $st_Sigma A$ is certainly thin, so it
  remains only to check the existence of all possible foldings.
  ...
  folding $beta$ of $alpha$. Let $D$ be any chamber in $alpha(Sigma)$ containing
  $A$. Then $beta(D)$ also contains $A$, and
  $ alpha beta(D)=rho^2(D)=D $ <eq:15-3-1-opposite-folding>
  by #book-ref("p", "15-2-7", [15.2.7]). Thus the two chambers $D$ and $beta(D)$
  which $alpha$ transforms into $D$ both contain $A$. Hence $alpha$ induces a
  folding of $st_Sigma A$.
  #qed
  ...
  #block(width: 100%, breakable: false)[
    ...
    #smallcaps[Proposition 15.3.2.] _Let $Sigma$ be an abstract Coxeter complex,
    $C$ a chamber in $Sigma$ and $S(C)$ the simplex of all faces of $C$. Then
    there exists a unique idempotent morphism of $Sigma$ onto $S(C)$._
  ]
  ...
  $C_1=C$ the result is clear by induction, so we assume $C_1 != C$. There is a
  folding $alpha$ of $Sigma$ with $alpha(C_1)=C$. Thus
  $alpha(Gamma)={C,C,...,alpha(C_m)}$. By induction there exists an endomorphism
  $delta$ leaving invariant all faces of $C$ such that
  ...
  chamber $C'$ containing $A$ there is an endomorphism $gamma$ fixing all faces
  of $C$ with $gamma(C')=C$. We now show that the element $gamma(A) in S(C)$ is
  uniquely determined, i.e. that if $gamma,delta$ are endomorphisms of $Sigma$
  leaving invariant all faces of $C$ such that $gamma(A) in S(C)$ and
  $delta(A) in S(C)$, then $gamma(A)=delta(A)$.
  ...
  induction that they agree on all faces of $C_(i-1)$. Then they agree on $C_i$
  and on all faces of $C_(i-1) inter C_i$, thus (by the usual argument) they
  agree on all faces of $C_i$. In particular $epsilon gamma,epsilon delta$ agree
  on all faces of $C_m=C'$. Thus $epsilon gamma(A)=epsilon delta(A)$. Since
  $gamma(A),delta(A)$ are in $S(C)$ this gives $gamma(A)=delta(A)$.
  ...
  $delta(C)=rho_(C')(C)$, $delta$ coincides with $rho_(C')$ on each face of $C$.
  Similarly $delta^(-1)$ coincides with $rho_C$ on each face of $C'$. Thus
  $delta rho_C$ is an endomorphism of $Sigma$ into $S(C')$:
  $
  ...
  $gamma$ leaves invariant the type of each face of $C'$. Since any two chambers
  may be joined by a gallery we may assume that $C,C'$ are adjacent. Now $gamma$
  ...
  $gamma(C')$ has just one face of each type, $gamma$ leaves invariant the type
  of each face of $C'$.
  #qed
  ...
  #smallcaps[Proof.] Let $alpha,beta$ be a pair of opposite foldings of $Sigma$
  with respect to which $rho$ is defined. Let $C,C'$ be a pair of adjacent
  chambers such that $C in alpha(Sigma)$, $C' in beta(Sigma)$. Then $rho$ leaves
  invariant each face of $C inter C'$ and interchanges $C,C'$. Thus $rho$
  preserves the type of $C$ and of each face of $C inter C'$, hence $rho$
  preserves the type of each face of $C$. By #book-ref("l", "15-3-4", [15.3.4])
  ...
  #block(width: 100%, breakable: false)[
    ...
    #smallcaps[Proposition 15.3.6.] _Let $Sigma$ be an abstract Coxeter complex.
    Then $W(Sigma)$ is the group of all type-preserving automorphisms of
    $Sigma$._
  ]
  ...
  of $Sigma$. Since any two chambers can be joined by a gallery it follows from
  #book-ref(
    "p",
    "15-2-7",
    [15.2.7],
  ) and the fact that $Sigma$ is a Coxeter complex that there exists
  $gamma in W(Sigma)$ such that $gamma(C)=delta(C)$. Thus $gamma^(-1) delta$ is
  a type-preserving automorphism of $Sigma$ which fixes $C$. But a chamber $C$
  has just one face of each type. Thus $gamma^(-1) delta$ fixes each face of
  $C$, so must be the identity, by #book-ref("l", "15-1-2", [15.1.2]). Hence
  $delta in W(Sigma)$.
  #qed
  ...
  rise to an involutary type-preserving automorphism of $Sigma$ as in #book-ref(
    "p",
    "15-2-7",
    [15.2.7],
  ...
  #block(width: 100%, breakable: false)[
    ...
    #smallcaps[Theorem 15.3.7.] _Let $Sigma$ be an abstract Coxeter complex and
    $C$ be a chamber of $Sigma$. Then the reflections in the faces of
    codimension $1$ in $C$ generate the group $W(Sigma)$. Moreover $W(Sigma)$ is
    a Coxeter group with respect to this set of generators._ (cf. #book-ref(
      "def",
      "2-4-2",
      [2.4.2],
    ).)
  ...
  #smallcaps[Proof.] Let the reflections in the faces of codimension $1$ in $C$
  be denoted by $w_i$, $i in I$; and let $H$ be the subgroup of $W(Sigma)$
  generated by
  #source(298, "285")
  the $w_i$. We show that for each chamber $D$ of $Sigma$ there exists $w in H$
  such that $D=w(C)$. We do this by induction on $dist C D$, the result being
  ...
  $C$, so has form $w_i (C)$ for some $i in I$. Thus $w'^(-1)(D)=w_i (C)$ and so
  $D=w'w_i (C)$, where $w'w_i in H$ as required.
  ...
  We now show that the reflections $w_i$ generate $W(Sigma)$ as a Coxeter group.
  Each element $w in W(Sigma)$ can be expressed as a product
  ...
  is a gallery joining $C$ to $w(C)$ of length $k$. Thus $dist(C, w(C))<=k$ and
  so $dist(C, w(C))<=l(w)$. Suppose conversely that $dist(C, w(C))=k$. Then
  there is a gallery
  $ Gamma={C=C_0,C_1,...,C_k=w(C)} $ <eq:15-3-7-shortest-gallery>
  joining $C$ to $w(C)$. Since $C,C_1$ are adjacent we have $C_1=w_(i_1)(C)$ for
  some $i_1 in I$. Since $C_1,C_2$ are adjacent we have $C_2=w_(i_1)(C')$, where
  $C,C'$ are adjacent. Hence $C_2=w_(i_1) w_(i_2)(C)$ for some $i_2 in I$.
  Arguing in a similar way we see that $C_k=w_(i_1) w_(i_2) dots w_(i_k)(C)$
  with each $i_alpha in I$. Hence
  ...
  #source(299, "286")
  This will be sufficient, using a theorem of Matsumoto, to show that $W(Sigma)$
  is a Coxeter group. Let $w=w_(i_1) w_(i_2) dots w_(i_k)$ and write
  ...
  $C_j in alpha(Sigma)$. Then $alpha(C_(j-1))=C_j$. By definition of $alpha$ we
  have $alpha(C_(j-1))=w_(i_1)(C_(j-1))$, hence $w_(i_1)(C_(j-1))=C_j$. Thus we
  have
  $
  ...
  The proof of #book-ref("th", "15-3-7", [15.3.7]) is completed by establishing
  ...
  #block(sticky: true)[
    ...
    Suppose $W$ satisfies the condition that, whenever $w_(i_2) dots w_(i_k)$ is
    reduced but $w_(i_1) dots w_(i_k)$ is not reduced, there exists an integer
    $j$ with $2<=j<=k$ such that
  ...
  #smallcaps[Proof.] Any relation between the generators $w_i$ can be expressed
  in the form
  ...
  $w_(i_alpha) dots w_(i_(j-1))=w_(i_(alpha+1)) dots w_(i_j)$. Both expressions
  in this equation are reduced. Using this relation and $w_(i_alpha)^2=1$ we
  deduce
  $
    w_(i_1) dots w_(i_r)
  ...
  where both expressions are reduced, is a consequence of the Coxeter relations
  $(w_i w_j)^(m_(i j))=1$. We again use induction on $s$. Since
  ...
  Both expressions in this equation are reduced, and by cancelling $w_(k_1)$ we
  see by induction that this relation can be deduced from the Coxeter relations.
  ...
  can be deduced from these relations. Repeating the argument a number of times,
  we eventually have to show that
  $
...
#[
  ...
  == 15.4 The Complex $Sigma(W, Pi)$ <sec:15-4>

  We shall now show, as is to be expected, that the Coxeter complex as defined
  in #section-ref("2.6") is an abstract Coxeter complex.
  ...
  #enum(
    numbering: "(i)",
    [_The Coxeter complex $Sigma$ of $W$ (defined as in #section-ref("2.6")),
    with partial order $prec.eq$ defined by $K_1 prec.eq K_2$ if and only if
    $K_1$ is contained in the closure $overline(K)_2$ of $K_2$._],
  ...
  #source(302, "289")
  closure $overline(C)$ are the elements $C_J$ defined in #section-ref("2.6").
  The stabilizer of $C_J$ under the action of $W$ on $Sigma$ is $W_J$, by
  #book-ref(
    "p",
    "2-6-1",
    [2.6.1],
  ). Moreover, each element of $Sigma$ can be expressed in the form $w(C_J)$ for
  exactly one subset $J$ of $Pi$, by #book-ref("p", "2-6-3", [2.6.3]). Let
  ...
  is a bijection between $Sigma$ and $Sigma(W, Pi)$. We show that this bijection
  preserves the partial order relations.
  ...
  particular $w^(-1) w'(C_K)$ is in $overline(C)$. However, the only elements of
  the complex contained in $overline(C)$ are those of the form $C_L$, for some
  subset $L$ of $Pi$, and if $w^(-1) w'(C_K)=C_L$ then we have $L=K$ by our
  earlier remarks. Thus $w^(-1) w' in W_K$ and $C_K$ is contained in
  $overline(C)_J$. It is clear from the definitions of $C_J,C_K$ that this
  implies that $J$ is a subset of $K$. Thus $w(C_J) succ.eq w'(C_K)$ if and only
  if $w^(-1) w' in W_K$ and $J$ is a subset of $K$.
  ...
  $prec.eq$ are the elements of $W$. Two elements $w_1,w_2$ of $W$ are adjacent
  if and only if $w_2=w_1 w_r$ for some $r in Pi$. Since the fundamental
  reflections $w_r$ generate $W$, it follows that $Sigma(W, Pi)$ is a chamber
  complex. $Sigma(W, Pi)$ is thin, because the elements of codimension $1$ are
  sets of the form ${w,w w_r}$ for some $w in W$, $r in Pi$. Thus each element
  of codimension $1$ is contained in two chambers. To show that $Sigma(W, Pi)$
  is an abstract Coxeter complex it remains to prove the existence of all
  possible foldings.
  ...
  Thus $W=W_1 union W_2$ and $W_1 inter W_2$ is empty. Let $alpha$ be the map of
  $Sigma(W, Pi)$
  #source(303, "290")
  ...
  $alpha(w W_J)=w W_J$ for all subsets $J$ of $Pi$. Thus $alpha$ operates as the
  identity on all the faces of the chamber $w$. Now suppose $w in W_2$. Then
  $alpha(w)=w_s w$, and
  $ alpha(w W_J)=w_s w W_J $ <eq:15-4-1-folded-negative-face>
  whenever $w W_J subset.eq W_2$. We show that $alpha(w W_J)=w_s w W_J$ even if
  $w W_J subset.eq.not W_2$ and this will prove that $alpha$ induces an
  isomorphism between the faces of $w$ and the faces of $w_s w$. Now $w in W_2$
  and so $w^(-1)(s) in Phi^-$. Also $w W_J subset.eq.not W_2$, so there is some
  element $w_J in W_J$ such that $w w_J in W_1$. Then
  $w_J^(-1) w^(-1)(s) in Phi^+$, whence $w^(-1)(s) in W_J (Phi^+)$. Thus we have
  $
  ...
  #block(width: 100%, breakable: false)[
    ...
    #smallcaps[Proposition 15.4.2.] _The group of type-preserving automorphisms
    of $Sigma(W, Pi)$ is isomorphic to $W$._
  ]
  ...
  #source(304, "291")
  (The notation is as in #book-ref("th", "15-4-1", [15.4.1]).) By replacing $w'$
  ...
  Coxeter complex whose group of type-preserving automorphisms is isomorphic to
  $W$.
  ...
  natural 1–1 correspondence with $rho_1,rho_2,...,rho_l$. Let $J$ be any subset
  of ${1,2,...,l}$ and
  $ C_J=inter.big_(i in J) A_i. $ <eq:15-4-3-fundamental-face>
  Then the elements $C_J$ are all faces of $C$, and every element of $Sigma$ is
  expressible in the form $w(C_J)$ for some $w in W(Sigma)$ and some $J$.

  Consider the complex $st C_J$. This is also an abstract Coxeter complex by
  #book-ref("l", "15-3-1", [15.3.1]). The group of type-preserving automorphisms
  of $st C_J$ is
  #source(305, "292")
  isomorphic to the subgroup $W_J$ of $W(Sigma)$ generated by the elements
  $rho_i$ for $i in J$, also by #book-ref("th", "15-3-7", [15.3.7]). In
  particular, $W_J$ operates transitively on the chambers of $Sigma$ containing
  $C_J$, and $abs(W_J)$ is the number of such chambers. Now $W_J$ stabilizes
  $C_J$ since each of its generators has this property. Suppose the stabilizer
  $H$ of $C_J$ in $W$ were greater than $W_J$. Then $H$ would operate on the
  chambers containing $C_J$, and so the chambers $w(C)$ for $w in H$ could not
  all be distinct. However, this contradicts #book-ref("l", "15-1-2", [15.1.2]).
  ...
  the chamber $w$ in $Sigma(W, Pi)$. Moreover, it induces an isomorphism between
  the faces $w(C_J)$ of $w(C)$ and the faces $w W_J$ of $w$. Thus it is an
  isomorphism between $Sigma$ and $Sigma(W, Pi)$.
  #qed
...
#[
  ...
  == 15.5 Buildings <sec:15-5>

  #metadata((kind: "anchor")) <def:15-5-building>
  A _building_ is a pair $(Omega,cal(A))$ where $Omega$ is a chamber complex and
  $cal(A)$ is a set of subcomplexes, called _apartments_, satisfying the
  following axioms:
  ...
  $Sigma_2$ containing $C_1,C_2$; etc. By #book-ref("def", "15-5-b4", [B4]) we
  have
  $
  ...
  The subgroups of $G$ containing $B$ are in 1–1 correspondence with the subsets
  $J$ of $I$, by #book-ref("th", "8-3-2", [8.3.2]), and have the form
  $P_J=lr(⟨B,n_i;i in J⟩)$ by #book-ref("p", "8-3-1", [8.3.1]). Let $Omega$ be
  the set of left cosets $g P_J$ for all $g in G$ and all subsets $J$ of $I$. We
  introduce a partial ordering on $Omega$ which is the reverse of
  ...
  we define $cal(A)$ to be the family of subsets $g Sigma_0$ of $Omega$ for all
  $g in G$. We shall show that $Omega$ is a building and that $cal(A)$ is a set
  of apartments in $Omega$. The building constructed in this way will be called
  $Omega(G; B, N)$.
  ...
  index $2$ in $P_J$ and hence normal in $P_J$, and we would have $n_i B n_i=B$,
  which contradicts #book-ref("def", "15-5-bn5", [BN5]). It follows that $g B$
  and $g n_i B$ are not the only chambers containing $g P_J$. Thus $Omega$ is
  thick.

  Now $P_J=B union B n_i B$ and the only cosets of $B inter N$ in $N inter P_J$
  are $B inter N$ and $n_i (B inter N)$ by #book-ref("p", "8-3-1", [8.3.1]) and
  #book-ref("th", "8-3-4", [8.3.4]). Thus $B$ and $n_i B$ are the only chambers
  containing $P_J$ of the form $n B$ for $n in N$. It follows that $n B$ and
  $n n_i B$ are the only chambers in $Sigma_0$ containing $n P_J$. Thus
  $Sigma_0$ is thin, and hence all the other apartments of $Omega$ are thin
  also.
  ...
  Finally, suppose we have two elements of $Omega$ and two apartments which each
  contain both these elements. By multiplying on the left by a suitable element
  of $G$ we may assume one of the apartments is $Sigma_0$ and that the elements
  are $P_J$ and $n P_K$, where $n in N$. Let the other apartment be $g Sigma_0$.
  We show there is an isomorphism between $Sigma_0$ and $g Sigma_0$ which leaves
  invariant $P_J,n P_K$ and all their faces. Since $P_J in g Sigma_0$ we have
  $g^(-1)P_J in Sigma_0$ and so $g^(-1)P_J=n'P_J$ for some $n' in N$. Thus
  $g n'=p_J in P_J$, and $g Sigma_0=p_J Sigma_0$. Now $n P_K in p_J Sigma_0$ and
  so $p_J^(-1)n P_K=n'P_K$ for some $n' in N$. It follows that
  $ P_J n P_K=P_J n'P_K. $ <eq:15-5-1-double-cosets>
  We now require the following lemma.

  #block(width: 100%, breakable: false)[
    #metadata((kind: "anchor")) <l:15-5-2>
    #smallcaps[Lemma 15.5.2.] _Let $G$ be a group with a $(B,N)$-pair. Then for
    each $n in N$ we have_
  ...
  We have $P_J n P_K=P_J n'P_K$ and so, by #book-ref("l", "15-5-2", [15.5.2]),
  ...
  #book-ref("def", "15-5-b4", [B4]) has been established, and so
...
#[
  ...
  == 15.6 Retractions onto an Apartment <sec:15-6>
  ...
  $Sigma' arrow.r Sigma$ which leaves invariant all faces of $C$. By #book-ref(
    "l",
    "15-1-2",
    [15.1.2],
  ) there is only one such isomorphism. The image of $A$ under this isomorphism
  is an element of $Sigma$ which is independent of the choice of $Sigma'$, by
  #book-ref("def", "15-5-b4", [B4]). This image will be called
  $retr_(Sigma,C)(A)$.
  ...
  #smallcaps[Proof.] Let $Sigma$ be an apartment of a building $Omega$. We know
  that $Sigma$ is a thin chamber complex and so must prove the existence of all
  possible foldings. Let $C,C'$ be adjacent chambers of $Sigma$ and let
  ...
  as shown below. #align(center)[#retraction-scheme()] <fig:15-6-2-retractions>
  ...
  Furthermore $alpha$ and $beta$ both leave invariant all faces of $A$. We shall
  show that $alpha,beta$ are a pair of opposite foldings of $Sigma$, thus
  proving that $Sigma$ is an abstract Coxeter complex.

  We define a set $Gamma_(Sigma)(A)$ of galleries of $Sigma$. A gallery $Gamma$
  of $Sigma$ lies in $Gamma_(Sigma)(A)$ if:
  ...
  Thus $Gamma_(Sigma)(A)$ is the set of galleries of $Sigma$ which start from a
  chamber containing $A$ and reach their destination as quickly as possible. We
  shall show that if $Gamma in Gamma_(Sigma)(A)$ then
  ...
  gallery from a chamber containing $A$ to $D$. This distance can be computed in
  any apartment containing $A$ and $D$: applying a retraction onto that
  apartment to a shorter gallery in $Omega$ would give a shorter gallery there,
  after removing any repetitions.
  ...
  this image has no repetitions and is still of least length. Thus $alpha$ maps
  $Gamma_(Sigma)(A)$ into itself. Similarly $Gamma_(Sigma)(A)$ is mapped into
  itself by $beta$.

  Let $Gamma={C=C_0,C_1,...,C_m}$ be a gallery in $Gamma_(Sigma)(A)$. We show by
  induction on $m$ that all faces of $C_m$ are invariant under $alpha$ and under
  $alpha beta$, but that $C_m$ is not fixed by $beta$. These facts are clear if
  $m=0$, since $alpha(C)=C$, $beta(C)=C'$ and $alpha beta(C)=alpha(C')=C$.
  Suppose therefore that $m>0$ and write $gamma=alpha$ or $alpha beta$. By
  induction $gamma$ fixes all faces of $C_(m-1)$. Now $gamma(C_m)$ contains
  $C_(m-1) inter C_m$, and so must be either $C_(m-1)$ or $C_m$. However,
  $gamma(C_m)=C_(m-1)$ would imply that
  $ gamma(Gamma)={C=gamma(C_0),gamma(C_1),...,C_(m-1),C_(m-1)} $
  <eq:15-6-2-collapsed-gallery>
  is not in $Gamma_(Sigma)(A)$, contrary to the fact that $alpha,beta$ transform
  $Gamma_(Sigma)(A)$ into itself. Hence $gamma$ fixes $C_m$. It also fixes all
  faces of $C_(m-1) inter C_m$, and so it fixes all faces of $C_m$.
  ...
  $D$. Then $alpha(Gamma)$ is also a gallery in $Gamma_(Sigma)(A)$ and has first
  term $C$ and last term $alpha(D)$. Thus $alpha$ fixes all the faces of
  ...
  Hence there are just two chambers in $Sigma$ such that $alpha(E)=D$, viz., $D$
  and $beta(D)$. Therefore $alpha$ is a folding. It follows by symmetry that
  $beta$ is also a folding, and by definition that $alpha,beta$ are opposite
  foldings. Thus $Sigma$ is an abstract Coxeter #box[complex.#qed]

  #block(sticky: true)[
    #metadata((kind: "anchor")) <def:15-6-3>
    _Definition 15.6.3._ Let $A,A'$ be two elements of a building $Omega$. Then
    $A,A'$ are said to have the same type in $Omega$ if they have the same type
    in any apartment containing $A,A'$.
  ]
  ...
  $Sigma_1$ onto the simplex $S(C)$ defined in #book-ref(
    "p",
    "15-3-2",
    [15.3.2],
...
#[
  ...
  #source(312, "299")
  == 15.7 Groups of Type-Preserving Automorphisms <sec:15-7>
  ...
  #block(sticky: true)[
    ...
    #smallcaps[Proposition 15.7.1.] _In the building $Omega(G; B, N)$, the group
    $G$ operates by left multiplication as a group of type-preserving
    automorphisms which is transitive on the pairs $(C,Sigma)$, where $C$ is a
    chamber and $Sigma$ is an apartment containing $C$._
  ]

  #smallcaps[Proof.] We use the notation of #book-ref("ex", "15-5-1", [15.5.1]).
  The elements of $Omega$ are cosets of the form $g P_J$. Consider the apartment
  ...
  same type in $Omega$ if and only if they have the same type in some apartment
  containing both. Thus $g_1 P_J$ and $g_2 P_K$ have the same type if and only
  if $J=K$. It is now clear that the map of $Omega$ into itself given by
  $ g P_J arrow.r x g P_J, quad x in G, $ <eq:15-7-1-left-action>
  is a type-preserving automorphism of $Omega$.

  Let $C$ be a chamber and $Sigma$ an apartment of $Omega$ containing $C$. Then
  $Sigma=g Sigma_0$ for some $g in G$, and $C=g n B$ for some $n in N$. Let
  $x=g n$. Then $C=x B$ and $Sigma=x Sigma_0$. Thus the element $x$ of $G$
  ...
  #block(sticky: true)[
    ...
    #smallcaps[Theorem 15.7.2.] _Let $(Omega,cal(A))$ be a building and $G$ be a
    group of type-preserving automorphisms of $Omega$ which is transitive on the
    pairs $(C,Sigma)$ with_
  ]
  #source(313, "300")
  _$C in Sigma$, where $C$ is a chamber and $Sigma$ an apartment of $Omega$. Let
  $C_0,Sigma_0$ be a fixed chamber and apartment with $C_0 in Sigma_0$, let $B$
  be the stabilizer of $C_0$ in $G$ and $N$ be the stabilizer of $Sigma_0$. Then
  the subgroups $B,N$ form a $(B,N)$-pair in $G$. Moreover, $W=N\/(B inter N)$
  is isomorphic to the group $W(Sigma)$ of type-preserving automorphisms of each
  apartment $Sigma$ of $Omega$._
  ...
  hypothesis shows that $N$ operates transitively on the chambers of $Sigma_0$.
  Thus $N$ induces on $Sigma_0$ the full group $W(Sigma_0)$ of type-preserving
  automorphisms, by #book-ref("l", "15-1-2", [15.1.2]). Thus we have an
  ...
  We now show that the subgroups $B,N$ satisfy the axioms for a $(B,N)$-pair. It
  has already been verified that $B inter N$ is normal in $N$ and that
  ...
  condition shows there exists $b_1 in B$ such that $g^(-1)(Sigma)=b_1(Sigma)$.
  Also, since $C_0$ is in $Sigma_0$ and $Sigma$, there exists $b_2 in B$ such
  that $b_2(Sigma_0)=Sigma$. Thus $g^(-1)b_2(Sigma_0)=b_1 b_2(Sigma_0)$ and it
  follows that $b_2^(-1)g b_1 b_2 in N$. Hence $g in B N B$ and we have
  $G=B N B$.

  Let $n_i$ be an element of $N$ which induces on $Sigma_0$ a generating
  involution $w_i$ of $W$ $(i in I)$. We show $n_i B n_i != B$. Suppose this is
  false, so that $n_i B=B n_i$. Then we have
  ...
  $C'$ containing $C_0 inter n_(i)(C_0)$. Let $Sigma$ be an apartment containing
  $C_0$ and $C'$. By transitivity there exists $b in B$ such that
  ...
  $A$. Thus the chambers $n C_0,n n_i C_0$ and $n b n_i C_0$ all contain $n A$.
  ...
  term of the gallery $Gamma$. Suppose this is false, and let $C_i$ be the first
  term in $Gamma$ not in $Sigma$. Let $D$ be the chamber of $Sigma$ other than
  $C_(i-1)$ containing $C_(i-1) inter C_i$. Then
  $
    retr_(Sigma,D)(C_(i-1)) & =C_(i-1), \
        retr_(Sigma,D)(C_i) & =C_(i-1)
  $ <eq:15-7-2-gallery-collapse>
  and $retr_(Sigma,D)(Gamma)$ is a gallery whose first term is $C_0$, whose last
  term contains $n A$, and which has two consecutive terms identical. This
  contradicts the fact that the length of $Gamma$ is minimal. Thus $Sigma$
  contains each term of $Gamma$. Similarly $Sigma_0$ contains $C_0$ and $n A$,
  so contains each term of $Gamma$.

  We shall show by induction on $i$ that $b_1 C_i=C_i$. This is clear if $i=0$.
  Assume inductively that $b_1 C_(i-1)=C_(i-1)$. Then
  ...
  $b_1 n b n_i C_0$ is either $n C_0$ or $n n_i C_0$. If $b_1 n b n_i C_0=n C_0$
  we have $n b n_i in B n B$, and if $b_1 n b n_i C_0=n n_i C_0$ we have
  $n b n_i in B n n_i B$. Thus
  ...
  J. Tits has recently carried out a detailed investigation of the buildings of
  the various different types. In particular he has shown that a finite building
