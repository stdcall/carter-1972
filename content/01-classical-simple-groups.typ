#import "main-defs.typ": V, bib-ref, ix, nx, source
#source(14, "1")
...
#nx(
  ...
  [$G F(q)$],
  group: "latin-g",
  after: "G-2-twisted-2-3-2m-plus1",
)[The Galois field with $q$ elements]The theory of finite simple groups is at
present at an extremely interesting stage of development. Until about 1955 the
only known finite simple groups were the cyclic groups of prime order, the
alternating groups, the families of classical simple groups over a finite field
discovered by Jordan #bib-ref(
  "jordan1870",
...
Since that time the families of classical simple groups have been described in a
unified way by means of the Lie theory, following the fundamental work of
...
simple groups known at the time of writing, with the exception of the cyclic and
alternating groups, may be regarded as groups of Lie type over finite fields.

In addition, the five Mathieu groups have been supplemented by the discovery
during the last few years of several new ‘sporadic’ simple groups, and it seems
not at all unlikely that these will be followed by others.

At the same time, classification theorems for simple groups have been proved of
increasing strength. Following the Feit-Thompson theorem, proving that every
non-cyclic simple group has even order, results have been established by Brauer,
Gorenstein, Suzuki, Walter and others classifying simple groups in many cases
where the Sylow 2-subgroup or the centralizer of an involution is assumed to be
known. These results impose severe restrictions on the structure of simple
groups as yet undiscovered. In fact it is not at all clear whether the simple
groups known at present constitute ‘almost all’ the simple groups which exist,
or whether there are large numbers of such groups still to be discovered.
...
order to do this some knowledge of root systems and Euclidean reflection groups
is required, and also some knowledge of the simple #nx(
  "K",
  [$K$],
  group: "latin-k",
)[A field]Lie algebras over the complex field. The results about reflection
groups have been developed from first principles in @ch:weyl-groups[chapter],
and the results we shall require about Lie algebras have been collected together
in @ch:simple-lie-algebras[chapter]. With this introductory material, we begin
in @ch:chevalley-groups[chapter] an exposition of the properties of Chevalley
groups, regarded as groups of automorphisms of Lie algebras. The properties of
the Lie families of simple groups are to a large extent independent of the field
over which the groups are defined, and this field will usually be an arbitrary
one. However we do specialize from time to time to the complex field or a finite
field.

In the present introductory chapter we shall #ix("linear groups")describe the
classical groups of #nx("frak-V", [$frak(V)$], group: "latin-v")[A vector
  space]non-singular linear transformations of a vector space.

== The Linear Groups <sec:linear-groups>
#nx("G-L-n-K", [$G L_n (K)$], group: "latin-g", after: "G-F-q")[The general
  linear group of degree $n$ over $K$]#nx(
  "P-G-L-n-K",
  [$P G L_n (K)$],
  group: "latin-p",
  after: "P-W-1-t",
)[The projective general linear group of degree $n$ over $K$]#nx(
  "nu-f",
  [$nu(f)$],
  group: "greek-ν",
)[The Witt index of a form $f$]Let #V be a vector space of dimension $n$ over a
field $K$. The group of all non-singular linear transformations of #V into
itself is called the general linear group $italic("GL")_n (K)$. The
transformations of determinant 1 form a normal subgroup $italic("SL")_n (K)$,
the special linear group. The factor group
$italic("GL")_n (K)\/italic("SL")_n (K)$ is isomorphic to the multiplicative
group of non-zero elements of $K$. The centre $Z$ of $italic("GL")_n (K)$
consists of all transformations of form $T(x)=lambda x$ for $lambda in K$ with
$lambda != 0$. The factor group $italic("GL")_n (K)\/Z$ is the projective
general linear group $italic("PGL")_n (K)$. It operates on the projective space
of dimension $n-1$ associated with #V. The centre of $italic("SL")_n (K)$ is the
subgroup $Z inter italic("SL")_n (K)$, and the factor group
$italic("SL")_n (K)\/Z inter italic("SL")_n (K)$ is the projective special
linear group $italic("PSL")_n (K)$.

The projective special linear groups are generally simple. In fact
$italic("PSL")_n (K)$ is a simple group for all $n >= 2$, except for the groups
$italic("PSL")_2(2)$ and $italic("PSL")_2(3)$ (cf. Huppert #bib-ref(
  "huppert1967",
), p. 182). The finite simple groups in this family are obtained by taking for
$K$ the Galois field $italic("GF")(q)$, where $q$ is some #nx(
  "S-L-n-K",
  [$S L_n (K)$],
  group: "latin-s",
  after: "frak-S-n",
)[The special linear group of degree $n$ over $K$]#nx(
  "S-L-n-K",
  [$S L_n (K)$],
  group: "latin-s",
  after: "frak-S-n",
)[The special linear group of degree $n$ over $K$]power of a prime. The
projective special linear group over this field is denoted by
$italic("PSL")_n (q)$ and its order is given by #nx(
  "order-group",
  [$bar.v thin bar.v$],
  group: "symbols",
  locator: false,
...
#nx(
  ...
  group: "latin-p",
  after: "P-G-L-n-K",
)[The projective symplectic group of degree $n$ over $K$]#nx(
  "S-p-n-K",
  [$S p_n (K)$],
  group: "latin-s",
  after: "S-L-n-K",
  definition: [@sec:symplectic-groups],
)[The symplectic group of degree $n$ over $K$]#block()[
  #ix("symplectic groups")We now suppose that the vector space #V is endowed
  with a non-singular bilinear scalar product which associates with each pair
  $x, y$ of elements of #V an element $(x, y)$ of $K$.
  #block(sticky: true)[
...
for all $x,y in frak(V)$. The isometries form a subgroup of $italic("GL")_n (K)$
called the symplectic group $italic("Sp")_n (K)$. Now this group is, to within
isomorphism, independent of the choice of the scalar product. In fact any
non-singular skew-symmetric scalar product can be represented, with respect to a
...
In particular, the dimension of any non-singular symplectic space is even. With
respect to the above basis the symplectic transformations are represented by
matrices $T$ satisfying the condition
...
#block()[
  ...
  $italic("PSp")_4(2)$. The symplectic transformations of a 2-dimensional space
  are just those of determinant 1, therefore we have
  $
...
#nx("O-2l-plus-q", [$O_(2l)^+(q)$], group: "latin-o", after: "O-n-K-f")[The
  orthogonal group of degree $2l$ over $G F(q)$ leaving invariant a quadratic
  form of index $l$]#nx(
  "O-2l-minus-q",
  [$O_(2l)^-(q)$],
  group: "latin-o",
  ...
  quadratic form of index $l-1$]#nx(
  "O-n-K-f",
  [$O_n (K,f)$],
  group: "latin-o",
)[The orthogonal group of degree $n$ over $K$ leaving invariant the quadratic
  form $f$]#nx("Omega-n-K-f", [$Omega_n (K,f)$], group: "greek-ω")[The
  commutator subgroup of the orthogonal group $O_n (K,f)$]#nx(
  "P-S-O-n-K-f",
  [$P S O_n (K,f)$],
  group: "latin-p",
  after: "P-S-p-n-K",
)[The projective special orthogonal group of degree $n$ over $K$ leaving
  invariant the quadratic form $f$]#nx(
  "S-O-n-K-f",
  [$S O_n (K,f)$],
  group: "latin-s",
  after: "S-p-n-K",
  ...
  [$Omega_n (K,f)$],
  group: "greek-ω",
)[The commutator subgroup of the orthogonal group $O_n (K,f)$]#ix(
  "orthogonal groups",
...
@sec:orthogonal-groups-characteristic-two[section].) We assume that there is
defined on #V a non-singular bilinear scalar product which is symmetric, so that
...
The number of inequivalent quadratic forms which exist on #V is dependent on the
field $K$. The most important invariant of a quadratic #source(
  18,
  "5",
)form is its index, which is defined in the following way. A subspace $frak(U)$
of #V is called isotropic if
...
subspaces of #V all have the same dimension (cf. Dieudonné #bib-ref(
  "dieudonne1948",
)), and this dimension is called the index of $f$. We shall denote the index of
$f$ by $nu$. $nu$ cannot be greater than $1/2 n$, and we say that a form $f$ has
maximal index if $nu=1/2 n$ when $n$ is even and $nu=1/2(n-1)$ when $n$ is odd.
...
$italic("PSO")_n (K,f)$ are generally simple. This is not so, however, and it is
necessary to descend to a smaller subgroup. We denote by $Omega_n (K,f)$ the
commutator subgroup of $O_n (K,f)$. Since $O_n (K,f)\/italic("SO")_n (K,f)$ is
abelian, $Omega_n (K,f)$ is a subgroup of $italic("SO")_n (K,f)$. We define the
...
We now describe the families of finite simple orthogonal groups. Let $K$ be the
finite field $italic("GF")(q)$ and #V be an $n$-dimensional vector space over
$K$. We distinguish between the cases when $n$ is odd and even. Suppose first
that $n$ is odd, and let $n=2l+1$. Then there are just two inequivalent
non-singular symmetric scalar products on #V, which can be represented #source(
  19,
  "6",
)by the matrices $A$ and $epsilon A$, where $epsilon$ is a non-square in $K$ and
$
  A=mat(1, 0, dots.c, 0; 0, 0, , I_l; dots.v, , , ; 0, I_l, , 0).
$ <eq:orthogonal-groups-odd-form>
These two scalar products give rise to the same orthogonal group $O_(2l+1)(q)$,
...
#nx(
  ...
  $f$]We now consider a vector space #V of dimension $n$ over a field $K$ and
suppose $lambda arrow overline(lambda)$ is an automorphism of $K$ of order 2.
Suppose #V is endowed with a non-singular Hermitian scalar product $(x,y)$. Thus
$(x,y)$ is linear in $x$, conjugate linear in $y$, and
...
$nu$ of $f$ is again defined as the dimension of the maximal isotropic subspaces
of #V.
...
automorphism of order 2 must be a field $italic("GF")(q^2)$ for some prime-power
#source(21, "8")$q$, and the involutary automorphism is given by
...
rather a different way from the orthogonal groups already considered. If $K$ is
a field of characteristic not 2 and $(x,y)$ is a symmetric scalar product on a
vector space #V over $K$, the corresponding quadratic form $f$ is defined by
$f(x)=(x,x)$, and therefore satisfies the condition
$
...
assumed to be necessarily non-singular but by a suitable choice of basis for #V
it can be represented by a matrix of form
$
  mat(
    0, 1, , , , , , , , , ;
    1, 0, , , , , , , , , ;
...
Let $n$ be the dimension of #V and $2l$ be the rank of the
@eq:orthogonal-groups-characteristic-two-scalar-matrix[above matrix]. Let
$frak(V)_0$ be the set of $x in frak(V)$ such that $(x,y)=0$ for all
...
#block(sticky: true)[
  The non-singular linear transformations $T$ of #V which satisfy the condition
]
...
A vector $x in frak(V)$ is called singular if $f(x)=0$, and a subspace of #V is
called totally singular if each vector in it is singular. Any two maximal
totally singular subspaces of #V have the same dimension $nu$, and $nu$ is
called the index of $f$. As before $nu$ is at most $1/2 n$.

We shall again denote by $Omega_n (K,f)$ the commutator subgroup of $O_n (K,f)$.
...
$n=2l$ is even.) The commutator subgroups $Omega_n (K,f)$ are usually simple. In
fact $Omega_n (K,f)$ is simple provided $n>=6$ and $nu>=1$. $Omega_n (K,f)$ need
not be simple if $n=4$ or if $n>=6$ and $nu=0$. We now describe the finite
groups of this type. A basis $e_1,e_2,dots.c,e_l,e_(-1),e_(-2),dots.c,e_(-l)$
can be chosen for #V such that $f$ is one of the two following forms:
$
...
respectively, and the respective orthogonal groups are denoted by $O_(2l)^+(q)$,
$O_(2l)^-(q)$. The orders of these groups are given by the same formulae as the
orders of the groups $O_(2l)^+(q)$, $O_(2l)^-(q)$ when $q$ is odd.

Now suppose that $f$ is a non-degenerate quadratic form of defect greater
than 0. The defect $d=n-2l$ is the dimension of the subspace $frak(V)_0$ of #V,
and on $frak(V)_0$ the form $f$ satisfies the relation
$
  f(lambda x+mu y)=lambda^2 f(x)+mu^2 f(y).
...
where $x$ lies in some suitably chosen non-singular symplectic subspace of #V of
dimension $2l$.
...
#bib-ref("dieudonne1948", "dieudonne1955"). We shall show in later chapters that
many of the simple classical groups, including all the finite families, can be
