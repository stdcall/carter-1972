...
#source(14, "1")
= 1. The Classical Simple Groups <ch:1>
== 1.1 Introduction <sec:1-1>
The theory of finite simple groups is at present at an extremely interesting
stage of development. Until about 1955 the only known finite simple groups were
the cyclic groups of prime order, the alternating groups, the families of
...
is required, and also some knowledge of the simple Lie algebras over the complex
field. The results about reflection groups have been developed from first
principles in #chapter-ref(2), and the results we shall require about Lie
algebras have been collected together in #chapter-ref(3). With this introductory
material, we begin in #chapter-ref(4) an exposition of the properties of
Chevalley groups, regarded as groups of automorphisms of Lie algebras. The
properties of the Lie families of simple groups are to a large extent
independent of the field over which the groups are defined, and this field will
usually be an arbitrary one. However we do specialize from time to time to the
complex field or a finite field.
...
normal subgroup $italic("SL")_n (K)$, the special linear group. The factor group
...
$K$ the Galois field $italic("GF")(q)$, where $q$ is some power of a prime. The
...
#source(16, "3")
== 1.3 The Symplectic Groups <sec:1-3>
#block()[
  We now suppose that the vector space #V is endowed with a non-singular
  bilinear scalar product which associates with each pair $x, y$ of elements of
  #V an element $(x, y)$ of $K$.
  #block(sticky: true)[
    We assume that this scalar product is skew-symmetric, so that
  ]
  $ (y,x)=-(x,y) $ <eq:1-3-1>
]
for all $x,y in frak(V)$. A space endowed with a scalar product of this type is
...
== 1.4 The Orthogonal Groups <sec:1-4>
...
#section-ref("1.6").) We assume that there is defined on #V a non-singular
bilinear scalar product which is symmetric, so that
$ (y,x)=(x,y) $ <eq:1-4-1>
...
field $K$. The most important invariant of a quadratic #source(18, "5")form is
its index, which is defined in the following way. A subspace $frak(U)$ of #V is
called isotropic if
...
== 1.5 The Unitary Groups <sec:1-5>
We now consider a vector space #V of dimension $n$ over a field $K$ and suppose
$lambda arrow overline(lambda)$ is an automorphism of $K$ of order 2. Suppose #V
is endowed with a non-singular Hermitian scalar product $(x,y)$. Thus $(x,y)$ is
linear in $x$, conjugate linear in $y$, and
...
== 1.6 The Orthogonal Groups in Characteristic 2 <sec:1-6>
