...
#[
  ...
  #nx("N-r-s", [$N_(r,s)$], group: "latin-n")[The structure constant of a simple
    Lie algebra determined by a pair of roots]#block(sticky: true)[Let]
  ...
  is a basis for #L. It consists of the fundamental co-roots $h_r$ together #nx(
    "e-r",
    [$e_r$],
    group: "latin-e",
  )[A root vector in a simple Lie algebra]with the set of all root vectors
  $e_r$. The elements of this basis multiply together as follows:
  ...
  The elements $N_(r,s)$ for $r,s in Phi$ #ix("structure constants")are called
  the structure constants of #L. They clearly depend upon the choice of the root
  vectors $e_r$. Our first task is to consider the relations between them.
  ...
  Using the relations obtained in
  @eq:structure-constants-cyclic-derivation[(ii)] above, we have
  $
  ...
  #block(sticky: true)[
    However, $A_(r s)=p-q$, as in @sec:roots-of-simple-lie-algebras[section],
    thus
  ]
  ...
  ] <l:root-chain-length-ratio>

  #proof[
    We consider separately the different possibilities for the $r$-chain through
    $s$, bearing in mind that the length of any $r$-chain is at most 4. We use
    the information given in @sec:dynkin-diagram[section] about the relative
  ...
  Now by the formulae obtained in
  @eq:structure-constants-cyclic-derivation[(ii)] above we have
  $
  ...
  We summarize the results we have obtained #ix("structure constants")in the
  following theorem.
  ...
  ] <th:structure-constant-relations>
...
#[
  ...
  Formula @eq:structure-constant-relations-iii[(iii)] of
  @th:structure-constant-relations[Theorem]
  suggests that, by a sufficiently careful choice of the root vectors $e_r$, it
  might be possible to arrange that
  ...
  use the isomorphism theorem for simple Lie algebras, stated in
  @th:simple-lie-algebra-isomorphism. In the notation of
  @th:simple-lie-algebra-isomorphism we define $frak(L)'=frak(L)$ and
  $p'_i=-p_i$ for $i=1,2,dots.c,l$. Then it is clear that $A_(i j)=A'_(i j)$. We
  also define $e_(p'_i)=-e_(-p_i)$ and $e_(-p'_i)=-e_(p_i)$.
  ...
  be the identity by the uniqueness part of @th:simple-lie-algebra-isomorphism.
  Thus $theta$ has order 2.

  Now by @l:positive-root-partial-sums each root $r in Phi^+$ can be expressed
  as a sum of fundamental roots
  ...
  #ix("Chevalley basis")We can now state Chevalley’s basis theorem for simple
  Lie algebras.

  #ix("Chevalley’s theorems, on existence of integral basis")#theorem()[
    #block(sticky: true)[
  ...
  #proof[
    ...
    each co-root $h_r$ is a linear combination of the fundamental co-roots with
    integer coefficients. This follows from the fact that the co-roots $h_r$ for
    $r in Pi$ form a fundamental system of the dual root system (cf.
    @p:dual-root-system).
  ]
  ...
  sign_ by $[e_r e_s]=plus.minus(p+1)e_(r+s)$, using
  @l:positive-root-partial-sums. The relation $[e_r e_(-r)]=h_r$ then determines
  the basis vectors $e_r$ for $r in Phi^-$.
  ...
  #ix("structure constants")Now every Chevalley basis determines a system of
  structure constants $N_(r,s)$ given by $[e_r e_s]=N_(r,s)e_(r+s)$. Since
  $N_(r,s)=plus.minus(p+1)$, where $p$ is defined as before, the absolute value
  of $N_(r,s)$ is determined. However, different Chevalley bases will give
  different values for the signs of the $N_(r,s)$. We consider the extent to
  which these signs can be chosen arbitrarily in determining a system of
  ...
  @sec:root-systems[section]. An ordered pair $(r,s)$ #ix(
    "special pair of roots",
  )of roots will be called a _special pair_ if $r+s in Phi$ and $0≺r≺s$. An
  ordered pair $(r,s)$ is called _extraspecial_ if $(r,s)$ is a special pair and
  if for all special pairs $(r_1,s_1)$ with $r+s=r_1+s_1$ we have $r≼r_1$. Then
  every root in $Phi^+$ which is the sum of two roots in $Phi^+$ can be
  expressed uniquely as the sum #ix("extraspecial pair of roots")of an
  extraspecial pair. Since by @l:positive-root-partial-sums every root in
  $Phi^+$ which is not in $Pi$ has this property, the extraspecial pairs are in
  1–1 correspondence with the roots in $Phi^+-Pi$.
  ...
  #proposition()[
    The signs of the structure constants $N_(r,s)$ may be chosen arbitrarily for
    extraspecial pairs $(r,s)$, and then the structure constants for all pairs
    are uniquely determined.
  ] <p:extraspecial-signs-determine-constants>
  ...
  #proof[
    ...
    $lambda_r lambda_s=lambda_(r+s)$ for such pairs. By
    @l:positive-root-partial-sums it follows that if $r in Phi^+$ and
    $r=n_1 p_1+dots.c+n_l p_l$ then
    $
    ...
    constants are uniquely determined by their values on the extraspecial pairs.
  ]
  ...
  pairs by the relations @eq:structure-constant-relations-i[(i)],
  @eq:structure-constant-relations-ii[(ii)],
  @eq:structure-constant-relations-iii[(iii)],
  @eq:structure-constant-relations-iv[(iv)] of @th:structure-constant-relations.
  To prove this consider the set of ordered pairs $(r,s)$ of roots such that
  $r+s in Phi$. If $(r,s)$ is such a pair, so are the following twelve pairs of
  roots:
  $
       (r,s), & quad (s,r), \
    (s,-r-s), & quad (-r-s,s), \
  ...
  one is a special pair. Moreover relations
  @eq:structure-constant-relations-i[(i)],
  @eq:structure-constant-relations-ii[(ii)],
  @eq:structure-constant-relations-iii[(iii)] of
  @th:structure-constant-relations determine all the structure constants in
  terms of the $N_(r,s)$ for special pairs $(r,s)$.
  ...
  $r+s+(-r_1)+(-s_1)=0$, relation @eq:structure-constant-relations-iv[(iv)] of
  @th:structure-constant-relations gives
  $
  ...
  have $overline(r)+overline(s)≺r+s$. Thus, by using relations
  @eq:structure-constant-relations-i[(i)],
  @eq:structure-constant-relations-ii[(ii)],
  @eq:structure-constant-relations-iii[(iii)],
  @eq:structure-constant-relations-iv[(iv)] of @th:structure-constant-relations,
  ...
  pair with #source(
    73,
    "60",
  )$overline(r)+overline(s)≺r+s$. By using induction on the sum $r+s$ it can be
  seen that $N_(r,s)$ is determined by the values of the structure constants on
...
#[
  ...
  #nx("M-r-s-i", [$M_(r,s,i)$], group: "latin-m")[The integer
    $1/(i!) N_(r,s) N_(r,r+s) dots N_(r,(i-1)r+s)$]#ix(
    "automorphisms, of Chevalley groups",
    sub: "of simple Lie algebras",
  )#ix(
    "exponential map",
  ...
  #lemma()[
    #block(sticky: true)[
      Let #L be a Lie algebra over a field of characteristic $0$ and $delta$ be
      a derivation of #L which is nilpotent, i.e. satisfies $delta^n=0$ for some
      $n$. Then
    ]
  ...
  ] <l:nilpotent-derivation-exponential>
  ...
  and Chevalley basis ${h_r,r in Pi;e_r,r in Phi}$. Then the map $ad e_r$ #ix(
    "derivations",
  )is a derivation of #L (cf. @sec:lie-algebras-and-subalgebras[section]) #ix(
    "nilpotent derivations",
  )and this derivation is in fact nilpotent. For we have
  $
...
#[
  ...
  $frak(L)_ZZ$ is an additive abelian group. By @th:chevalley-basis the Lie
  product of two basis vectors lies in $frak(L)_ZZ$, thus $frak(L)_ZZ$ is closed
  under Lie multiplication. $frak(L)_ZZ$ is therefore a Lie algebra over $ZZ$.

  #metadata((kind: "anchor")) <def:lie-algebra-base-change>#nx(
    "L-lie-algebra",
    [$frak(L)_K$],
    group: "latin-l",
    after: "frak-L-r",
    definition: [@def:lie-algebra-base-change],
  )[The Lie algebra over $K$ constructed from the simple Lie algebra $frak(L)$
    over $CC$] Now let $K$ be any field. We form the tensor product of the
  additive group of $K$ with the additive group of $frak(L)_ZZ$, and define
  $
  ...
  ] <p:lie-algebra-base-change>

  #proof[
    This is clear, since by @th:chevalley-basis the multiplication constants of
    #L are in $ZZ$.
  ]
  ...
  #proposition()[
    ...
    $t in K$.
  ] <p:root-element-automorphism>

  #proof[
    Observe first that $overline(x)_r (t)$ is non-singular. Since
    $x_r (zeta)x_r (-zeta)=1$, it follows immediately from the definitions that
    $overline(x)_r (t)overline(x)_r (-t)=1$. Thus $overline(x)_r (-t)$ is the
    inverse of $overline(x)_r (t)$.

    To show that $overline(x)_r (t)$ is an automorphism of $frak(L)_K$ we
    consider the effect of $overline(x)_r (t)$ on the basis of $frak(L)_K$. Let
  ...
  Now that we have established the results of @p:lie-algebra-base-change and
  @p:root-element-automorphism we shall simplify the notation. We shall write
  $h_r$ for $overline(h)_r$, $e_r$ for $overline(e)_r$, $x_r (t)$ for
  ...
  #metadata((kind: "anchor")) <def:chevalley-group>#nx(
    "x-r-t",
    [$x_r (t)$],
    group: "latin-x",
  )[The generator $exp(t op("ad") e_r)$ of a Chevalley group]#nx(
    "A-l-K",
    [$A_l (K)$],
    group: "latin-a",
    definition: [@def:chevalley-group],
  )[The Chevalley group of type $A_l$ over $K$]#nx(
    "B-l-K",
    [$B_l (K)$],
    group: "latin-b",
  )[The Chevalley group of type $B_l$ over $K$]#nx(
    "C-l-K",
    [$C_l (K)$],
    group: "latin-c",
    after: "C-l",
  )[The Chevalley group of type $C_l$ over $K$]#nx(
    "D-l-K",
    [$D_l (K)$],
    group: "latin-d",
  )[The Chevalley group of type $D_l$ over $K$]#nx(
    "E-6-K",
    [$E_6(K)$],
    group: "latin-e",
    after: "E-6",
  )[The Chevalley group of type $E_6$ over $K$]#nx(
    "E-7-K",
    [$E_7(K)$],
    group: "latin-e",
    after: "E-7",
  )[The Chevalley group of type $E_7$ over $K$]#nx(
    "E-8-K",
    [$E_8(K)$],
    group: "latin-e",
    after: "E-8",
  )[The Chevalley group of type $E_8$ over $K$]#nx(
    "F-4-K",
    [$F_4(K)$],
    group: "latin-f",
  )[The Chevalley group of type $F_4$ over $K$]#nx(
    "G-2-K",
    [$G_2(K)$],
    group: "latin-g",
    after: "G-2",
  )[The Chevalley group of type $G_2$ over $K$]#nx(
    "L-chevalley-group",
    [$frak(L)(K)$],
    group: "latin-l",
    after: "L-lie-algebra",
  )[The Chevalley group of type $frak(L)$ over $K$]#nx(
    "A-l-K",
    [$A_l (K)$],
    group: "latin-a",
    definition: [@def:chevalley-group],
    ...
    group: "latin-e",
    after: "E-7",
  )[The Chevalley group of type $E_7$ over $K$]#ix(
    "Chevalley group, definition",
  ...
  ] <p:chevalley-group-independence>

  #proof[
    We must show that $frak(L)(K)$ is independent of the choice of the #source(
      78,
      "65",
    )Chevalley basis of #L. Let ${h_r,r in Pi;e_r,r in Phi}$ be a Chevalley
    basis of #L. We must show that any Chevalley basis of #L can be transformed
    by an automorphism of #L into one of the form
    ...
    of #L (@th:simple-lie-algebra-isomorphism) we may restrict attention to
    Chevalley bases corresponding to a fixed Cartan subalgebra $frak(H)$. The
    isomorphism theorem @th:simple-lie-algebra-isomorphism also shows that there
    is an automorphism of #L which transforms any set of fundamental roots into
    any other; and also that for a given system of fundamental roots there is an
    automorphism of #L which transforms any set of fundamental root vectors
    $e_r,r in Pi$, into any other. Thus we may restrict ourselves to Chevalley
    bases in which $frak(H)$, $Pi$ and the $e_r$ for $r in Pi$ are fixed. Since
    the structure constants $N_(r,s)$ are all determined to within a sign we
    see, using @l:positive-root-partial-sums, that each root vector $e_r$ is
    determined to within a sign. Thus any Chevalley basis of #L can be
    transformed by an automorphism of #L into one of the form
    ${h_r,r in Pi;plus.minus e_r,r in Phi}$. It is now evident that the group
    generated by the elements $x_r (t)$ is independent of the Chevalley basis.
    For if $-e_r$ is chosen as a root vector instead of $e_r$, the generator
    $x_r (t)$ is simply replaced by $x_r (-t)$, its inverse. Hence the
    isomorphism type of the group $frak(L)(K)$ depends only upon #L and $K$.
  ]
...
#[
  ...
  #lemma()[
    #block(sticky: true)[
      ...
      #source(79, "66")$y in frak(L)$ is represented by a nilpotent matrix. #ix(
        "nilpotent derivations",
  ...
  ] <p:rank-one-projective-linear-group>

  #proof[
    We first apply @l:exponential-conjugation to the Lie algebra $A_1$ over
    $CC$. If $zeta in CC$ and $e_r,e_(-r)$ are the matrices defined
    @eq:groups-a1-sl2-basis[above] we have
    $
    ...
    fact will be given in @sec:group-sl2[section].) Thus there is a surjective
    homomorphism
    $
