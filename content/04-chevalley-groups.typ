#let chapter = [
  #[
    ...
    #smallcaps[Proof.] We consider separately the different possibilities for
    the $r$-chain through $s$, bearing in mind that the length of any $r$-chain
    is at most 4. We use the information given in #section-ref("dynkin-diagram")
    ...
    #smallcaps[Theorem 4.1.2.] _The structure constants of a simple Lie algebra
    #L over $CC$ satisfy the following relations:_
    #enum(
      numbering: "(i)",
      indent: 1.1em,
      body-indent: 0.5em,
  ...
  #[
    ...
    $frak(L)'=frak(L)$ and $p'_i=-p_i$ for $i=1,2,dots.c,l$. Then it is clear
    that $A_(i j)=A'_(i j)$. We also define $e_(p'_i)=-e_(-p_i)$ and
    $e_(-p'_i)=-e_(p_i)$.
    ...
    #smallcaps[Proof.] Since $A_(r s)$ and $N_(r,s)$ are integers, it only
    remains to prove that each co-root $h_r$ is a linear combination of the
    fundamental co-roots with integer coefficients. This follows from the fact
    that the co-roots $h_r$ for $r in Pi$ form a fundamental system of the dual
    root system (cf. #book-ref("p", "3-6-1", [3.6.1])). #qed
    ...
    #smallcaps[Proposition 4.2.2.] _The signs of the structure constants
    $N_(r,s)$ may be chosen arbitrarily for extraspecial pairs $(r,s)$, and then
    the structure constants for all pairs are uniquely determined._

    #smallcaps[Proof.] Let ${h_r,r in Pi;e_r,r in Phi}$ and
    ${h_r,r in Pi;e'_r,r in Phi}$ be two Chevalley bases giving rise to systems
    of structure constants $(N_(r,s))$, $(N'_(r,s))$. Thus
    $[e_r e_s]=N_(r,s)e_(r+s)$ and $[e'_r e'_s]=N'_(r,s)e'_(r+s)$.
    ...
    pairs by the relations #book-ref("eq", "4-1-2-i", [(i)]), #book-ref(
      ...
      "4-1-2-ii",
      [(ii)],
    ), #book-ref("eq", "4-1-2-iii", [(iii)]), #book-ref(
      "eq",
      "4-1-2-iv",
      [(iv)],
    ...
    exactly one is a special pair. Moreover relations #book-ref(
      ...
      "4-1-2-ii",
      [(ii)],
    ), #book-ref("eq", "4-1-2-iii", [(iii)]) of #book-ref(
      "th",
      "4-1-2",
      [4.1.2],
    ) determine all the structure constants in terms of the $N_(r,s)$ for
    special pairs $(r,s)$.
  ...
  #[
    ...
    #smallcaps[Proof.] $exp delta$ is a non-singular linear map, its inverse
    being $exp(-delta)$. Now we have
  ...
  #[
    ...
    of $frak(L)_K$ for all $t in K$. #qed

    Now that we have established the results of #book-ref("p", "4-4-1", [4.4.1])
    and #book-ref("p", "4-4-2", [4.4.2]) we shall simplify the notation. We
    shall write $h_r$ for $overline(h)_r$, $e_r$ for $overline(e)_r$, $x_r (t)$
    for $overline(x)_r (t)$, and $A_r (t)$ for $overline(A)_r (t)$. This
    omission of the bars will not lead to confusion or inconsistency since the
    objects originally called $h_r,e_r,x_r (t),A_r (t)$ are special cases of
    ...
    #smallcaps[Proof.] We must show that $frak(L)(K)$ is independent of the
    choice of the #source(78, "65")Chevalley basis of #L. Let
    ${h_r,r in Pi;e_r,r in Phi}$ be a Chevalley basis of #L. We must show that
    any Chevalley basis of #L can be transformed by an automorphism of #L into
    one of the form ${h_r,r in Pi;plus.minus e_r,r in Phi}$. Firstly, since any
    two Cartan subalgebras of #L can be transformed into one another by some
    ...
    of the form ${h_r,r in Pi;plus.minus e_r,r in Phi}$. It is now evident that
    the group generated by the elements $x_r (t)$ is independent of the
    Chevalley basis. For if $-e_r$ is chosen as a root vector instead of $e_r$,
    the generator $x_r (t)$ is simply replaced by $x_r (-t)$, its inverse. Hence
    the isomorphism type of the group $frak(L)(K)$ depends only upon #L and $K$.
    #qed
  ]
  ...
  #[
    ...
    #block(sticky: true)[
      ...
      suppose we have a representation of #L by matrices under Lie
      multiplication. Suppose #source(79, "66")$y in frak(L)$ is represented by
      a nilpotent matrix. Then $ad y$ is a nilpotent derivation of #L and_
    ]
    ...
    #smallcaps[Proof.] We first apply #book-ref("l", "4-5-1", [4.5.1]) to the
    Lie algebra $A_1$ over $CC$. If $zeta in CC$ and $e_r,e_(-r)$ are the
    matrices defined #book-ref("eq", "4-5-sl2-basis", [above], highlight: false)
    we have
    $
    ...
    fact will be given in #section-ref("group-sl2").) Thus there is a surjective
    homomorphism
