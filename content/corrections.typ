...
#let entry(id, body) = {
  ...
  let edition = record.at("edition", default: none)
  metadata((correction: id))
  block(above: 1.1em, below: 0.35em, sticky: true)[
    #text(weight: "semibold")[#id]
    #h(0.6em)
    #if edition != none { [#edition reprint p. #page] } else if page != none {
      [Original p. #page]
    } else { [Bibliography] }
...
#let correction-list() = [
  ...
  This list records 100 editorial changes and one bibliographic update, and ends
  with the author’s changes in the 1989 reprint. Page numbers refer to the
  printed 1972 edition, or to the 1989 reprint where marked. Mathematical
  clarifications are distinguished from spelling and notation corrections.
  Computations and bounded formal proofs support specific claims; they do not
  constitute a formal verification of the whole book. Detailed original readings
  and evidence are retained in the accompanying source project.
  ...
  == Chapter 14 · Properties of twisted groups
    ...
    handles the small-field cases separately. Hartley’s Lemma, added by the
    author in the 1989 reprint as a Supplement to this page, lists the values
    that do occur; it agrees with this correction and was checked by exact
    computation for all twisted types over many finite fields.]
  ...
  #entry("E100")[In the $attach(D, tl: 3)_4$ case of 14.4.1(c), the root
    subgroup satisfies $X_(S_3)^1 subset.eq (G^1)'$, replacing $in$: $X_(S_3)^1$
    is a subgroup, as in the neighbouring conclusions for $S_4$ and for all
    $S$.]
  ...
  == Chapter 16 · Sporadic simple groups
    ...
    $23$. The adjacent prose already gives the correct factor. The list of
    sporadic groups added in the 1989 reprint repeats $23$ and is corrected in
    the same way.]
  ...
  #entry("E098")[In the passage added in 1989, the representation over $RR$
    “gives” a representation over $upright("GF")(3)$, replacing “give.”]
  #entry("E099")[In the list added in 1989, the discoverers of _O’N_ are “O’Nan,
    Sims,” replacing “O’Nans, Sims.”]
  ...
  The provisional reference “Notes on finite BN-pairs, to appear” is replaced by
  ...
  The Wiley Classics Library reprint of 1989 was photographed from the 1972
  plates, with the author’s corrections pasted in. Its pages were aligned with
  those of the 1972 edition, and every line that differs was read in both. All
  of the author’s changes are adopted in this edition.

  #block(above: 1em, below: 0.4em, sticky: true)[*Corrections by the author*]
  - p. 13: the fundamental system is $Pi$, and the sum runs from $i = 1$; these
    are E001 and E002.
  - p. 37: the radical in $abs(x) = sqrt((x, x))$ receives its vinculum, which
    this edition already draws.
  - p. 175: the second index of $x_(r_1 + r_2)(1)$ is lowered; E044 also lowers
    the first.
  - p. 310: the final table names $op("Fi")'_24$, as in E087. The factor $23$ in
    the order of $op("Fi")_22$ (E086) is left uncorrected.
  - p. 269: the broken bar in $h_r -> h_(overline(r))$ is restored.

  Every other reading corrected in this list is printed unchanged in 1989. In
  chapters 13 and 14 this was checked entry by entry; p. 263 (E072) carries no
  reference to the Supplement. The copy of the reprint used here also has a
  previous owner’s pencil and ink marks, among them arrows on the multiple bonds
  of the table in 13.3.8 and notes in Italian. They are not the author’s and are
  not adopted.

  #block(above: 1em, below: 0.4em, sticky: true)[*Additions*]
  After p. 310 the reprint adds an account of the sporadic groups found since
  1971 and a list of all twenty-six sporadic groups with their orders and
  discoverers. The first of the added pages is missing from the copy of the
  reprint used here. The text therefore begins with the first complete paragraph
  of the next page, and a footnote in the book records the gap. All orders agree
  with the GAP character table library. E086, E098 and E099 correct misprints in
  the added pages.

  Below the list, a Supplement states Hartley’s Lemma, with a reference to
  p. 263. It gives the values $chi(r)$ attained by elements $h(chi) in H^1$ of
  the twisted groups, which the proof of 14.4.1(b) needs, and agrees with E072.
  The Supplement follows chapter 16 and links to that proof. An exact
  computation of the groups of values for every root confirms the lemma for
  $attach(A, tl: 2)_l$ $(2 <= l <= 7)$, $attach(D, tl: 2)_l$ $(4 <= l <= 7)$,
  $attach(E, tl: 2)_6$ and $attach(D, tl: 3)_4$ over fifteen fields, and for
  $attach(B, tl: 2)_2$, $attach(F, tl: 2)_4$ and $attach(G, tl: 2)_2$ over the
  first few fields of each kind.
]
