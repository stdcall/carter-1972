...
def body_metrics(page):
    ...
    pix = page.get_pixmap(matrix=fitz.Matrix(CONFIG["render_scale"], CONFIG["render_scale"]), clip=area,
                         colorspace=fitz.csGRAY, alpha=False)
    ink = Image.frombytes('L', (pix.width, pix.height), pix.samples)
    bounds = ink.point(lambda value: 255 if value < CONFIG["ink_threshold"] else 0).getbbox()
    bottom = (pix.y + bounds[3]) / CONFIG["render_scale"] if bounds else area.y0
    return {
...
def check_whitespace(pdf, bookmarks):
    ...
    chapter_starts = {b['pdf_page']: b['title'] for b in bookmarks
                      if b['depth'] == 0}
    first_text = next((b['pdf_page'] for b in bookmarks
                       if b['title'] == 'Preface'), 3)
    pages = []
    for i, metric in enumerate(metrics):
        ...
        if number < first_text:
            ...
            row['classification'] = 'end of document'
              ...
              and (number in chapter_starts or metric['gap_pt'] < CONFIG['short_spill_threshold_pt'])):
    ...
    return {
        ...
        'scope': ('Bottom whitespace inside the text area, measured from '
                  'rendered ink at 144 dpi. Normal margins, front matter and '
                  'ordinary chapter endings excluded; very short spill pages '
                  'still require review. Does not replace full visual QA.'),
