...
PREFIXES = {'th', 'l', 'p', 'cor', 'eq', 'fig', 'def', 'ex', 'ch', 'sec', 'pg', 'bib', 'ed'}  # ed: editorial.bib
EXPRESSION = '''(
...
def source_checks(path, text, config):
    ...
    for m in re.finditer(r'[\u0000-\u0008\u000b\u000c\u000e-\u001f\u00ad\u200b\ufeff\ufffd]|[\u0400-\u04ff]', clean):
        add('T002', m.start(), 'Control character or Cyrillic in this English corpus')
    for m in re.finditer(r'\b(?:TODO|FIXME|UNREADABLE)\b|\?\?\?', clean):
        add('T003', m.start(), 'Unresolved placeholder marker')
    labels = []
...
def semantic_checks(data, expected_pages):
    ...
    for item in data['metadata']:
        ...
        if not isinstance(value, dict):
            continue
        name = label_name(item)
        # Index marks share one label: it is the key the index queries them
        # by, not a reference target.
        if name and not (value.get('kind') == 'index-mark' and name in ('ix', 'nx')):
            semantic_labels.append(name)
        kind = value.get('kind')
        ...
        if kind == 'source':
            ...
            if printed.isdecimal() and value['file-page'] != int(printed)+13:
                add('T015', value, 'Source anchor sequence number does not match the printed page')
    ...
    if sources != expected_pages:
        add('T015', {}, f'Source coverage differs from data/source-pages.json: {sources} != {expected_pages}')
...
def input_hashes(root):
    paths = [root/'corrections.json',
             *sorted((root/'content').rglob('*.typ')),
             root/'references.bib', root/'editorial.bib',
