...
checked correspondence to Carter's definitions, not a new proof of the library
results. None of these files formalizes the entire surrounding chapter.

Run `just check-lean` with the existing sibling `book-ocr` checkout and its
prepared Lean/mathlib installation. Override `BOOK_OCR` for another checkout.
The generic runner checks the pinned toolchain and mathlib commit, source and
proof hashes, warnings and axioms. It never installs or updates an environment.
Fresh output goes to `build/.cache/lean-*`; it is not committed.

Exact hypotheses, content bindings and scope are in `bindings.json`.
The selection audit is summarized in `../README.md`.
